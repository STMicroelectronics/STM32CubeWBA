...
#ifdef PKA_HW_ACCELERATION
#include "hw.h"
static int ecp_mul_HW_PKA_P256( mbedtls_ecp_point *R,
                                const mbedtls_mpi *m,
                                const mbedtls_ecp_point *P )
{
  int ret;
  MBEDTLS_MPI_CHK( mbedtls_mpi_grow( &R->X, 8 ) );
  MBEDTLS_MPI_CHK( mbedtls_mpi_grow( &R->Y, 8 ) );
  MBEDTLS_MPI_CHK( mbedtls_mpi_grow( &R->Z, 8 ) );
  R->Z.p[0] = 1;
  while ( ! HW_PKA_Enable( ) );
  HW_PKA_P256_StartEccScalarMul( (const uint32_t*)m->p,
                                 (const uint32_t*)P->X.p,
                                 (const uint32_t*)P->Y.p );
  while ( ! HW_PKA_EndOfOperation( ) );
  HW_PKA_P256_ReadEccScalarMul( (uint32_t*)R->X.p, (uint32_t*)R->Y.p );
  HW_PKA_Disable( );
 cleanup:
  return ret;
}
#endif /* ! PKA_HW_ACCELERATION */
             ...
             mbedtls_ecp_restart_ctx *rs_ctx )
{
    ...
    if( mbedtls_ecp_get_type( grp ) == MBEDTLS_ECP_TYPE_SHORT_WEIERSTRASS )
#ifdef PKA_HW_ACCELERATION
      if ( *(uint32_t*)grp->G.X.p == 0xD898C296UL )
        ret = ecp_mul_HW_PKA_P256( R, m, P );
      else
#endif /* ! PKA_HW_ACCELERATION */
