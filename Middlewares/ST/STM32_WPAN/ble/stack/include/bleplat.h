...
/**
  * @brief  This function is called by the Bluetooth® LE stack when it is
            initialized or reset (via hci_reset). The user shall call here the functions to
            reset the Timer, AES, PKA, NVM and RNG needed for the Bluetooth® LE stack.
  * @param  None
  * @retval None
  */
...
/**
  ...
  * @param  type: The type of data to be stored either security data (BLEPLAT_NVM_TYPE_SEC) or GATT data (BLEPLAT_NVM_TYPE_GATT)
  * @param  data: The data buffer to be stored
  * @param  size: The size of data to be stored
  * @param  extra_data: If there is extra data to be stored too. If not, this parameter shall be passed with “NULL” value
...
/**
  ...
  * @param  type: The type of data to be read, either security data (BLEPLAT_NVM_TYPE_SEC)
            or GATT data (BLEPLAT_NVM_TYPE_GATT)
...
/**
  ...
  * @param  mode: Mode of deleting data from the NVM, either clear all the
            security database (BLEPLAT_NVM_ALL) or the current read NVM block (BLEPLAT_NVM_CURRENT)
  * @retval None
  */
...
/**
  ...
  * @param  local_private_key: table of 8 x 32-bit words that contains the P-256
            private key (Little Endian format)
  * @retval status (BLEPLAT_XX)
  */
...
/**
  ...
  * @param  local_private_key: table of 8 x 32-bit words that contains the local P-256 private key (Little Endian format)
  * @param  remote_public_key: table of 32 x 32-bit words that contains the remote P-256 public key (Little Endian format)
  * @retval status (BLEPLAT_XX)
  */
...
/**
  ...
  * @param  dh_key: table of 8 x 32-bit words that is filled by the function with the generated DH key (Little Endian format)
  * @retval status (BLEPLAT_XX)
  */
extern int BLEPLAT_PkaReadDhKey( uint32_t* dh_key );

/**
  * @brief  Callback function implemented by the Bluetooth® LE stack that must be
            called by the user to inform the Bluetooth® LE stack about completion of
            P-256 public key generation or DH key computation
  * @param  None
  * @retval status (BLEPLAT_XX)
  */
...
/**
  ...
  * @param  key: table of 16 bytes that contains the key to use (Little Endian format)
  * @param  input: table of 16 bytes that contains the block to encrypt
  * @param  output: table of 16 bytes that is filled by the function with the encrypted block
  * @retval None
  */
...
/**
  * @brief  CMAC computation: the function can be called several times with output_tag
            set to 0 to append data to the computation. It must be called once at the
            end with output_tag not set at 0 to complete the CMAC computation.
  * @param  input: table of “input_length” bytes that contains the data to append for the CMAC computation
  * @param  input_length: number of bytes in “input”.
  * @param  output_tag: must be set to 0 for append. Otherwise: table of 16 bytes that is filled by the function with the computed CMAC tag.
  * @retval None
  */
...
/**
  ...
  * @param  val: pointer to a 32-bit table of size ‘n’ that are filled with random values by the function.
  * @retval None
  */
...
/**
  ...
  * @param  layer: The timer ID to be started
...
/**
  ...
  * @param  layer: The timer ID to be stopped
  * @retval None
  */
extern void BLEPLAT_TimerStop( uint8_t layer );

/**
  * @brief  The timer callback function called when the timeout of a given timer has elapsed
  * @param  layer: The timer ID
  * @retval None
  */
