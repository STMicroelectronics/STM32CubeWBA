 ...
 */
/*lint -emacro(413, MCP_END_OF) "null pointer in left argument to operator [MISRA 2012 Rule 1.3 (REQUIRED)]" */
/*lint -emacro(923, MCP_END_OF) "cast from pointer to unsigned long [MISRA Rule 11.6 (REQUIRED)]" */
/*lint -emacro(9087, MCP_END_OF) "cast obj A * <-> obj B * [MISRA Rule 11.3 (REQUIRED)]" */
#define MCP_END_OF(_type_, _member_)    ((size_t)(&((_type_ *)NULL)->_member_) + sizeof(((_type_ *)NULL)->_member_))
...
#define MCP_ASSOC_RSP_STATUS_ADDR_CONFLICT  0xf0U
...
struct mcp_wsstartreq {
     ...
     * and blacklisted EAPOL parents (MCP_TLV_ATTRIBUTEs). */
};
...
struct mcp_wsstartconf {
    ...
    uint8_t seqno;
    uint8_t flags; /* MCP_FLAG_CONFIRM */
    uint8_t status; /* mcp_STATUS_SUCCESS or mcp_STATUS_INVALID_PARAMETER */
    uint8_t rsv[3];
};

/* WS-START.indication */
#define MCP_WISUN_STATE_DISABLED        0
#define MCP_WISUN_STATE_NO_PAN          1
#define MCP_WISUN_STATE_ACQUIRE_KEYS    2
#define MCP_WISUN_STATE_ACQUIRE_CONFIG  3
#define MCP_WISUN_STATE_ROUTE_CONFIG    4
#define MCP_WISUN_STATE_OPERATIONAL     5

struct mcp_wsstartind {
    uint8_t cmd; /* MCP_WISUN_START */
    uint8_t hdr_rsv;
    uint8_t seqno;
    uint8_t flags; /* MCP_FLAG_INDICATION */
    uint8_t status; /* mcp_STATUS_SUCCESS or mcp_STATUS_CHANNEL_ACCESS_FAILURE */
    uint8_t state; /* MCP_WISUN_STATE_... */
    uint8_t rsv[2];
    /* TLVs may follow to define additional state-dependent information. */
};
...
#define MCP_IDENT_PROD_STR_MAX_LEN          7U
...
struct mcp_identconf {
    ...
    uint32_t hw_id;
    uint32_t hw_rev;
    char pname[MCP_IDENT_PROD_STR_MAX_LEN + 1U]; /* Equivalent to license product string */
};
#define MCP_AMEND_802154e  0x0001U
#define MCP_AMEND_802154g  0x0002U
...
#define MCP_ERROR_NONE          0U
#define MCP_ERROR_GENERAL       1U
#define MCP_ERROR_UNSUPPORTED   2U
#define MCP_ERROR_MALFORMED     3U
#define MCP_ERROR_OVERFLOW      4U
...
/*lint -emacro(740, MCP_TLV_TYPE) "cast to incompatible indirect type [MISRA Rule 1.3 (REQUIRED)]" */
/*lint -emacro(9087, MCP_TLV_TYPE) "cast obj A * <-> obj B * [MISRA Rule 11.3 (REQUIRED)]" */
/*lint -emacro(9005, MCP_TLV_TYPE) "attempt to cast away const/volatile [MISRA Rule 11.8 (REQUIRED)]" */
/*lint -emacro(9087, MCP_TLV_NEXT) "cast obj A * <-> obj B * [MISRA Rule 11.3 (REQUIRED)]" */
#define MCP_TLV_TYPE(_p_)       (((struct mcp_tlv_hdr *)(_p_))->type)
#define MCP_TLV_SIZE(_p_)       __mcp_tlv_size(_p_)
#define MCP_TLV_SPACE(_len_)    (((_len_) + 7U) & ~7U)
 ...
 */
/*lint -emacro(946, MCP_TLV_FOREACH) "Relational/subtract operation on pointers [MISRA Rule 18.2 (REQUIRED), MISRA Rule 18.3 (REQUIRED)]" */
/*lint -emacro(9087, MCP_TLV_FOREACH) "cast obj A * <-> obj B * [MISRA Rule 11.3 (REQUIRED)]" */
/*lint -emacro(9005, MCP_TLV_FOREACH) "attempt to cast away const/volatile [MISRA Rule 11.8 (REQUIRED)]" */
...
/* MCP TLV type codes. */
#define MCP_TLV_PAD         0U /* 0x00 */
#define MCP_TLV_PAYLOAD     1U /* 0x01 - When present, always the final TLV. */
#define MCP_TLV_ENERGYLIST  2U /* 0x02 */
#define MCP_TLV_PANDESC     3U /* 0x03 */
#define MCP_TLV_SECURITY    4U /* 0x04 */
#define MCP_TLV_RANGING     5U /* 0x05 */
#define MCP_TLV_UWB         6U /* 0x06 */
#define MCP_TLV_HIE         7U /* 0x07 */
#define MCP_TLV_PIE         8U /* 0x08 */
#define MCP_TLV_BCN_BITMAP  9U /* 0x09 */
#define MCP_TLV_HOPPING     10U /* 0x0a */
#define MCP_TLV_MPM_SCAN    11U /* 0x0b */
#define MCP_TLV_EBEACON     12U /* 0x0c */
#define MCP_TLV_MODE_SWITCH 13U /* 0x0d */
#define MCP_TLV_ATTRIBUTE   14U /* 0x0e */
#define MCP_TLV_REGISTER    15U /* 0x0f */
#define MCP_TLV_PENDADDR    16U /* 0x10 */
#define MCP_TLV_LINKQUALITY 17U /* 0x11 */
#define MCP_TLV_MIE         18U /* 0x12 */
#define MCP_TLV_TSCH_DIAG   19U /* 0x13 */
#define MCP_TLV_PHY_DIAG    20U /* 0x14 */

/*lint -emacro(9005, MCP_TLV_IS_IE) "attempt to cast away const/volatile [MISRA Rule 11.8 (REQUIRED)]" */
...
#define MCP_TLV_IS_VENDOR_IE(_p_) ( \
        (MCP_TLV_TYPE(_p_) == MCP_TLV_HIE && ((struct mcp_ie *)(_p_))->id == (uint8_t)mcp_IEEE802154_HIE_VENDOR_SPECIFIC)    \
        || (MCP_TLV_TYPE(_p_) == MCP_TLV_PIE && ((struct mcp_ie *)(_p_))->id == (uint8_t)mcp_IEEE802154_PIE_VENDOR_SPECIFIC)    \
        || (MCP_TLV_TYPE(_p_) == MCP_TLV_MIE && ((struct mcp_ie *)(_p_))->id == (uint8_t)mcp_IEEE802154_MIE_VENDOR_SPECIFIC) \
        )

static inline unsigned int
__mcp_tlv_size(const void *tlv)
{
    /*lint -save -e9087 [cast performed between pointer of different types - MISRA 2012 Rule 11.3, required] */
    const struct mcp_tlv_hdr *t = (const struct mcp_tlv_hdr *)tlv;
    /*lint -restore */
    unsigned int sz;
...
/* Payload TLV header (without flex-array) for strict C99 compliance, when placed in parent structures. */
struct mcp_payload_hdr {
    uint8_t type; /* MCP_TLV_PAYLOAD */
    uint8_t len; /* MUST be zero. */
    uint16_t datalen;
    uint16_t protocol; /* MCP_PAYLOAD_PROTOCOL_... */
...
/* Payload protocols. See also IEEE 802.3 Ethertypes (www.iana.org/assignments/ieee-802-numbers/ieee-802-numbers.xhtml). */
#define MCP_PAYLOAD_PROTOCOL_UNSPEC     0x0000U /* Protocol not specified. */
#define MCP_PAYLOAD_PROTOCOL_KMP        0x0001U /* Key Management Protocol. */
#define MCP_PAYLOAD_PROTOCOL_6LOWPAN    0xA0EDU /* 6LoWPAN Encapsulated. */
...
/* EnergyList TLV header (without flex-array) for strict C99 compliance, when placed in parent structures. */
struct mcp_energylist_hdr {
    uint8_t type; /* MCP_TLV_ENERGYLIST */
    uint8_t len; /* MCP_TLV_LEN(sizeof(struct mcp_energylist) + num_channels) */
    uint16_t ch_offset; /* Channel Number (N) = index_into_energy[] + ch_offset */
    uint8_t rsv[4];
...
/* IE TLV header (without flex-array) for strict C99 compliance, when placed in parent structures. */
struct mcp_ie_hdr {
    uint8_t type; /* MCP_TLV_HIE, MCP_TLV_PIE or MCP_TLV_MIE */
    uint8_t len; /* MCP_TLV_LEN(sizeof(struct mcp_ie) + ielen) */
    uint16_t ielen;
...
/* Beacon Bitmap TLV header (without flex-array) for strict C99 compliance, when placed in parent structures. */
struct mcp_bbits_hdr {
    uint8_t type; /* MCP_TLV_BCN_BITMAP */
    uint8_t len; /* MCP_TLV_LEN(sizeof(struct mcp_bbits) + sizeof(bits)) */
    uint8_t rsv[2];
    uint16_t index;
...
/* Attribute TLV Header (without flex-array) for strict C99 compliance, when placed in parent structures. */
struct mcp_attr_hdr {
    uint8_t type; /* MCP_TLV_ATTRIBUTE */
    uint8_t len; /* MCP_TLV_LEN(sizeof(struct mcp_attr) + sizeof(value)) */
    uint16_t id; /* PIBAttributeID */
...
/* Register TLV header (without flex-array) for strict C99 compliance, when placed in parent structures. */
struct mcp_register_hdr {
    uint8_t type; /* MCP_TLV_REGISTER */
    uint8_t len; /* MCP_TLV_LEN(sizeof(struct mcp_register) + strlen(name) + 1) */
    uint16_t offset;
    uint32_t value;
...
/* Pending Address TLV header (without flex-array) for strict C99 compliance, when placed in parent structures. */
struct mcp_pendaddr_hdr {
    uint8_t type; /* MCP_TLV_PENDADDR */
    uint8_t len; /* MCP_TLV_LEN(sizeof(struct mcp_pendaddr) + sizeof(addrlist)) */
    uint8_t mode;
    uint8_t rsv[5];
...
/*lint -emacro(413, MCP_STATISTICS_SIZE_V*) "null pointer in left argument to operator [MISRA 2012 Rule 1.3 (REQUIRED)]" */
/*lint -emacro(923, MCP_STATISTICS_SIZE_V*) "cast from pointer to unsigned long [MISRA Rule 11.6 (REQUIRED)]" */
/*lint -emacro(9087, MCP_STATISTICS_SIZE_V*) "cast obj A * <-> obj B * [MISRA Rule 11.3 (REQUIRED)]" */
...
/* Security KeyDescriptor header (without flex-array) for strict C99 compliance, when placed in parent structures. */
struct mcp_keydesc_hdr {
    struct mcp_attr_hdr attr;
    uint64_t usage;
    uint8_t keymode; /* MCP_KEYMODE_... */
    uint8_t keyindex;
...
/* Channel Plan Attribute (ID==0x0212) */
struct mcp_channel_plan {
    struct mcp_attr_hdr attr;
    uint64_t ch0_freq; /* in Hz */
    uint32_t ch_spacing; /* in Hz */
...
/* Wi-SUN Region Attribute (ID==0x0220) */
struct mcp_wsregion {
    struct mcp_attr_hdr attr;
    uint16_t reg_domain;
    uint16_t op_class;
...
/* Wi-SUN FAN 1.1 Channel Plan Descriptor Attribute (ID==0x0222) */
struct mcp_wschplandesc {
    struct mcp_attr_hdr attr;
    uint16_t reg_domain;
    uint16_t ch_plan_id;
    uint8_t rsv[4];
