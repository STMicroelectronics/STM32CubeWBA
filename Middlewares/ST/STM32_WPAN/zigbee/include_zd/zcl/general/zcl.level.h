 ...
 * LC.S.C07.Rsp | Stop (with On/Off) | True
 * LC.S.C08.Rsp | Move to Freq | True
 *
 * Client Attributes
...
/* Level Command IDs  */
enum {
    ZCL_LEVEL_CLI_CMD_MOVE_LEVEL = 0x00, /**< Move to Level */
    ZCL_LEVEL_CLI_CMD_MOVE = 0x01, /**< Move */
    ZCL_LEVEL_CLI_CMD_STEP = 0x02, /**< Step */
    ZCL_LEVEL_CLI_CMD_STOP = 0x03, /**< Stop */
    ZCL_LEVEL_CLI_CMD_MOVE_LEVEL_ONOFF = 0x04, /**< Move to Level (with On/Off) */
    ZCL_LEVEL_CLI_CMD_MOVE_ONOFF = 0x05, /**< Move (with On/Off) */
    ZCL_LEVEL_CLI_CMD_STEP_ONOFF = 0x06, /**< Step (with On/Off) */
    ZCL_LEVEL_CLI_CMD_STOP_ONOFF = 0x07, /**< Stop (with On/Off) */
    ZCL_LEVEL_CLI_CMD_MOVE_FREQ = 0x08 /**< Move to Closest Frequency */
};
...
struct ZbZclLevelClientMoveToFreqT {
...
struct ZbZclLevelServerCallbacksT {
    ...
    /**< Callback to application, invoked on receipt of a Move To Level command. Set with_onoff to true in the req struct when utilizing the
     * onoff cluster on the same endpoint. The application is expected to update the ZCL_LEVEL_ATTR_CURRLEVEL attribute. */
    ...
    /**< Callback to application, invoked on receipt of a Move command. Set with_onoff to true in the req struct when utilizing the onoff
     * cluster on the same endpoint. The application is expected to update the ZCL_LEVEL_ATTR_CURRLEVEL attribute. */
    ...
    /**< Callback to application, invoked on receipt of a Step command. Set with_onoff to true in the req struct when utilizing the onoff
     * cluster on the same endpoint. The application is expected to update the ZCL_LEVEL_ATTR_CURRLEVEL attribute. */
    ...
    /**< Callback to application, invoked on receipt of a Stop command. Set with_onoff to true in the req struct when utilizing the onoff
     * cluster on the same endpoint. The application is expected to update the ZCL_LEVEL_ATTR_CURRLEVEL attribute. */

    enum ZclStatusCodeT (*move_to_freq)(struct ZbZclClusterT *cluster,
        struct ZbZclLevelClientMoveToFreqT *req, struct ZbZclAddrInfoT *srcInfo, void *arg);
    /**< Callback to application, invoked on receipt of a Move to Frequency command.
     * The application is expected to update the ZCL_LEVEL_ATTR_CURRFREQ attribute. */
};
...
enum ZclStatusCodeT ZbZclLevelClientMoveToFreqReq(struct ZbZclClusterT *cluster,
    const struct ZbApsAddrT *dst, struct ZbZclLevelClientMoveToFreqT *req,
    void (*callback)(struct ZbZclCommandRspT *rsp, void *arg), void *arg);
