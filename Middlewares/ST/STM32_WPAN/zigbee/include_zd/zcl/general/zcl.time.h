...
struct ZbZclTimeServerCallbacks {
     ...
     * Returns current time in seconds since Zigbee Epoch: Jan 1st 2000. */

    void (*set_time)(struct ZbZclClusterT *clusterPtr, uint32_t time_val, void *arg);
    /**< Callback to application, invoked on receipt of Write Attribute request for the
     * ZCL_TIME_ATTR_TIME attribute. The set_time app callback should also set the
     * ZCL_TIME_ATTR_LAST_SET_TIME attribute if successful.
     * The time_val parameter is the current time in seconds since Zigbee Epoch: Jan 1st 2000. */
};
 ...
 * @param cluster Cluster instance from which to send this command
 * @return Current time in seconds since Zigbee Epoch: Jan 1st 2000
 */
