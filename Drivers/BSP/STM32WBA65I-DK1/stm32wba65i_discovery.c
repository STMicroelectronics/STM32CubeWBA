/**
  ******************************************************************************
  * @file    stm32wba65i_discovery.c
  * @author  MCD Application Team
  * @brief   This file provides a set of firmware functions to manage
  *          - LEDs
  *          - Joystick
  *          - COM port
  *          available on STM32WBA65I_DK1 board (MB2143) from
  *          STMicroelectronics.
  ******************************************************************************
  * @attention
  *
...
/** @defgroup STM32WBA65I_DK1 STM32WBA65I-DK1
  * @{
  */
...
static GPIO_TypeDef *LED_PORT[LEDn] = {LD5_GPIO_PORT, LD6_GPIO_PORT};
...
/**
  * @brief  This method returns the STM32WBA65I-DK1 BSP Driver revision
  * @retval version: 0xXYZR (8bits for each decimal, R for RC)
  */
int32_t BSP_GetVersion(void)
{
  return (int32_t)STM32WBA65I_DK1_BSP_VERSION;
}
...
{
  return (const uint8_t *)STM32WBA65I_DK1_BSP_BOARD_NAME;
}
  ...
  if (HAL_ADC_Init(hadc) == HAL_OK)
  {
    if (HAL_ADCEx_Calibration_Start(hadc) == HAL_OK)
    {
      ...
      if ((status == HAL_OK) && (MXInit->ContinuousConvMode == ENABLE))
      {
        ...
        AnalogWDGConfig.Channel        = JOY1_ADC_CHANNEL;
...
#endif /* (USE_ADC_REGISTER_CALLBACKS == 1) */
/**
  * @}
  */
