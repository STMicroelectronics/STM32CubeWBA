/**
  ******************************************************************************
  * @file    stm32wba65i_discovery_bus.c
  * @author  MCD Application Team
  * @brief   This file provides a set of firmware functions to communicate
  *          with  external devices available on STM32WBA65I_DK1 board
...
/** @defgroup STM32WBA65I_DK1_BUS BUS
  * @{
  */

/** @defgroup STM32WBA65I_DK1_BUS_Private_Constants BUS Private Constants
  * @{
  */
...
/** @defgroup STM32WBA65I_DK1_BUS_Private_Types BUS Private Types
  * @{
  */
typedef struct
{
...
/** @defgroup STM32WBA65I_DK1_BUS_Private_Variables BUS Private Variables
  * @{
  */
...
/** @defgroup STM32WBA65I_DK1_BUS_Exported_Variables BUS Exported Variables
  * @{
  */
...
/** @defgroup STM32WBA65I_DK1_BUS_Private_FunctionPrototypes BUS Private FunctionPrototypes
  * @{
  */
...
/** @defgroup STM32WBA65I_DK1_BUS_Exported_Functions BUS Exported Functions
  * @{
  */
...
#endif
    /* Init the SPI */
    if (MX_SPI3_Init(&hbus_spi3, SPI_GetPrescaler( HAL_RCC_GetPCLK1Freq(), BUS_SPI3_BAUDRATE)) != HAL_OK)
    {
      ret = BSP_ERROR_BUS_FAILURE;
    }
  }
...
{
  int32_t ret = BSP_ERROR_NONE;
  int16_t remaining_length = Length;

  if (IS_SPI_LIMITED_INSTANCE(hbus_spi3.Instance))
  {
    while (( remaining_length > BUS_SPI3_TSIZE_MAX) && (ret == BSP_ERROR_NONE))
    {
      if(HAL_SPI_Transmit(&hbus_spi3, pData, BUS_SPI3_TSIZE_MAX, BUS_SPI3_TIMEOUT) == HAL_OK)
      {
        ret = BSP_ERROR_NONE;
      }
      else
      ...
      remaining_length = remaining_length - BUS_SPI3_TSIZE_MAX;
      pData += BUS_SPI3_TSIZE_MAX;
    }
    if ((ret == BSP_ERROR_NONE) && (HAL_SPI_Transmit(&hbus_spi3, pData, remaining_length, BUS_SPI3_TIMEOUT) == HAL_OK))
    {
      ret = BSP_ERROR_NONE;
    }
    else
...
int32_t  BSP_SPI3_Recv(uint8_t *pData, uint16_t Length)
{
  ...
  if (IS_SPI_LIMITED_INSTANCE(hbus_spi3.Instance))
  {
    while (( remaining_length > BUS_SPI3_TSIZE_MAX) && (ret == BSP_ERROR_NONE))
    {
      if(HAL_SPI_Receive(&hbus_spi3, pData, BUS_SPI3_TSIZE_MAX, BUS_SPI3_TIMEOUT) == HAL_OK)
      {
        ret = BSP_ERROR_NONE;
      }
      else
    ...
    if ((ret == BSP_ERROR_NONE) && (HAL_SPI_Receive(&hbus_spi3, pData, remaining_length, BUS_SPI3_TIMEOUT) == HAL_OK))
    {
      ret = BSP_ERROR_NONE;
    }
    else
  ...
  {
    if(HAL_SPI_Receive(&hbus_spi3, pData, Length, BUS_SPI3_TIMEOUT) == HAL_OK)
    {
      ret = BSP_ERROR_NONE;
    }
  }
...
static void SPI3_MspInit(SPI_HandleTypeDef* hspi)
{
  ...
  GPIO_InitStructure.Pin       = BUS_SPI3_MISO_PIN;
  GPIO_InitStructure.Mode      = GPIO_MODE_AF_PP;
  GPIO_InitStructure.Pull      = GPIO_NOPULL;
  GPIO_InitStructure.Speed     = GPIO_SPEED_FREQ_HIGH;
  GPIO_InitStructure.Alternate = BUS_SPI3_MISO_AF;
  HAL_GPIO_Init(BUS_SPI3_MISO_GPIO_PORT, &GPIO_InitStructure);
...
/** @defgroup STM32WBA65I_DK1_BUS_Private_Functions BUS Private Functions
  * @{
  */
/**
