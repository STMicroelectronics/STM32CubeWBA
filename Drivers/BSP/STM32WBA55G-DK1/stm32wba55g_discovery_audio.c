/**
  ******************************************************************************
  * @file    stm32wba55g_discovery_audio.c
  * @author  MCD Application Team
  * @brief   This file provides the Audio driver for the STM32WBA55G_DK1 board.
  ******************************************************************************
  * @attention
  *
  ...
  ******************************************************************************
  ...
  ==============================================================================
   + This driver supports STM32WBAxx devices on STM32WBA55G_DK1 (MB1802 and MB1803) board.
   ...
   + For each mode, you may need to implement the relative callback functions into your code.
      ...
      the stm32wba55g_discovery_audio.h file. (refer to the example for more details on the callbacks implementations)
   ...
   + The driver API and the callback functions are at the end of the stm32wba55g_discovery_audio.h file.
...
/** @defgroup STM32WBA55G_DK1_AUDIO AUDIO
  * @{
  */

/** @defgroup STM32WBA55G_DK1_AUDIO_Private_Macros AUDIO Private Macros
  * @{
  */
...
/** @defgroup STM32WBA55G_DK1_AUDIO_Exported_Variables AUDIO Exported Variables
  * @{
  */
...
/** @defgroup STM32WBA55G_DK1_AUDIO_Private_Variables AUDIO Private Variables
  * @{
  */
...
/** @defgroup STM32WBA55G_DK1_AUDIO_Private_Function_Prototypes AUDIO Private Function Prototypes
  * @{
  */
...
/** @addtogroup STM32WBA55G_DK1_AUDIO_OUT_Exported_Functions
  * @{
  */
/**
    ...
    if (status == BSP_ERROR_NONE)
    {
      ...
      if (status == BSP_ERROR_NONE)
      {
        ...
        else
        {
          ...
          if (Audio_In_Ctx[0].State != AUDIO_IN_STATE_RESET)
          {
            codec_init.InputDevice  = (Audio_In_Ctx[0].Device == AUDIO_IN_DEVICE_ANALOG_MIC)
                                      ? CS42L51_IN_MIC1 : CS42L51_IN_LINE1;
          }
          else
          {
...
void HAL_SAI_TxCpltCallback(SAI_HandleTypeDef *hsai)
{
  ...
  /* Manage the remaining file size and new address offset: This function
     should be coded by user (its prototype is already declared in stm32wba55g_discovery_audio.h) */
...
__weak HAL_StatusTypeDef MX_SAI1_Init(SAI_HandleTypeDef *hsai, MX_SAI_Config_t *MXInit)
{
  ...
  if ((hsai->Instance == AUDIO_IN_SAI_PDM) && (Audio_In_Ctx[1U].State != AUDIO_IN_STATE_RESET))
  {
    ...
    if ((MXInit->AudioFrequency == AUDIO_FREQUENCY_8K) || (MXInit->AudioFrequency == AUDIO_FREQUENCY_16K) ||
        (MXInit->AudioFrequency == AUDIO_FREQUENCY_32K))
    {
      /* Set PDM clock to 1024KHz so Mckdiv to 24 (Fsai/(Fpdm*2)) */
      hsai->Init.Mckdiv = 24U;
    }
    else if ((MXInit->AudioFrequency == AUDIO_FREQUENCY_48K) || (MXInit->AudioFrequency == AUDIO_FREQUENCY_96K) ||
             (MXInit->AudioFrequency == AUDIO_FREQUENCY_192K))
    {
      /* Set PDM clock to 3072KHz so Mckdiv to 8 (Fsai/(Fpdm*2)) */
      hsai->Init.Mckdiv = 8U;
    }
    else if ((MXInit->AudioFrequency == AUDIO_FREQUENCY_11K) || (MXInit->AudioFrequency == AUDIO_FREQUENCY_22K) ||
             (MXInit->AudioFrequency == AUDIO_FREQUENCY_44K))
    {
...
/** @defgroup STM32WBA55G_DK1_AUDIO_OUT_Private_Functions AUDIO OUT Private Functions
  * @{
  */
#if (USE_HAL_SAI_REGISTER_CALLBACKS == 1)
/**
...
/** @addtogroup STM32WBA55G_DK1_AUDIO_IN_Exported_Functions
  * @{
  */
...
int32_t BSP_AUDIO_IN_Init(uint32_t Instance, BSP_AUDIO_Init_t *AudioInit)
{
  ...
  else if ((Instance == 0U) && (AudioInit->BitsPerSample != AUDIO_RESOLUTION_16B))
  {
    ret = BSP_ERROR_FEATURE_NOT_SUPPORTED;
  }
  else if ((Instance == 1U) && (AudioInit->BitsPerSample != AUDIO_RESOLUTION_8B))
  {
    ret = BSP_ERROR_FEATURE_NOT_SUPPORTED;
  }
  else if ((Instance == 0U) && (AudioInit->ChannelsNbr != 1U) && (AudioInit->Device == AUDIO_IN_DEVICE_ANALOG_MIC))
  {
    ret = BSP_ERROR_FEATURE_NOT_SUPPORTED;
  }
  else if ((Instance == 0U) && (AudioInit->ChannelsNbr != 2U) && (AudioInit->Device == AUDIO_IN_DEVICE_LINE_IN))
  {
    ret = BSP_ERROR_FEATURE_NOT_SUPPORTED;
  }
  ...
  else if ((Instance == 1U) && (Audio_In_Ctx[0].State != AUDIO_IN_STATE_RESET))
  {
    ret = BSP_ERROR_FEATURE_NOT_SUPPORTED;
  }
  else
    ...
    if (Instance == 0U)
    {
      ...
      if (ret == BSP_ERROR_NONE)
      {
        ...
        if (ret == BSP_ERROR_NONE)
        {
          ...
          mx_config.MonoStereoMode        = (AudioInit->Device == AUDIO_IN_DEVICE_ANALOG_MIC) ? SAI_MONOMODE :
                                            SAI_STEREOMODE;
          ...
          mx_config.SlotActive            = (AudioInit->Device == AUDIO_IN_DEVICE_ANALOG_MIC) ? SAI_SLOTACTIVE_0 :
          ...
          if (ret == BSP_ERROR_NONE)
          {
            ...
            codec_init.Resolution   = CS42L51_RESOLUTION_16B; /* Not used */
            codec_init.Volume       = AudioInit->Volume;
            codec_init.InputDevice  = (AudioInit->Device == AUDIO_IN_DEVICE_ANALOG_MIC) ? CS42L51_IN_MIC1 :
                                      CS42L51_IN_LINE1;
...
int32_t BSP_AUDIO_IN_RecordPDM(uint32_t Instance, uint8_t *pBuf, uint32_t NbrOfBytes)
{
  ...
  else
  {
    ...
    if (HAL_SAI_Receive_DMA(&haudio_in_sai, (uint8_t *)pBuf, (uint16_t)NbrOfBytes) != HAL_OK)
    {
      ret = BSP_ERROR_PERIPH_FAILURE;
    }
    else
...
int32_t BSP_AUDIO_IN_SetDevice(uint32_t Instance, uint32_t Device)
{
  ...
  {
    if ((Instance == 0U) && (Audio_In_Ctx[Instance].Device != Device))
    {
      MX_SAI_Config_t mx_config;
      ...
      mx_config.AudioMode             = SAI_MODESLAVE_RX;
      mx_config.ClockStrobing         = SAI_CLOCKSTROBING_FALLINGEDGE;
      mx_config.MonoStereoMode        = (Device == AUDIO_IN_DEVICE_ANALOG_MIC) ? SAI_MONOMODE : SAI_STEREOMODE;
      mx_config.DataSize              = SAI_DATASIZE_16;
      mx_config.FrameLength           = 32;
      mx_config.ActiveFrameLength     = 16;
      mx_config.OutputDrive           = SAI_OUTPUTDRIVE_ENABLE;
      mx_config.Synchro               = SAI_SYNCHRONOUS;
      mx_config.SynchroExt            = SAI_SYNCEXT_DISABLE;
      mx_config.SlotActive            = (Device == AUDIO_IN_DEVICE_ANALOG_MIC) ? SAI_SLOTACTIVE_0 :
                                        (SAI_SLOTACTIVE_0 | SAI_SLOTACTIVE_1);
      ...
      else
      {
        ...
        codec_init.InputDevice  = (Device == AUDIO_IN_DEVICE_ANALOG_MIC) ? CS42L51_IN_MIC1 : CS42L51_IN_LINE1;
...
int32_t BSP_AUDIO_IN_SetSampleRate(uint32_t Instance, uint32_t  SampleRate)
{
  ...
  else
  {
    ...
    else /* (Instance == 1U) */
    {
      ...
      /* Prepare haudio_in_sai handle */
      mx_config.AudioFrequency        = SampleRate;
      mx_config.AudioMode             = SAI_MODEMASTER_RX;
      mx_config.ClockStrobing         = SAI_CLOCKSTROBING_FALLINGEDGE;
      mx_config.MonoStereoMode        = SAI_STEREOMODE;
      mx_config.DataSize              = SAI_DATASIZE_8;
      ...
      /* Re-initialize SAI1 with new sample rate */
      else if (MX_SAI1_Init(&haudio_in_sai, &mx_config) != HAL_OK)
      {
        ret = BSP_ERROR_PERIPH_FAILURE;
      }
      /* Store new sample rate on audio in context */
...
int32_t BSP_AUDIO_IN_SetBitsPerSample(uint32_t Instance, uint32_t BitsPerSample)
{
  ...
  else if ((Instance == 0U) && (BitsPerSample != AUDIO_RESOLUTION_16B))
  {
    ret = BSP_ERROR_FEATURE_NOT_SUPPORTED;
  }
  else if ((Instance == 1U) && (BitsPerSample != AUDIO_RESOLUTION_8B))
  {
    ret = BSP_ERROR_FEATURE_NOT_SUPPORTED;
  }
  /* Check audio in state */
...
int32_t BSP_AUDIO_IN_SetChannelsNbr(uint32_t Instance, uint32_t ChannelNbr)
{
  ...
  else if ((Instance == 0U) && (Audio_In_Ctx[Instance].Device == AUDIO_IN_DEVICE_ANALOG_MIC) && (ChannelNbr != 1U))
  {
    ret = BSP_ERROR_FEATURE_NOT_SUPPORTED;
  }
  else if ((Instance == 0U) && (Audio_In_Ctx[Instance].Device == AUDIO_IN_DEVICE_LINE_IN) && (ChannelNbr != 2U))
  {
    ret = BSP_ERROR_FEATURE_NOT_SUPPORTED;
  }
...
void BSP_AUDIO_IN_IRQHandler(uint32_t Instance, uint32_t InputDevice)
{
  if (((Instance == 0U) || (Instance == 1U)) &&
      ((InputDevice == AUDIO_IN_DEVICE_DIGITAL_MIC) || (InputDevice == AUDIO_IN_DEVICE_ANALOG_MIC) ||
       (InputDevice == AUDIO_IN_DEVICE_LINE_IN)))
  {
...
/** @defgroup STM32WBA55G_DK1_AUDIO_IN_Private_Functions AUDIO IN Private Functions
  * @{
  */
...
/** @defgroup STM32WBA55G_DK1_AUDIO_Private_Functions AUDIO Private Functions
  * @{
  */
...
static void SAI_MspInit(SAI_HandleTypeDef *hsai)
{
  ...
  if (hsai->Instance == AUDIO_IN_SAI)
  {
    if (Audio_In_Ctx[0U].State == AUDIO_IN_STATE_INIT) /* SAI interface */
    {
      /* Enable SAI clock */
...
static void SAI_MspDeInit(SAI_HandleTypeDef *hsai)
{
  ...
  if (hsai->Instance == AUDIO_IN_SAI)
  {
    if (Audio_In_Ctx[0U].State != AUDIO_IN_STATE_RESET) /* SAI interface */
    {
      /* Disable SAI DMA Channel IRQ */
