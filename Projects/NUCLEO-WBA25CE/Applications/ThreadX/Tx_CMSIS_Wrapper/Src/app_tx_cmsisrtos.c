...
/**
  ******************************************************************************
  * @file    ThreadX/Tx_CMSIS_Wrapper/Src/app_tx_cmsisrtos.c
...
static void ThreadOne_Entry(void *argument)
{
  ...
  while(1)
  {
    ...
    if (APP_SYNC_GET(SyncObject, DEFAULT_WAIT_TIME) == osOK)
    {
      ...
      /*sync object acquired, toggle the LED_BLUE each 500ms for 5s */
      Led_Toggle(LED_BLUE_GPIO_Port, LED_BLUE_Pin, 10);
...
/**
  ...
  * @param  GPIOx: Pointer to GPIO port where the LED is connected
  * @param  GPIO_Pin: GPIO pin number corresponding to the LED
