...
static char a_GapDeviceName[] = {  'B', 'e', 'a', 'c', 'o', 'n', ' ', 'A', 'o', 'A', ' ', 'T', 'a', 'g' }; /* Gap Device Name */
...
void APP_BLE_Init(void)
{
  ...
  /* Setting parameters for the function aci_gap_start_observation_proc */
  ret = aci_gap_start_observation_proc(SCAN_INTERVAL_MS * 1000 / 625, /* LE_Scan_Interval: Scan interval in units of 0.625 ms */
                                       SCAN_WINDOW_MS * 1000 / 625,   /* LE_Scan_Window: Scan window in units of 0.625 ms */
                                       PASSIVE_SCAN,                  /* LE_Scan_Type: Scan type (0x00 for passive scanning) */
                                       0x00,                          /* Own_Address_Type: Own address type (0x00 for public address) */
                                       0x00,                          /* Filter_Duplicates: Duplicate filtering (0x00 for disabled) */
                                       0x00                           /* Scanning_Filter_Policy: Scanning filter policy (0x00 for basic unfiltered scanning filter policy) */
                                       );
  if (ret != BLE_STATUS_SUCCESS)
  {
    LOG_INFO_APP("aci_gap_start_observation_proc() failed:0x%02x\r\n", ret);
  }
  else
  {
    LOG_INFO_APP("aci_gap_start_observation_proc --> SUCCESS\r\n");
  } 
   
  
  /* Setting parameters for the function hci_le_periodic_advertising_create_sync */
  ret = hci_le_periodic_advertising_create_sync(0x00,                 /* Options: Use Advertising_SID, Advertiser_Address_Type, and Advertiser_Address parameters */
                                                0x00,                 /* Advertising_SID: Value of the Advertising SID subfield in the ADI field of the PDU (0x00) */
                                                0x00,                 /* Advertiser_Address_Type: Public Device Address or Public Identity Address (0x00) */
                                                beacon_address,       /* Advertiser_Address: Address of the advertiser */
                                                0x0000,               /* Skip: Maximum number of periodic advertising events that can be skipped (0x0000) */
                                                SYNC_TIMEOUT_MS / 10, /* Sync_Timeout: Synchronization timeout for the periodic advertising train in units of 10 ms */
                                                0x10                  /* Sync_CTE_Type: Do not sync to packets without a Constant Tone Extension (0x10) */
                                                );
  if (ret != BLE_STATUS_SUCCESS)
  {
    LOG_INFO_APP("hci_le_periodic_advertising_create_sync() failed:0x%02x\r\n", ret);
  }
  else
  {
    LOG_INFO_APP("hci_le_periodic_advertising_create_sync --> SUCCESS\r\n");
  }
    ...
    case HCI_LE_META_EVT_CODE:
    {
      ...
      switch (p_meta_evt->subevent)
      {
        ...
        case HCI_LE_EXTENDED_ADVERTISING_REPORT_SUBEVT_CODE:
        {
          ...
          uint16_t event_type =  p_ext_adv_report->Event_Type;
          
          if(event_type == 0x0010)
          {
            print_beacon_data(p_ext_adv_report->Data_Length, p_ext_adv_report->Data, "Beacon");
          }
          else if(event_type == 0x0000)
          {
            print_beacon_data(p_ext_adv_report->Data_Length, p_ext_adv_report->Data, "ExtAdv Beacon");
          }
      ...
      case HCI_LE_PERIODIC_ADVERTISING_SYNC_ESTABLISHED_SUBEVT_CODE:
        {
          ...
          LOG_INFO_APP("hci_le_periodic_advertising_sync_established_event --> EVENT\n");
          
          /* Scanning is no more needed. Disable it. */
          ret = aci_gap_terminate_gap_proc(GAP_OBSERVATION_PROC);
          if (ret != BLE_STATUS_SUCCESS)
          {
            LOG_INFO_APP("aci_gap_terminate_gap_proc() failed:0x%02x\n", ret);
          }
          else
          {
          ...
          if (ret != BLE_STATUS_SUCCESS)
          {
            LOG_INFO_APP("hci_le_set_connectionless_iq_sampling_enable() failed:0x%02x\n", ret);
          }
          else
          {
            LOG_INFO_APP("hci_le_set_connectionless_iq_sampling_enable --> SUCCESS\n");
          }
          break;
        }
      case HCI_LE_CONNECTIONLESS_IQ_REPORT_SUBEVT_CODE:
        {
          ...
          LOG_INFO_APP("IQ samples (%s): ", cte_type_str[p_connless_iq_report->CTE_Type]);
          for(int i = 0; i < p_connless_iq_report->Sample_Count; i++)
          {
            LOG_INFO_APP("(%d,%d)", p_connless_iq_report->IQ_Sample[i].I_Sample, p_connless_iq_report->IQ_Sample[i].Q_Sample);
          }
          LOG_INFO_APP("\n");
          break;
        }
      case HCI_LE_PERIODIC_ADVERTISING_SYNC_LOST_SUBEVT_CODE:
        {
          LOG_INFO_APP("Sync lost! Restart the scanner device to get again the Beacon AoA Tag IQ samples !\n");
          break;
        }
...
static void print_beacon_data(uint16_t adv_data_len, uint8_t * adv_data, char * head_string)
{
  ...
  while(i < adv_data_len){
       ...
       memcmp(&adv_data[i+2], ibeacon_company_id, 2) ==0 && memcmp(&adv_data[i+4], ibeacon_beacon_type, 2) == 0)
    {
      ...
      LOG_INFO_APP("%s UUID: %08X-%04X-%04X-%04X-%08X%04X ", head_string, (unsigned int) BE_TO_HOST_32(uuid),(unsigned int) BE_TO_HOST_16(uuid+4),(unsigned int) BE_TO_HOST_16(uuid+6),\
        (unsigned int) BE_TO_HOST_16(uuid+8),(unsigned int) BE_TO_HOST_32(uuid+10),(unsigned int) BE_TO_HOST_16(uuid+14));
      LOG_INFO_APP("Major: %d  Minor: %d ", major, minor);
      beacon_found = TRUE;
      break;
    }
    i += AD_len+1;
