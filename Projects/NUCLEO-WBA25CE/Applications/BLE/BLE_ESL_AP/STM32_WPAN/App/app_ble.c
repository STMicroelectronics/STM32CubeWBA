...
typedef __PACKED_STRUCT packed_Periodic_Advertising_Response_t_s {
...
static char a_GapDeviceName[] = {  'E', 'S', 'L', '_', 'A', 'P' }; /* Gap Device Name */
...
static uint8_t analyse_adv_report(uint8_t adv_data_size, const uint8_t *p_adv_data,
                                  uint8_t address_type, const uint8_t *p_address);
  ...
  switch (p_event_pckt->evt)
  {
    case HCI_DISCONNECTION_COMPLETE_EVT_CODE:
    {
      ...
      if (p_disconnection_complete_event->Connection_Handle == bleAppContext.connectionHandle)
      {
        ...
        UTIL_SEQ_SetEvt(1 << CFG_EVENT_READ_REMOTE_FEATURES_COMPLETE);
    ...
    case HCI_LE_META_EVT_CODE:
    {
      ...
      switch (p_meta_evt->subevent)
      {
        ...
        case HCI_LE_ENHANCED_CONNECTION_COMPLETE_SUBEVT_CODE:
        {
          ...
          bleAppContext.deviceServerBdAddrType = p_enhanced_conn_complete->Peer_Address_Type;
          memcpy(bleAppContext.a_deviceServerBdAddr, p_enhanced_conn_complete->Peer_Address, BD_ADDR_SIZE);
        ...
        case HCI_LE_CONNECTION_COMPLETE_SUBEVT_CODE:
        {
          ...
          bleAppContext.deviceServerBdAddrType = p_conn_complete->Peer_Address_Type;
          memcpy(bleAppContext.a_deviceServerBdAddr, p_conn_complete->Peer_Address, BD_ADDR_SIZE);
        ...
        case HCI_LE_ADVERTISING_REPORT_SUBEVT_CODE:
        {
          ...
          analyse_adv_report(p_adv_report->Advertising_Report[0].Length_Data, p_adv_report->Advertising_Report[0].Data,
                             p_adv_report->Advertising_Report[0].Address_Type, p_adv_report->Advertising_Report[0].Address);
        ...
        case HCI_LE_EXTENDED_ADVERTISING_REPORT_SUBEVT_CODE:
        {
          ...
          analyse_adv_report(p_ext_adv_report->Data_Length, p_ext_adv_report->Data,
                             p_ext_adv_report->Address_Type, p_ext_adv_report->Address);
        ...
        {
          uint32_t conn_interval_us = 0;
          hci_le_enhanced_connection_complete_v2_event_rp0 *p_enhanced_conn_complete;
          p_enhanced_conn_complete = (hci_le_enhanced_connection_complete_v2_event_rp0 *) p_meta_evt->data;
          conn_interval_us = p_enhanced_conn_complete->Conn_Interval * 1250;
          LOG_INFO_APP(">>== HCI_LE_ENHANCED_CONNECTION_COMPLETE_V2_SUBEVT_CODE - Connection handle: 0x%04X\n", p_enhanced_conn_complete->Connection_Handle);
          LOG_INFO_APP("     - Connection established with @:%02x:%02x:%02x:%02x:%02x:%02x\n",
                      p_enhanced_conn_complete->Peer_Address[5],
                      p_enhanced_conn_complete->Peer_Address[4],
                      p_enhanced_conn_complete->Peer_Address[3],
        ...
        case HCI_LE_READ_REMOTE_FEATURES_PAGE_0_COMPLETE_SUBEVT_CODE:
        {
          ...
          LOG_DEBUG_APP("LE_Features: ");
          for(int i = 0; i < 8; i++)
            LOG_DEBUG_APP("0x%02X ", p_read_remote_features_complete->LE_Features[i]);
          LOG_DEBUG_APP("\n");
        ...
        case HCI_LE_PERIODIC_ADVERTISING_SUBEVENT_DATA_REQUEST_SUBEVT_CODE:
        {
          ...
          {
            ESL_AP_SubeventDataRequest(i % PAWR_NUM_SUBEVENTS);
          }
        ...
        case HCI_LE_PERIODIC_ADVERTISING_RESPONSE_REPORT_SUBEVT_CODE:
        {
          ...
          LOG_DEBUG_APP("HCI_LE_PERIODIC_ADVERTISING_RESPONSE_REPORT tx %d, num_resp %d\n", p_periodic_adv_response_report->Tx_Status, p_periodic_adv_response_report->Num_Responses);

          periodic_adv_resp_p = (packed_Periodic_Advertising_Response_t *)&p_periodic_adv_response_report->Responses;
    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
    {
      ...
      switch (p_blecore_evt->ecode)
      {
        ...
        case ACI_GAP_PAIRING_COMPLETE_VSEVT_CODE:
        {
          ...
          if (p_pairing_complete->Status == SMP_PAIRING_STATUS_SUCCESS)
          {
            ESL_AP_ESLConnected(bleAppContext.connectionHandle, bleAppContext.deviceServerBdAddrType, bleAppContext.a_deviceServerBdAddr);
          }
          else if (p_pairing_complete->Status == SMP_PAIRING_STATUS_ENCRYPT_FAILED) //TODO: check if this is ok
          {
            esl_info_t esl_info;
            NVMDB_HandleType esl_db_h;
            NVMDB_status_t status;

            LOG_ERROR_APP("Pairing failed for key missing on peer device\n");
            ...
            if(status == 0)
            {
              ...
              LOG_ERROR_APP("Remove device from NVM list\n");
              NVMDB_DeleteRecord(&esl_db_h);
            }
            if(status ==0 || status == 1)
            {
              LOG_ERROR_APP("Remove bond\n");
              aci_gap_remove_bonded_device(bleAppContext.deviceServerBdAddrType, bleAppContext.a_deviceServerBdAddr); //TODO: check if address is not an identity address
            }
          }

          if (p_pairing_complete->Status != SMP_PAIRING_STATUS_SUCCESS)
          {
            aci_gap_terminate(bleAppContext.connectionHandle, HCI_REMOTE_USER_TERMINATED_CONNECTION_ERR_CODE);
          }
        ...
        case ACI_L2CAP_COC_CONNECT_CONFIRM_VSEVT_CODE:
        {
          ...
          aci_l2cap_coc_connect_confirm_event_rp0 *p_event;
          p_event = (aci_l2cap_coc_connect_confirm_event_rp0 *) p_blecore_evt->data;

          LOG_DEBUG_APP("MTU: %d\nMPS: %d\nResult: %d\nChannel number: %d\n",
                      p_event->MTU,
                      p_event->MPS,
                      p_event->Result,
                      p_event->Channel_Number);
        ...
        {
          LOG_DEBUG_APP(">>== ACI_L2CAP_COC_TX_POOL_AVAILABLE_VSEVT_CODE\n");
        ...
        case ACI_L2CAP_COMMAND_REJECT_VSEVT_CODE:
        {
          ...
          LOG_ERROR_APP("Reason: 0x%04X\n", p_event->Reason);
          if(p_event->Data_Length > 0)
          {
            LOG_ERROR_APP("Reason Data:");
            for(int i = 0;  i < p_event->Data_Length; i++)
            {
              LOG_ERROR_APP("%02X ", p_event->Data[i]);
            }
            LOG_ERROR_APP("\n");
          }
        }
      ...
      case ACI_L2CAP_COC_FLOW_CONTROL_VSEVT_CODE:
        {
          LOG_DEBUG_APP(">>== ACI_L2CAP_COC_FLOW_CONTROL_VSEVT_CODE\n");
          aci_l2cap_coc_flow_control_event_rp0 *p_event;
          p_event = (aci_l2cap_coc_flow_control_event_rp0 *) p_blecore_evt->data;

          OTP_CLIENT_L2CAPCredits(p_event->Credits);
        }
        break;
...
static void Ble_Hci_Gap_Gatt_Init(void)
{
  ...
  if (ret != BLE_STATUS_SUCCESS)
  {
    LOG_ERROR_APP("  Fail   : hci_le_write_suggested_default_data_length command, result: 0x%02X\n", ret);
  }

  uint8_t LE_Event_Mask[8] = {0xFF,0xFF,0xFF,0xFF, 0xFF, 0xFF,0xFF,0xFF};
  ret= hci_le_set_event_mask(LE_Event_Mask);

  if (ret != BLE_STATUS_SUCCESS)
  {
    LOG_ERROR_APP("  Fail   : hci_le_set_event_mask command, result: 0x%02X\n", ret);
  }
  else
  {
    LOG_DEBUG_APP("  Success: hci_le_set_event_mask command\n");
  }
...
{
  tBleStatus status;
  Adv_Set_t advertising_set_parameters;
  uint8_t peer_address[6] = {0};

  status = aci_gap_adv_set_configuration_v2(0, /* Adv_Mode */
                                            0, /* Advertising_Handle */
                                            0, /* Adv_Event_Properties */
                                            1600, /* Primary_Adv_Interval_Min */
                                            1600, /* Primary_Adv_Interval_Max */
                                            ADV_CH_37|ADV_CH_38|ADV_CH_39, /* Primary_Adv_Channel_Map */
                                            CFG_BD_ADDRESS_TYPE, /* Own_Address_Type */
                                            GAP_PUBLIC_ADDR, /* Peer_Address_Type */
                                            peer_address, /* Peer_Address */
                                            HCI_ADV_FILTER_NO, /* Adv_Filter_Policy */
                                            0, /* Adv_TX_Power */
                                            HCI_TX_PHY_LE_1M, /* Primary_Adv_PHY */
                                            0, /* Secondary_Adv_Max_Skip */
                                            HCI_TX_PHY_LE_1M, /* Secondary_Adv_PHY */
                                            0, /* Adv_SID */
                                            0, /* NScan_Req_Notification_Enable */
                                            0, /* Primary_Adv_PHY_Options */
                                            0 /*Secondary_Adv_PHY_Options*/);
  if (status != BLE_STATUS_SUCCESS)
  {
    LOG_ERROR_APP("==>> aci_gap_adv_set_configuration_v2 - fail, result: 0x%02X\n", status);
  }
  else
  {
    LOG_DEBUG_APP("==>> Success: aci_gap_adv_set_configuration_v2\n");
  }

  status = aci_gap_adv_set_periodic_parameters(0, /* Advertising_Handle */
                                               (PAWR_INTERVAL_MS*100)/125, /* Periodic_Adv_Interval_Min */
                                               (PAWR_INTERVAL_MS*100)/125, /* Periodic_Adv_Interval_Max */
                                               0, /* Periodic_Adv_Properties */
                                               PAWR_NUM_SUBEVENTS, /* Num_Subevents */
                                               (PAWR_SUBEVENT_INTERVAL_MS*100)/125, /* Subevent_Interval */
                                               (PAWR_RESPONSE_SLOT_DELAY_MS*100)/125, /* Response_Slot_Delay */
                                               PAWR_RESPONSE_SLOT_SPACING_US/125, /* Response_Slot_Spacing */
                                               PAWR_NUM_RESPONSE_SLOTS /* Num_Response_Slots */);
  if (status != BLE_STATUS_SUCCESS)
  {
    LOG_ERROR_APP("==>> aci_gap_adv_set_periodic_parameters - fail, result: 0x%02X\n", status);
  }
  else
  {
    LOG_DEBUG_APP("==>> Success: aci_gap_adv_set_periodic_parameters\n");
  }

  status = aci_gap_adv_set_periodic_enable(1, 0);
  if (status != BLE_STATUS_SUCCESS)
  {
    LOG_ERROR_APP("==>> aci_gap_adv_set_periodic_enable - fail, result: 0x%02X\n", status);
  }
  else
  {
    LOG_DEBUG_APP("==>> Success: aci_gap_adv_set_periodic_enable\n");
  }
  ...
  status = aci_gap_adv_set_enable(1, 1, &advertising_set_parameters);
  if (status != BLE_STATUS_SUCCESS)
  {
    LOG_ERROR_APP("==>> hci_le_set_extended_advertising_enable - fail, result: 0x%02X\n", status);
  }
  else
  {
    LOG_DEBUG_APP("==>> Success: hci_le_set_extended_advertising_enable\n");
  }

  HAL_Delay(1000);

  status = aci_gap_adv_set_enable(0, 1, &advertising_set_parameters);

  if (status != BLE_STATUS_SUCCESS)
  {
    LOG_ERROR_APP("==>> Stop ext adv - fail, result: 0x%02X\n", status);
  }
}
...
static void start_general_connection(void)
{
  ...
  status = aci_gap_start_general_connection_establish_proc(HCI_SCAN_TYPE_PASSIVE, /* LE_Scan_Type */
                                                           SCAN_INT_MS(500), /* LE_Scan_Interval */
                                                           SCAN_WIN_MS(500), /* LE_Scan_Window */
                                                           CFG_BD_ADDRESS_TYPE, /* Own_Address_Type */
                                                           HCI_SCAN_FILTER_NO, /* Scanning_Filter_Policy */
                                                           1 /* Filter_Duplicates */);
  if (status != BLE_STATUS_SUCCESS)
  {
    LOG_ERROR_APP("aci_gap_start_general_connection_establish_proc - fail, result: 0x%02X\n", status);
    bleAppContext.Device_Connection_Status = APP_BLE_IDLE;
    Error_Handler();
  }
  else
  {
    LOG_DEBUG_APP("==>> aci_gap_start_general_connection_establish_proc - Success\n");
...
void periodic_sync_info_transfer(void)
{
  ...
  status = hci_le_periodic_advertising_set_info_transfer(bleAppContext.connectionHandle, 0, 0);
  if (status != BLE_STATUS_SUCCESS)
  {
    LOG_ERROR_APP("hci_le_periodic_advertising_set_info_transfer - fail, result: 0x%02X, conn handle: 0x%04X\n", status, bleAppContext.connectionHandle);
    return;
  }
  else
  {
    LOG_DEBUG_APP("==>> hci_le_periodic_advertising_set_info_transfer - Success\n");
  }
}
...
void connection_setup(void)
{
   ...
   UTIL_SEQ_ClrEvt(1 << CFG_EVENT_READ_REMOTE_FEATURES_COMPLETE);
   ret = hci_le_read_remote_features_page_0(bleAppContext.connectionHandle);
   LOG_DEBUG_APP("hci_le_read_remote_features - result: 0x%02X\n", ret);
   if(ret != BLE_STATUS_SUCCESS)
   {
   ...
   UTIL_SEQ_WaitEvt(1 << CFG_EVENT_READ_REMOTE_FEATURES_COMPLETE);
   //TODO: check if remote device has wanted feature

   ret = aci_gap_send_pairing_req(bleAppContext.connectionHandle, 0);
   if (ret != BLE_STATUS_SUCCESS)
   {
     LOG_ERROR_APP("Pairing request cmd failure: 0x%02X\n", ret);
   }
   else
   {
     LOG_DEBUG_APP("Pairing request cmd success\n");
...
void create_periodic_advertising_connection(uint8_t subevent, uint8_t Peer_Address_Type, uint8_t Peer_Address[6])
{
  ...
  init_param_phy.Scan_Interval = SCAN_INT_MS(SCAN_INTERVAL_MS); /* Ignored */
  init_param_phy.Scan_Window = SCAN_WIN_MS(SCAN_WINDOW_MS); /* Ignored */
  init_param_phy.Conn_Interval_Min = CONN_INT_MS(CONNECTION_INTERVAL_MS);
  init_param_phy.Conn_Interval_Max = CONN_INT_MS(CONNECTION_INTERVAL_MS);
  init_param_phy.Conn_Latency = 0;
  init_param_phy.Supervision_Timeout = CONN_SUP_TIMEOUT_MS(SUPERVISION_TIMEOUT_MS);
  init_param_phy.Min_CE_Length = 2;
  init_param_phy.Max_CE_Length = 2;

  LOG_DEBUG_APP("!!!!! create_periodic_advertising_connection \n Peer_Address @:%02x:%02x:%02x:%02x:%02x:%02x\n",
              Peer_Address[5],
              Peer_Address[4],
              Peer_Address[3],
              Peer_Address[2],
              Peer_Address[1],
              Peer_Address[0]);
  LOG_DEBUG_APP("      Connection Interval:   %d ms\n  - Supervision Timeout: %d ms\n",
              CONNECTION_INTERVAL_MS,
              SUPERVISION_TIMEOUT_MS);

  ble_status = aci_gap_ext_create_connection(0, /* Initiating_Mode */
                                             GAP_DIRECT_CONNECTION_ESTABLISHMENT_PROC, /* Procedure */
                                             CFG_BD_ADDRESS_TYPE, /* Own_Address_Type */
                                             Peer_Address_Type, /* Peer_Address_Type */
                                             Peer_Address, /* Peer_Address */
                                             0, /* Advertising_Handle */
                                             subevent, /* Subevent */
                                             HCI_INIT_FILTER_NO, /* Initiator_Filter_Policy, ignored */
                                             HCI_INIT_PHYS_SCAN_CONN_LE_1M,
                                             &init_param_phy);
  if (ble_status != BLE_STATUS_SUCCESS)
  {
    LOG_ERROR_APP("aci_gap_ext_create_connection - fail, result: 0x%02X\n", ble_status);
    return;
  }
  else
  {
    LOG_DEBUG_APP("==>> aci_gap_ext_create_connection - Success\n");
  }
  bleAppContext.Device_Connection_Status = APP_BLE_LP_CONNECTING;
}
    ...
    while(i < adv_data_size)
    {
      ...
      case AD_TYPE_16_BIT_SERV_UUID:
        for(int j=i+2; j<i+ad_length; j++)
        {
          ...
          {
            LOG_INFO_APP("+SCAN: %d,%02X%02X%02X%02X%02X%02X\n",
                        address_type,
                        p_address[5],
                        p_address[4],
                        p_address[3],
...
static void Connect_Request(void)
{
  ...
  result = aci_gap_create_connection(SCAN_INT_MS(500), /* LE_Scan_Interval */
                                     SCAN_WIN_MS(500), /* LE_Scan_Window */
                                     bleAppContext.deviceServerBdAddrType,
                                     &bleAppContext.a_deviceServerBdAddr[0],
                                     CFG_BD_ADDRESS_TYPE, /* Own_Address_Type */
                                     CONN_INT_MS(CONNECTION_INTERVAL_MS),
                                     CONN_INT_MS(CONNECTION_INTERVAL_MS),
                                     0, /* Conn_Latency */
                                     CONN_SUP_TIMEOUT_MS(SUPERVISION_TIMEOUT_MS),
                                     2,
                                     2);
  if (result == BLE_STATUS_SUCCESS)
  {
    bleAppContext.Device_Connection_Status = APP_BLE_LP_CONNECTING;
    LOG_DEBUG_APP("==>> GAP Create connection Success\n");
  }
  else
  {
    LOG_ERROR_APP("==>> GAP Create connection Failed, result: 0x%02x\n", result);
  }
  return;
}
...
uint8_t  Scan_proc(void)
{
  ...
  if (bleAppContext.Device_Connection_Status != APP_BLE_CONNECTED_CLIENT)
  {
    ...
    status = aci_gap_start_general_discovery_proc(paramA,
                                                  paramB,
                                                  CFG_BD_ADDRESS_DEVICE, /* Own_Address_Type */
                                                  1 /* Filter_Duplicates */);
    if (status != BLE_STATUS_SUCCESS)
    {
      LOG_ERROR_APP("aci_gap_start_general_discovery_proc - fail, result: 0x%02X\n", status);
      return status;
    }
    else
    {
      bleAppContext.Device_Connection_Status = (APP_BLE_ConnStatus_t)paramC;
      LOG_DEBUG_APP("==>> aci_gap_start_general_discovery_proc - Success\n");
    }
  }
  else
  {
