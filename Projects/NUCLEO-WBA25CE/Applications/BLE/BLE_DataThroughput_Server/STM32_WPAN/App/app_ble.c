...
static char a_GapDeviceName[] = {  'D', 'a', 't', 'a', ' ', 'T', 'r', 'a', 'n', 's', 'f', 'e', 'r', ' ', 'S', 'e', 'r', 'v', 'e', 'r' }; /* Gap Device Name */
    ...
    case HCI_LE_META_EVT_CODE:
    {
      ...
      switch (p_meta_evt->subevent)
      {
        ...
        case HCI_LE_ENHANCED_CONNECTION_COMPLETE_SUBEVT_CODE:
        {
          ...
          DT_SERVHandleNotification.EvtOpcode = DT_SERV_CONN_HANDLE_EVT;
          DT_SERVHandleNotification.ConnectionHandle = bleAppContext.connectionHandle;
          DT_SERV_APP_EvtRx(&DT_SERVHandleNotification);
          /**
    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
    {
      ...
      switch (p_blecore_evt->ecode)
      {
        ...
        case ACI_GAP_PAIRING_COMPLETE_VSEVT_CODE:
        {
          ...
          UTIL_SEQ_SetEvt(1 << CFG_EVENT_PROC_GAP_COMPLETE);
...
static void LinkConfiguration(void)
{
  ...
  if (status != BLE_STATUS_SUCCESS)
  {
    LOG_INFO_APP("  Fail   : set data length command   : error code: 0x%02X\n", status);
  }
  else
  {
...
void BLE_SVC_L2CAP_Conn_Update(void)
{
  ...
  do {
  ret = aci_l2cap_connection_parameter_update_req(bleAppContext.connectionHandle,
                                                     paramA,
                                                     paramB,
                                                     paramC,
                                                     paramD);
  if (ret != BLE_STATUS_SUCCESS)
  {
    LOG_DEBUG_APP("  Fail   : BLE_SVC_L2CAP_Conn_Update() 0x%02X\n", ret);
  }
  else
  {
    LOG_INFO_APP("  Success: BLE_SVC_L2CAP_Conn_Update()\n");
  }
  }
...
static void Adv_Request(APP_BLE_ConnStatus_t NewStatus)
{
  ...
  if (ret != BLE_STATUS_SUCCESS)
  {
    LOG_INFO_APP("==>> delete tx power level - fail\n");
  }
  ...
  if (ret != BLE_STATUS_SUCCESS)
  {
      LOG_INFO_APP("==>> Start Fast Advertising Failed , result: 0x%02X\n", ret);
  }
  else
  {
      LOG_INFO_APP("==>> Success: Start Fast Advertising\n");
  }
  BleStackCB_Process();
  return;
}
