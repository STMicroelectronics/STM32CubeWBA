...
/* Extended CONNECTABLE with p2pClient_Ext device */
  ...
  adv_set_param[i].address_type = 1;//GAP_STATIC_RANDOM_ADDR;
  adv_set_param[i].peer_address_type = GAP_PUBLIC_ADDR;
  adv_set_param[i].p_peer_address = &a_peeraddr[0];
  adv_set_param[i].adv_set.Advertising_Handle = i;
  adv_set_param[i].adv_set.Duration = 0;
...
uint8_t ADV_EXT_Start(void)
{
  ...
  for (loop = 0 ; loop < ADV_SET_COUNT ; loop++)
  {
    if (adv_set_param[loop].enable != 0)
    {
      ...
      status = aci_gap_adv_set_configuration_v2(0x00,
                                                ...
                                                0x01, // primary advertising PHY = 1M
                                                0x00,
                                                0x01, // secondary advertising is 1M
                                                adv_set_param[loop].sid,
                                                0x00, //scan req notification disabled
                                                0, // primary adv PHY options
                                                0); // secondary adv PHY options
      ...
      if (adv_set_param[loop].address_type == GAP_STATIC_RANDOM_ADDR)
      {
        ...
        status = aci_gap_adv_set_random_address(adv_set_param[loop].adv_set.Advertising_Handle,
                                                           &a_bd_addr[0]);
        if (status == BLE_STATUS_SUCCESS)
        {
