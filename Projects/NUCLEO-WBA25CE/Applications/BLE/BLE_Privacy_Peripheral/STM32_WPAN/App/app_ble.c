...
PRIVACY_SERVER_APP_ConnHandleNotEvt_t PRIVACY_SERVERHandleNotification;
...
{
  10, AD_TYPE_COMPLETE_LOCAL_NAME, 'P', 'E', 'R', 'I', 'P', 'H', '_', 'X', 'X',  /* Complete name */
...
/* USER CODE BEGIN EV */
void configure_filter_and_resolving_list();
void Terminate_Connection_After_Bonding();
void advertise_filter_accept_list();
/* USER CODE END EV */
...
void APP_BLE_Init(void)
{
  ...
  UTIL_SEQ_RegTask(1U << CFG_TASK_CONFIGURE_FILTER_RESOLVING_LIST, UTIL_SEQ_RFU, configure_filter_and_resolving_list);
  UTIL_SEQ_RegTask(1U << CFG_TASK_ADV_FILTER_LIST, UTIL_SEQ_RFU, advertise_filter_accept_list);
...
SVCCTL_UserEvtFlowStatus_t SVCCTL_App_Notification(void *p_Pckt)
{
  ...
  switch (p_event_pckt->evt)
  {
    case HCI_DISCONNECTION_COMPLETE_EVT_CODE:
    {
      ...
      PRIVACY_SERVERHandleNotification.EvtOpcode = PRIVACY_SERVER_DISCON_HANDLE_EVT;
      PRIVACY_SERVERHandleNotification.ConnectionHandle = p_disconnection_complete_event->Connection_Handle;
      PRIVACY_SERVER_APP_EvtRx(&PRIVACY_SERVERHandleNotification);
    ...
    case HCI_LE_META_EVT_CODE:
    {
      ...
      switch (p_meta_evt->subevent)
      {
        ...
        case HCI_LE_ENHANCED_CONNECTION_COMPLETE_SUBEVT_CODE:
        {
          ...
          PRIVACY_SERVERHandleNotification.EvtOpcode = PRIVACY_SERVER_CONN_HANDLE_EVT;
          PRIVACY_SERVERHandleNotification.ConnectionHandle = p_enhanced_conn_complete->Connection_Handle;
          PRIVACY_SERVER_APP_EvtRx(&PRIVACY_SERVERHandleNotification);
        ...
        case HCI_LE_CONNECTION_COMPLETE_SUBEVT_CODE:
        {
          ...
          PRIVACY_SERVERHandleNotification.EvtOpcode = PRIVACY_SERVER_CONN_HANDLE_EVT;
          PRIVACY_SERVERHandleNotification.ConnectionHandle = p_conn_complete->Connection_Handle;
          PRIVACY_SERVER_APP_EvtRx(&PRIVACY_SERVERHandleNotification);
...
static void Ble_Hci_Gap_Gatt_Init(void)
{
  ...
  uint8_t scan_resp_data[28] = {0x09,0x09,0x72,0x63,0x6D,0x2D,0x74,0x65,0x73,0x74,0x11,0x07,0xE6,0x9D,0x48,0x19,0xF0,0xFE,0xAD,0x92,0x64,0x45,0x04,0xAF,0xC0,0xA3,0x0B,0xE5};
  /* Add number of record for Device Info Characteristic */
  static const uint8_t p_additional_svc_record[1] = {0x03};
  uint8_t p_sc_key_type = 0x01;
  ...
  ret = aci_hal_write_config_data(CONFIG_DATA_SC_KEY_TYPE_OFFSET,
                                  CONFIG_DATA_SC_KEY_TYPE_LEN,
                                  (uint8_t*) &p_sc_key_type);
  if (ret != BLE_STATUS_SUCCESS)
  {
    LOG_INFO_APP("  Fail   : aci_hal_write_config_data command - CONFIG_DATA_SC_KEY_TYPE_OFFSET, result: 0x%02X\n", ret);
  }
  else
  {
    LOG_INFO_APP("  Success: aci_hal_write_config_data command - CONFIG_DATA_SC_KEY_TYPE_OFFSET\n");
  }
  ...
  /* fill scan_resp payload */
  hci_le_set_scan_response_data(28,scan_resp_data);
  ...
  */
  ...
  a_GATT_DevInfoData[4] = BOARD_ID_NUCLEO_WBA2X;
  ...
  a_GATT_DevInfoData[11] = FW_ID_PRIVACY;
    ...
    case AD_TYPE_MANUFACTURER_SPECIFIC_DATA:
      {
        ...
        p_adv_data[i+5] = BOARD_ID_NUCLEO_WBA2X; /* Board ID */
        p_adv_data[i+6] = FW_ID_PRIVACY; /* FW ID */
...
void configure_filter_and_resolving_list()
{
  ...
  Bonded_Device_Entry_t Bonded_Devices_Entry[20];
  uint8_t nb_bonded_devices;
  List_Entry_t List_Entry[20];
  uint8_t i;

  /* Get the bonded device identity address and type */
  ret = aci_gap_get_bonded_devices(&nb_bonded_devices, Bonded_Devices_Entry);
  if (nb_bonded_devices > 0)
  {
    for (i=0; i<nb_bonded_devices;i++)
    {
      List_Entry[i].Address_Type = Bonded_Devices_Entry[i].Address_Type;
      memcpy(List_Entry[i].Address, Bonded_Devices_Entry[i].Address, 6);
    }
    
    /* Append to both resolving list and Filter Accept List */
    ret = aci_gap_add_devices_to_list(nb_bonded_devices,List_Entry, 0x04);
    if (ret != BLE_STATUS_SUCCESS)
    {
      APP_DBG_MSG("  Fail   : aci_gap_add_devices_to_list command, result: 0x%02X\n", ret);
    }
    else
    {
      APP_DBG_MSG("  Success: aci_gap_add_devices_to_list command\n");
    }
...
void Terminate_Connection_After_Bonding()
{
  ...
  {
    ret = aci_gap_terminate(bleAppContext.connectionHandle, 0x13);
    if (ret != BLE_STATUS_SUCCESS)
    {
  ...
  UTIL_SEQ_SetTask( 1U << CFG_TASK_CONFIGURE_FILTER_RESOLVING_LIST, CFG_SEQ_PRIO_0); 
...
void advertise_filter_accept_list()
{
  tBleStatus ret;
  /* Start Advertising */
  ret = aci_gap_set_undirected_connectable(CONN_INT_MS(50),
                                           CONN_INT_MS(100),
                                           CFG_BD_ADDRESS_TYPE,
  ...
  if (ret != BLE_STATUS_SUCCESS)
  {
    LOG_INFO_APP("==>> aci_gap_set_undirected_connectable - fail, result: 0x%02X\n", ret);
  }
  else
  {
    LOG_INFO_APP("==>> aci_gap_set_undirected_connectable - Success\n");
  }
  ...
  /* USER CODE END ADV_DATA_UPDATE_1 */
  ret = aci_gap_update_adv_data(adv_data_len, adv_data_p);
  if (ret != BLE_STATUS_SUCCESS)
  {
    LOG_INFO_APP("==>> Start Advertising Failed, result: 0x%02X\n", ret);
  }
...
void APP_BSP_Button3Action(void)
{
  ...
  UTIL_SEQ_SetTask( 1U << CFG_TASK_ADV_FILTER_LIST, CFG_SEQ_PRIO_0);
}
#endif
