...
static char a_GapDeviceName[] = {  'S', 'e', 'r', 'i', 'a', 'l', ' ', 'C', 'o', 'm', ' ', 'P', 'e', 'r', 'i', 'p', 'h', 'e', 'r', 'a', 'l' }; /* Gap Device Name */
...
void APP_BLE_Init(void)
{
  ...
  UTIL_SEQ_RegTask(1U << CFG_TASK_CONN_UPDATE_REG_ID, UTIL_SEQ_RFU, Connection_Interval_Update_Req);
  UTIL_SEQ_RegTask(1U << CFG_TASK_PERIPH_SECURITY_REQ_ID, UTIL_SEQ_RFU, Periph_security_request); 
    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
    {
      ...
      switch (p_blecore_evt->ecode)
      {
        ...
        case (ACI_L2CAP_COC_CONNECT_VSEVT_CODE):
        {
          ...
          BleCoCContextPeriph.Max_Payload_Size = CFG_BLE_COC_MPS_MAX;
          ...
          ret = aci_l2cap_coc_connect_confirm(BleCoCContextPeriph.Conn_Handle,
                                              BleCoCContextPeriph.Max_Transmission_Unit,
                                              BleCoCContextPeriph.Max_Payload_Size,
                                              BleCoCContextPeriph.Initial_Credits,
                                              0x0000,
                                              4,
                                              &Channel_Number,
                                              &Channel_Index_List);
...
void BLE_SVC_L2CAP_Conn_Update(uint16_t ConnectionHandle, uint16_t conn1, uint16_t conn2)
{
  ...
  if (ret != BLE_STATUS_SUCCESS)
  {
    LOG_INFO_APP("BLE_SVC_L2CAP_Conn_Update(), Failed: 0x%02X\n", ret);
  }
  else
  {
...
static void Connection_Interval_Update_Req(void)
{
  ...
  {
    BLE_SVC_L2CAP_Conn_Update(bleAppContext.connectionHandle,
...
{
  tBleStatus status;
  status = aci_gap_slave_security_req(bleAppContext.connectionHandle);
  if (status != BLE_STATUS_SUCCESS)
  {
    LOG_INFO_APP("Periph security request cmd failure: 0x%02X\n", status);
  }
  else
  {
    LOG_INFO_APP("Periph security request cmd success\n");
  }
...
void APP_BSP_Button2Action(void)
{
  ...
  {
    LOG_INFO_APP("**PERIPH SECURITY REQ \n");
    UTIL_SEQ_SetTask(1U << CFG_TASK_PERIPH_SECURITY_REQ_ID, CFG_SEQ_PRIO_0);
  }
  return;
}
