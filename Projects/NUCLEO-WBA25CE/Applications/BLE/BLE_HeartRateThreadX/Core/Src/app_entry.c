...
uint32_t MX_APPE_Init(void *p_param)
{
  ...
  /* Create PKA mutex */
  TXstatus = tx_mutex_create (&PkaMutex,
                             "PKA Mutex",
                             0);

  if( TXstatus != TX_SUCCESS )
  {
    LOG_ERROR_APP( "PKA Mutex ThreadX objects creation FAILED, status: %d", TXstatus);
    Error_Handler();
  }
...
int PKACTRL_MutexTake(void)
{
  ...
  /* Take the mutex */
  TXstatus = tx_mutex_get(&PkaMutex,
                          TX_WAIT_FOREVER);
...
int PKACTRL_MutexRelease(void)
{
  ...
  /* Release the mutex */
  TXstatus = tx_mutex_put(&PkaMutex);
