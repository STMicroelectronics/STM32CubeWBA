...
/* USER CODE BEGIN PD */
#define CFG_DEV_ID_SECURITY                  (0xA0)
...
void APP_BLE_Init(void)
{
  ...
  UTIL_SEQ_RegTask(1U << CFG_TASK_DISCONN_REQ_ID, UTIL_SEQ_RFU, Disconnection_request);  
      ...
      switch (ad_type)
      {
        ...
        case AD_TYPE_MANUFACTURER_SPECIFIC_DATA:          
          if ((ad_length >= 7) && (p_adv_data[i + 4] == 0x02))
          {
            ...
            if(p_adv_data[i + 6] == CFG_DEV_ID_SECURITY)
            {
              ...
              APP_DBG_MSG("-- ID_SECURITY peripheral detected, db addr 0x%02X:%02X:%02X:%02X:%02X:%02X\n",
            ...
            {
              APP_DBG_MSG("-- ID_SECURITY peripheral not detected\n");
            }
          }
          else if ((ad_length >= 7) && (p_adv_data[i + 2] == 0x01))
          {
            APP_DBG_MSG("--- ST MANUFACTURER data BlueSTv1\n");
            if(p_adv_data[i + 3] == CFG_DEV_ID_SECURITY)
            {
              bleAppContext.a_deviceServerBdAddr[0] = p_adv_report->Advertising_Report[0].Address[0];
              bleAppContext.a_deviceServerBdAddr[1] = p_adv_report->Advertising_Report[0].Address[1];
              bleAppContext.a_deviceServerBdAddr[2] = p_adv_report->Advertising_Report[0].Address[2];
              ...
              APP_DBG_MSG("-- ID_BLE_TEST peripheral, db addr 0x%02X:%02X:%02X:%02X:%02X:%02X\n",
                              bleAppContext.a_deviceServerBdAddr[5],
                              bleAppContext.a_deviceServerBdAddr[4],
                              bleAppContext.a_deviceServerBdAddr[3],
                              bleAppContext.a_deviceServerBdAddr[2],
            ...
            {
              APP_DBG_MSG("-- ID_BLE_TEST peripheral not detected\n");
            }
...
{
  tBleStatus status;
  status = aci_gap_send_pairing_req(bleAppContext.connectionHandle, 0x01);
  if (status != BLE_STATUS_SUCCESS)
  {
...
void APP_BSP_Button3Action(void)
{
  if (bleAppContext.Device_Connection_Status == APP_BLE_CONNECTED_CLIENT)
  {
    APP_DBG_MSG("**DISCONNECTION REQ \n");
    UTIL_SEQ_SetTask(1U << CFG_TASK_DISCONN_REQ_ID, CFG_SEQ_PRIO_0);
  }
  return;
}
#endif
