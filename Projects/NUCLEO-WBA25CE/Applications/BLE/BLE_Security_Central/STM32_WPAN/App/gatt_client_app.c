...
typedef struct
{
  ...
  uint8_t SecurityNotificationCharProperties;
...
void GATT_CLIENT_APP_Init(void)
{
  ...
  for(index = 0; index < CFG_BLE_NUM_CLIENT_CONTEXTS; index++)
  {
    ...
    a_ClientContext[index].svcInfo[SVC_ID_SECURITY].uuid  = ST_SECURITY_SERVICE_UUID;
...
/* USER CODE BEGIN gatt_parse_chars_1 */
        else if (uuid == ST_SECURITY_NOTIFY_CHAR_UUID)
        {
          ...
          LOG_INFO_APP(", ST_SECURITY_NOTIFY_CHAR_UUID charac found\n");
        }
        else if (uuid == ST_SECURITY_WRITE_CHAR_UUID)
        {
          ...
          LOG_INFO_APP(", ST_SECURITY_WRITE_CHAR_UUID charac found\n");
        }
