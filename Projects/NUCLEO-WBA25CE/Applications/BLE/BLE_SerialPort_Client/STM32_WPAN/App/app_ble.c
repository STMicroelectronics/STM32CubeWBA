...
static char a_GapDeviceName[] = {  'S', 'e', 'r', 'i', 'a', 'l', ' ', 'C', 'o', 'm', ' ', 'C', 'e', 'n', 't', 'r', 'a', 'l' }; /* Gap Device Name */
...
/* USER CODE BEGIN PFP */
static uint8_t analyse_adv_report(uint8_t adv_data_size, uint8_t const*p_adv_data,
    ...
    case HCI_LE_META_EVT_CODE:
    {
      ...
      switch (p_meta_evt->subevent)
      {
        case HCI_LE_CONNECTION_UPDATE_COMPLETE_SUBEVT_CODE:
        {
          ...
          LOG_INFO_APP("The BLE_SerialPort application is now ready to send messages.\n");
          LOG_INFO_APP("You may proceed with entering and sending your messages.\n");
          LOG_INFO_APP("\n");
        ...
        case HCI_LE_ADVERTISING_REPORT_SUBEVT_CODE:
        {
          ...
          found_status = analyse_adv_report(p_adv_report->Advertising_Report[0].Length_Data,
                                            p_adv_report->Advertising_Report[0].Data,
                                            p_adv_report->Advertising_Report[0].Address_Type,
                                            p_adv_report->Advertising_Report[0].Address);
          if (found_status != 0)
          {
        ...
        case HCI_LE_EXTENDED_ADVERTISING_REPORT_SUBEVT_CODE:
        {
          ...
          found_status = analyse_adv_report(p_ext_adv_report->Data_Length, 
                                            p_ext_adv_report->Data,
                                            p_ext_adv_report->Address_Type, 
                                            p_ext_adv_report->Address);
          if (found_status != 0)
          {
            APP_BLE_Procedure_Gap_Central(PROC_GAP_CENTRAL_SCAN_TERMINATE);
          }
    ...
    case AD_TYPE_MANUFACTURER_SPECIFIC_DATA:
      {
            ...
            (p_adv_data[i + 6] == FW_ID_SERIALPORT_SERVER))
        {
          ...
          LOG_INFO_APP("  Serial Port Server, db addr 0x%02X:%02X:%02X:%02X:%02X:%02X\n",
...
{
    tBleStatus result;
    GATT_CLIENT_APP_ConnHandle_Notif_evt_t gatt_client_app;
    
    if (bleAppContext.deviceServerFound != 0)
    {
        LOG_INFO_APP("Create connection to Serial Port Server (new API)\n");

        result = aci_gap_create_connection(
            0x0060, // LE_Scan_Interval (60 ms)
            0x0030, // LE_Scan_Window (30 ms)
            bleAppContext.deviceServerBdAddrType,
            &bleAppContext.a_deviceServerBdAddr[0],
            GAP_STATIC_RANDOM_ADDR, // Own_Address_Type: Random address
            CONN_INT_MS(20u), // Conn_Interval_Min
            CONN_INT_MS(100u), // Conn_Interval_Max
            0u, // Conn_Latency
            CONN_SUP_TIMEOUT_MS(1000u), // Supervision_Timeout
            CONN_CE_LENGTH_MS(20u), // Minimum_CE_Length
            CONN_CE_LENGTH_MS(20u)  // Maximum_CE_Length
        );
        ...
        if (result == BLE_STATUS_SUCCESS)
        {
            ...
            if(new_ConnHdl != 0u)
            {
                gatt_client_app.ConnHdl = new_ConnHdl;
                gatt_client_app.ConnOpcode = PEER_CONN_HANDLE_EVT;
                GATT_CLIENT_APP_Notification(&gatt_client_app);
                new_ConnHdl = 0u;
            }
        }
        else
...
static void Scan_Request(void)
{
    ...
    Scan_Param_Phy_t scan_param_phy = {.Scan_Type = HCI_SCAN_TYPE_PASSIVE,   // 0x00: Passive, 0x01: Active
                                       .Scan_Interval = SCAN_INT_MS(500),    // Macro come prima, oppure valore diretto
                                       .Scan_Window = SCAN_WIN_MS(500)
                                       };
    ...
    if (bleAppContext.Device_Connection_Status != APP_BLE_CONNECTED_CLIENT)
    {
        ...
        result = aci_gap_ext_start_scan(
            0x00,                        // Scan_Mode: Reserved, set to 0
            0x02,                        // Procedure: GAP_GENERAL_DISCOVERY_PROC
            GAP_STATIC_RANDOM_ADDR,      // Own_Address_Type: Random address
            0x01,                        // Filter_Duplicates
            0x0000,                      // Duration: 0 = scan until stopped
            0x0000,                      // Period: 0 = scan until stopped
            0x00,                        // Scanning_Filter_Policy: Accept all
            0x01,                        // Scanning_PHYs: 1M PHY
            &scan_param_phy              // Scan_Param_Phy: parametri per ogni PHY
        );

        if (result != BLE_STATUS_SUCCESS)
        {
            LOG_INFO_APP("aci_gap_ext_start_scan - fail, result: 0x%02X\n", result);
        }
        else
        {
            LOG_INFO_APP("==>> aci_gap_ext_start_scan - Success\n");
        }
    }
...
{
  tBleStatus status;
  status = aci_gap_send_pairing_req(bleAppContext.connectionHandle,
                                    0x00 // Force_Rebond: 0 = do not force, 1 = force pairing even if bonded
                                    );
  if (status != BLE_STATUS_SUCCESS)
  {
    LOG_INFO_APP("Pairing request cmd failure: 0x%02X\n", status);
  }
  else
  {
    LOG_INFO_APP("Pairing request cmd success\n");
  }
...
static void Central_conn_interv_update(void)
{
  ...
  if (status != BLE_STATUS_SUCCESS)
  {
    LOG_INFO_APP("aci_gap_start_connection_update() Failed, status 0x%02X\n",status);
  }
  else
  {
    LOG_INFO_APP("aci_gap_start_connection_update(), Successfully\n");
  }
...
void APP_BSP_Button3Action(void)
{
  ...
  {
    LOG_INFO_APP("**CONN INTERVAL TOGGLE\n");
