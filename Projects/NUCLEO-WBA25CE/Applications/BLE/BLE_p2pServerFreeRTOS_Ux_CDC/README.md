## __BLE_p2pServerFreeRTOS_Ux_CDC Application Description__
...
The BLE p2pServer application advertises and waits for a connection from either:
 ...
 - The BLE __BLE_p2pServerFreeRTOS_Ux_CDC application running on STM32WBAxx devices.
 - A remote interface.

Once connected, the BLE_p2pServer can receive messages from the Client and send notification to it.

This example uses the X-CUBEFREERTOS pack and USBX stack to display logs on the User USB using the CDC ACM USB Class.

### __Keywords__

Connectivity, BLE, BLE protocol, BLE pairing, BLE profile, USB, USBX, FreeRTOS.
...
To retrieve logs from the application:

 - Connect the Nucleo Board to your PC with a USB cable type A to type C to User USB connector (USB_USER)
...
- B3 :
  - Not connected : Perform a deinitialization of the BLE feature, followed by an initialization.
  - Connected     : Update connection interval.

To retrieve logs from the application, connect a serial terminal program to the COM port enumerated after the application has been loaded. Note that as the USB stack is initialized after the other peripherals, initialization logs might be missing.
