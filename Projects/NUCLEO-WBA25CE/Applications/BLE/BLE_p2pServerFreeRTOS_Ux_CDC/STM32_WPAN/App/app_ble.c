    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
    {
      ...
      switch (p_blecore_evt->ecode)
      {
        ...
        case ACI_HAL_END_OF_RADIO_ACTIVITY_VSEVT_CODE:
        {
          ...
          osTimerStart(switchOffGPIOTimerHandle, pdMS_TO_TICKS(LED_ON_TIMEOUT_MS));
...
void Print_Device_Info(void)
{
  ...
  LOG_INFO_APP("-- Public Bluetooth Address: %02x:%02x:%02x:%02x:%02x:%02x\n",a_BdAddr[5],a_BdAddr[4],a_BdAddr[3],a_BdAddr[2],a_BdAddr[1],a_BdAddr[0]);
}
/* USER CODE END FD */
...
void APP_BSP_Button1Action(void)
{
  ...
  {
    /* Send notification */
    osSemaphoreRelease(P2PAPPSemaphoreHandle);
  }
...
void APP_BSP_Button3Action(void)
{
  if (bleAppContext.Device_Connection_Status != APP_BLE_CONNECTED_SERVER)
  {
    /* Perform Deinit then Init */
    uint16_t init_cmd;
    /* Deinit */
    init_cmd = 1;
    osMessageQueuePut(BLEInitCmdQueueHandle, &init_cmd, 0U, 0U);
