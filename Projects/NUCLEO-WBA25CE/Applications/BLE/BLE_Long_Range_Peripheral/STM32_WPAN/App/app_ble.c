...
static char a_GapDeviceName[] = {  'P', 'e', 'e', 'r', ' ', 't', 'o', ' ', 'P', 'e', 'e', 'r', ' ', 'S', 'e', 'r', 'v', 'e', 'r', ' ', 'L', 'o', 'n', 'g', 'R', 'a', 'n', 'g', 'e' }; /* Gap Device Name */

/* Advertising Data */
uint8_t a_AdvData[28] =
{
  11, AD_TYPE_COMPLETE_LOCAL_NAME, 'p', '2', 'p', 'S', '_', 'L', 'R', '_', 'X', 'X',  /* Complete name */
      ...
      else
      {
        ...
        if ((phy_tx == HCI_TX_PHY_LE_CODED) && (phy_rx == HCI_TX_PHY_LE_CODED))
        {
          if (toggle_flag == 0)
          {
            ...
            status = hci_le_set_phy(bleAppContext.connectionHandle, 0, HCI_TX_PHYS_LE_CODED_PREF, HCI_TX_PHYS_LE_CODED_PREF, 1);
            if (status != BLE_STATUS_SUCCESS)
            {
              APP_DBG_MSG("Fail\n");
            }
          ...
          else
          {
            ...
            status = hci_le_set_phy(bleAppContext.connectionHandle, 0, HCI_TX_PHYS_LE_CODED_PREF, HCI_TX_PHYS_LE_CODED_PREF, 2);
            if (status != BLE_STATUS_SUCCESS)
            {
              APP_DBG_MSG("Fail\n");
            }
...
/* Add number of record for Device Info Characteristic */
  ...
  */
  ...
  a_GATT_DevInfoData[11] = FW_ID_P2P_SERVER_LONG_RANGE;
    ...
    case AD_TYPE_MANUFACTURER_SPECIFIC_DATA:
      {
        ...
        p_adv_data[i+6] = FW_ID_P2P_SERVER_LONG_RANGE; /* FW ID */
