...
/* Extended CONNECTABLE with p2pClient_Ext device */
  ...
  strcpy(adv_set_param[i].username, "LR_P2P");
  adv_set_param[i].sid = i;
  adv_set_param[i].interval_min = 200;
  adv_set_param[i].interval_max = 300;
  adv_set_param[i].tx_power = CFG_TX_POWER;
...
uint8_t ADV_EXT_Start(void)
{
  ...
  for (loop = 0 ; loop < ADV_SET_COUNT ; loop++)
  {
    if (adv_set_param[loop].enable != 0)
    {
      ...
      {
        LOG_INFO_APP("Advertising set count limit has been reached ! please update parameters app_conf.h file\n");
      }
      LOG_INFO_APP("  Adv name     : %s\n", (char*)adv_set_param[loop].username );
      LOG_INFO_APP("  SID          : %d\n", adv_set_param[loop].sid);
      LOG_INFO_APP("  Data length  : %d\n", adv_set_param[loop].data_len );
      
      // Initialize Primary advertising PHY to LE coded */                                                                                                                                         
      status = aci_gap_adv_set_configuration_v2(0x02,
                                              ...
                                              HCI_PRIMARY_ADV_PHY_LE_CODED, // primary advertising PHY = LE coded
                                              0x00,
                                              HCI_SECONDARY_ADV_PHY_LE_CODED, // secondary advertising is LE coded
                                              adv_set_param[loop].sid,
                                              0x00, //scan req notification disabled
                                              HCI_PRIMARY_ADV_PHY_OPTIONS, // primary adv PHY options
                                              HCI_SECONDARY_ADV_PHY_OPTIONS); // secondary adv PHY options
      if (status == BLE_STATUS_SUCCESS)
      {
      ...
      {
        uint8_t a_bd_addr[BD_ADDR_SIZE];
        
        a_bd_addr[5] = 0x00;
        a_bd_addr[4] = 0xC0;
        a_bd_addr[3] = 0x11;
        a_bd_addr[2] = 0x22;
        a_bd_addr[1] = (uint8_t)READ_REG(RTC->SSR);
        a_bd_addr[0] = (uint8_t)READ_REG(RTC->SSR);
        ...
        if (status == BLE_STATUS_SUCCESS)
        {
          LOG_INFO_APP("  Success: hci_le_set_advertising_set_random_address\n");
        }
        else
        {
          LOG_INFO_APP("  Fail   : hci_le_set_advertising_set_random_address, result: 0x%02X\n", status);
        }
      }
      else
      {
        LOG_INFO_APP("  Public Bluetooth address used\n");
      }
      ...
      if (adv_set_param[loop].data == NULL)
      {
        ...
        if (status == BLE_STATUS_SUCCESS)
        {
          LOG_INFO_APP("  Success: ADV_EXT_Build_data\n");
        }
        else
        {
          LOG_INFO_APP("  Fail   : ADV_EXT_Build_data, result: 0x%02X\n", status);
        }
      }
      ...
      if (status == BLE_STATUS_SUCCESS)
      {
        LOG_INFO_APP("  Success: aci_gap_adv_set_adv_data/aci_gap_adv_set_scan_resp_data\n");
      }
      else
      {
        LOG_INFO_APP("  Fail   : aci_gap_adv_set_adv_data/aci_gap_adv_set_scan_resp_data, result: 0x%02X\n", status);
      }
      
      status = hci_le_set_extended_advertising_enable(1, 1, &adv_set_param[loop].adv_set);
      if (status == BLE_STATUS_SUCCESS)
      {
