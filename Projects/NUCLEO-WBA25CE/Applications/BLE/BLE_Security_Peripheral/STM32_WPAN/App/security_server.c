...
/**
  ...
  * @brief   Security_Server definition.
  ******************************************************************************
  * @attention
  *
...
typedef struct{
  uint16_t  Security_serverSvcHdle;                  /**< Security_server Service Handle */
...
static SECURITY_SERVER_Context_t SECURITY_SERVER_Context;
...
static SVCCTL_EvtAckStatus_t SECURITY_SERVER_EventHandler(void *p_pckt);
 ...
 */
#define COPY_SECURITY_SERVER_UUID(uuid_struct)       COPY_UUID_128(uuid_struct,0x00,0x00,0xfe,0x50,0xcc,0x7a,0x48,0x2a,0x98,0x4a,0x7f,0x2e,0xd5,0xb3,0xe5,0x8f)
#define COPY_LED_C_UUID(uuid_struct)       COPY_UUID_128(uuid_struct,0x00,0x00,0xfe,0x51,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
#define COPY_SWITCH_C_UUID(uuid_struct)       COPY_UUID_128(uuid_struct,0x00,0x00,0xfe,0x52,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
 ...
 */
static SVCCTL_EvtAckStatus_t SECURITY_SERVER_EventHandler(void *p_Event)
{
  SVCCTL_EvtAckStatus_t return_value;
  hci_event_pckt *p_event_pckt;
  evt_blecore_aci *p_blecore_evt;
  aci_gatt_attribute_modified_event_rp0 *p_attribute_modified;
  SECURITY_SERVER_NotificationEvt_t                 notification;
    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
      ...
      switch(p_blecore_evt->ecode)
      {
        case ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE:
        {
          ...
          if(p_attribute_modified->Attr_Handle == (SECURITY_SERVER_Context.Switch_CCharHdle + CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))
          {
            ...
            switch(p_attribute_modified->Attr_Data[0])
            {
              ...
              case (0x00):
                ...
                notification.EvtOpcode = SECURITY_SERVER_SWITCH_C_NOTIFY_DISABLED_EVT;
                SECURITY_SERVER_Notification(&notification);
              ...
              case GATT_CHAR_UPDATE_SEND_NOTIFICATION:
                ...
                notification.EvtOpcode = SECURITY_SERVER_SWITCH_C_NOTIFY_ENABLED_EVT;
                SECURITY_SERVER_Notification(&notification);
          ...
          else if(p_attribute_modified->Attr_Handle == (SECURITY_SERVER_Context.Led_CCharHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
          {
            return_value = SVCCTL_EvtAckFlowEnable;

            notification.EvtOpcode = SECURITY_SERVER_LED_C_WRITE_NO_RESP_EVT;
  ...
  return(return_value);
}/* end SECURITY_SERVER_EventHandler */
 ...
 * @retval None
 */
void SECURITY_SERVER_Init(void)
{
  Char_UUID_t  uuid;
  tBleStatus ret;
   ...
   */
  SVCCTL_RegisterSvcHandler(SECURITY_SERVER_EventHandler);

  /**
   * Security_Server
   *
   * Max_Attribute_Records = 1 + 2*2 + 1*no_of_char_with_notify_or_indicate_property + 1*no_of_char_with_broadcast_property
   * service_max_attribute_record = 1 for Security_Server +
   *                                2 for LED_C +
   *                                2 for SWITCH_C +
  ...
  COPY_SECURITY_SERVER_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_service(UUID_TYPE_128,
                             (Service_UUID_t *) &uuid,
                             PRIMARY_SERVICE,
                             max_attr_record,
                             &(SECURITY_SERVER_Context.Security_serverSvcHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    LOG_INFO_BLE("  Fail   : aci_gatt_add_service command: Security_Server, error code: 0x%02X\n", ret);
  }
  else
  {
    LOG_INFO_BLE("  Success: aci_gatt_add_service command: Security_serverSvcHdle = 0x%04X\n",SECURITY_SERVER_Context.Security_serverSvcHdle);
  }
  ...
  ret = aci_gatt_add_char(SECURITY_SERVER_Context.Security_serverSvcHdle,
                          UUID_TYPE_128,
                          (Char_UUID_t *) &uuid,
                          SizeLed_C,
                          ...
                          &(SECURITY_SERVER_Context.Led_CCharHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
  ...
  ret = aci_gatt_add_char(SECURITY_SERVER_Context.Security_serverSvcHdle,
                          ...
                          0x10,
                          CHAR_VALUE_LEN_VARIABLE,
                          &(SECURITY_SERVER_Context.Switch_CCharHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
 ...
 *
 */
tBleStatus SECURITY_SERVER_UpdateValue(SECURITY_SERVER_CharOpcode_t CharOpcode, SECURITY_SERVER_Data_t *pData)
{
  ...
  switch(CharOpcode)
  {
    case SECURITY_SERVER_LED_C:
      ret = aci_gatt_update_char_value(SECURITY_SERVER_Context.Security_serverSvcHdle,
                                       SECURITY_SERVER_Context.Led_CCharHdle,
                                       0, /* charValOffset */
