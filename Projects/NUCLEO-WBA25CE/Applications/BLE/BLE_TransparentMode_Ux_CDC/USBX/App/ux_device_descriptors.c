...
uint8_t *USBD_Get_String_Framework(ULONG *Length)
{
  ...
  /* Read UDN to generate a unique serial number */
  if (OTP_Read(0, &p_otp_addr) == HAL_OK)
  {
    dev_id_str[0] = Hex_To_Char((p_otp_addr->bd_address[1] & 0xF0) >> 4);
    dev_id_str[1] = Hex_To_Char(p_otp_addr->bd_address[1] & 0x0F);
    dev_id_str[2] = Hex_To_Char((p_otp_addr->bd_address[0] & 0xF0) >> 4);
    dev_id_str[3] = Hex_To_Char(p_otp_addr->bd_address[0] & 0x0F);
    memcpy(USBD_string_framework + count + len - 3, (uint8_t *)dev_id_str, 4);
  }
