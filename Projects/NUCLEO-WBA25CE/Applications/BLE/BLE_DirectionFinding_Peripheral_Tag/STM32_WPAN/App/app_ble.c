...
DF_TAG_APP_ConnHandleNotEvt_t DF_TAGHandleNotification;

static char a_GapDeviceName[] = {  'D', 'i', 'r', 'e', 'c', 't', 'i', 'o', 'n', 'F', '_', 'T', 'a', 'g' }; /* Gap Device Name */
...
void APP_BLE_Init(void)
{
  ...
  /* USER CODE BEGIN APP_BLE_Init_2 */
	...
	/* 0x0006 = 0x0002 (Advertising) | 0x0004 (Peripheral connection) */
...
SVCCTL_UserEvtFlowStatus_t SVCCTL_App_Notification(void *p_Pckt)
{
  ...
  switch (p_event_pckt->evt)
  {
    case HCI_DISCONNECTION_COMPLETE_EVT_CODE:
    {
      ...
      DF_TAGHandleNotification.EvtOpcode = DF_TAG_DISCON_HANDLE_EVT;
      DF_TAGHandleNotification.ConnectionHandle = p_disconnection_complete_event->Connection_Handle;
      DF_TAG_APP_EvtRx(&DF_TAGHandleNotification);
    ...
    case HCI_LE_META_EVT_CODE:
    {
      ...
      switch (p_meta_evt->subevent)
      {
        ...
        case HCI_LE_ENHANCED_CONNECTION_COMPLETE_SUBEVT_CODE:
        {
          ...
          DF_TAGHandleNotification.EvtOpcode = DF_TAG_CONN_HANDLE_EVT;
          DF_TAGHandleNotification.ConnectionHandle = p_enhanced_conn_complete->Connection_Handle;
          DF_TAG_APP_EvtRx(&DF_TAGHandleNotification);
          ...
          ret = hci_le_set_connection_cte_transmit_parameters(
              p_enhanced_conn_complete->Connection_Handle,          // Connection handle that identifies the connection
          ...
          {
            LOG_INFO_APP("==>> hci_le_set_connection_cte_transmit_parameters : Success\n");
          }

          /* Enable the CTE response */
          ret = hci_le_connection_cte_response_enable(p_enhanced_conn_complete->Connection_Handle, ENABLE);

          if (ret != BLE_STATUS_SUCCESS)
          {
            LOG_INFO_APP("Failed to enable CTE response: reason=0x%02X\n", ret);
          }
          else
          {
            LOG_INFO_APP("==>> hci_le_connection_cte_response_enable : Success\n");
          }
        ...
        case HCI_LE_CONNECTION_COMPLETE_SUBEVT_CODE:
        {
          ...
          DF_TAGHandleNotification.EvtOpcode = DF_TAG_CONN_HANDLE_EVT;
          DF_TAGHandleNotification.ConnectionHandle = p_conn_complete->Connection_Handle;
          DF_TAG_APP_EvtRx(&DF_TAGHandleNotification);
          ...
          /* Set the CTE transmit parameters */
          ret = hci_le_set_connection_cte_transmit_parameters(
              p_conn_complete->Connection_Handle,          // Connection handle that identifies the connection
              CTE_TYPE_SUPPORT,           // Support for any CTE type
              TX_SWITCHING_PATTERN_LENGTH,// Length of the antenna switching pattern
              antenna_ids                 // List of antenna IDs in the pattern
          );
          ...
          /* Enable the CTE response */
          ret = hci_le_connection_cte_response_enable(p_conn_complete->Connection_Handle, ENABLE);
...
void APP_BLE_Procedure_Gap_Peripheral(ProcGapPeripheralId_t ProcGapPeripheralId)
{
  ...
  switch(ProcGapPeripheralId)
  {
    ...
    {
      status = aci_gap_terminate(bleAppContext.connectionHandle, 0x13);
      if (status != BLE_STATUS_SUCCESS)
      {
         LOG_INFO_APP("aci_gap_terminate failure: reason=0x%02X\n", status);
      }
...
void APP_BSP_Button1Action(void)
{
  ...
  {
    LOG_INFO_APP("Already connected.\n");
