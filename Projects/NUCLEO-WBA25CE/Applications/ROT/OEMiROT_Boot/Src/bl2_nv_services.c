...
HAL_StatusTypeDef plat_init_nv_counter(void)
{
  ...
  BOOT_LOG_INF("Consistent BL2 NV Counter %d  = 0x%" PRIx32, PLAT_NV_COUNTER_3, counter_value);
  ...
  BOOT_LOG_INF("Consistent BL2 NV Counter %d  = 0x%" PRIx32, PLAT_NV_COUNTER_4, counter_value);
#endif
                                      ...
                                      uint32_t Data, uint32_t *Updated)
{
  ...
  *Updated = 1U;
  ...
  uwAddressNextWrite += NVCNT_ELEMENT_SIZE;
  uhNbWrittenElements++;
  BOOT_LOG_INF("Counter %d set to 0x%" PRIx32 "", CounterId, Data);
  return HAL_OK;
}
  ...
  enum nv_counter_t CounterId)
{
  ...
  return plat_set_nv_counter(CounterId, security_cnt + 1u, &Updated);
}
#endif
                                             ...
                                             uint32_t size, uint8_t *val)
{
  ...
  while (counter >= PAGE_HEADER_SIZE)
  {
    ...
    if ((err == ARM_DRIVER_OK) && (addressvalue != NVCNT_PAGESTAT_ERASED))
    {
      ...
      if (NVCNT_ID_VALUE(addressvalue) == CounterId)
      {
        /* Calculate crc of variable data and virtual address */
...
{
  NVCNT_ELEMENT_TYPE addressvalue = 0U;
  __IO NVCNT_DATA_TYPE header[1] = {0U};
