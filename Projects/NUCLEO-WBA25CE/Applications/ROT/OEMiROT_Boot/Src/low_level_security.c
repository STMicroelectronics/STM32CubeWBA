...
const struct mpu_armv8m_region_cfg_t region_cfg_init_s[] = {
           ...
           /* region 1: RW/PRIV_ONLY/EXEC_OK for OEMiROT Boot */
           {
               ...
               FLASH_BASE_S + FLASH_AREA_BL2_HDP_END_OFFSET - 1,
           ...
           /* Region 7: Allows execution of RSSLIB */
           {
               7,
               RSS_LIB_BASE,
...
void LL_SECU_CheckStaticProtections(void)
{
  ...
  end = (FLASH_AREA_BL2_HDP_END_OFFSET- 1) / PAGE_SIZE;
#endif /* OEMUROT_ENABLE */
...
static void active_tamper(void)
{
  ...
  else
  {
    ...
    if (READ_REG(TAMP->ERCFGR) != DeviceSecretConf)
    {
      Error_Handler();
    }
