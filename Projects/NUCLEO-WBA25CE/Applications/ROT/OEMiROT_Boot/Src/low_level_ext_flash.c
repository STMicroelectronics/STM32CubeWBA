...
static int32_t Ext_Flash_ReadData(uint32_t addr, void *data, uint32_t cnt)
{
  ...
  {
    BOOT_LOG_ERR("read xspi not allowed 0x%x n=%x \r\n", (addr + EXT_FLASH_BASE_ADDRESS), cnt);
  ...
  if (err != BSP_ERROR_NONE)
  {
    BOOT_LOG_ERR("failed read external memory 0x%x n=%x \r\n", (addr + EXT_FLASH_BASE_ADDRESS), cnt);
                                      ...
                                      const void *data, uint32_t cnt)
{
  ...
  {
    BOOT_LOG_ERR("write xspi not allowed 0x%x n=%x \r\n", (addr + EXT_FLASH_BASE_ADDRESS), cnt);
    ARM_EXT_FLASH0_STATUS.error = DRIVER_STATUS_ERROR;
    return ARM_DRIVER_ERROR_PARAMETER;
  }
  ...
  if (err != BSP_ERROR_NONE)
  {
    BOOT_LOG_ERR("->>> Failed to write 0x%" PRIx32 " bytes at 0x%" PRIx32 " (Error 0x%" PRIx32 ")", cnt, (uint32_t)(EXT_FLASH_BASE_ADDRESS + addr), err);
    return ARM_DRIVER_ERROR;
  }
  return ARM_DRIVER_OK;
}
...
static int32_t Ext_Flash_EraseSector(uint32_t addr)
{
  ...
  {
    BOOT_LOG_ERR("erase xspi not allowed 0x%x\r\n", (addr + EXT_FLASH_BASE_ADDRESS));
    ARM_EXT_FLASH0_STATUS.error = DRIVER_STATUS_ERROR;
    return ARM_DRIVER_ERROR_PARAMETER;
  }
  ...
  if (err != BSP_ERROR_NONE)
  {
    BOOT_LOG_ERR("->>> Erase failed at 0x%" PRIx32 " (Error 0x%" PRIx32 ")", (uint32_t)(EXT_FLASH_BASE_ADDRESS + addr), err);
    return ARM_DRIVER_ERROR;
  }
  return ARM_DRIVER_OK;
}
