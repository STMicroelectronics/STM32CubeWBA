...
{
  /* Clock enabling */
  RCC->AHB1ENR |= 0x81021101UL;
  RCC->AHB2ENR |= 0x60A70087UL;
  RCC->AHB4ENR |= 0x00000024UL;
  RCC->AHB5ENR |= 0x00000003UL;
  RCC->APB1ENR1 |= 0x00200001UL;
  RCC->APB1ENR2 |= 0x00000020UL;
  RCC->APB2ENR  |= 0x01264000UL;
  RCC->APB7ENR  |= 0x002008E2UL;

  /* Illegal access interrupts configuration */
  GTZC_TZIC->IER1 |= 0x00022081UL;
  GTZC_TZIC->IER2 |= 0x010F80E8UL;
  GTZC_TZIC->IER3 |= 0x01D13848UL;
  GTZC_TZIC->IER4 |= 0x03D0EF8FUL;

  /* Securable peripherals security and privilege management */
  GTZC_TZSC->SECCFGR1  |= 0x00022081UL;
  GTZC_TZSC->SECCFGR2  |= 0x010F80E8UL;
  GTZC_TZSC->SECCFGR3  |= 0x01D13848UL;

  /* TrustZone-aware peripherals security and privilege management */

  /* PWR */
  PWR->SECCFGR  |= 0x000070EFUL;

  /* RCC */
  RCC->SECCFGR  |= 0x000010FBUL;

  /* GPIO */
  GPIOA->SECCFGR |= 0x0000FFE7UL;
  GPIOB->SECCFGR |= 0x0000937CUL;
  GPIOC->SECCFGR |= 0x0000E000UL;
  GPIOH->SECCFGR |= 0x00000008UL;

  /* SYSCFG */
  SYSCFG->SECCFGR |= 0x0000000BUL;

  /* DMA */
  LPDMA1->SECCFGR  |= 0x000000FFUL;

  /* EXTI */
  EXTI->SECCFGR1  |= 0x0019FFFFUL;
  ...
  /* TAMP */
  TAMP->SECCFGR  |= 0x80FF80FFUL;

  /* Registers accesses disabling */
  PWR->DBPR     &= ~PWR_DBPR_DBP;

  /* Clock disabling */
  RCC->AHB1ENR &= ~(0x01021001UL);
  RCC->AHB2ENR &= ~(0x20A70087UL);
  RCC->AHB4ENR &= ~(0x00000024UL);
  RCC->AHB5ENR &= ~(0x00000003UL);
  RCC->APB1ENR1 &= ~(0x00200001UL);
  RCC->APB1ENR2 &= ~(0x00000020UL);
