...
#define FLASH0_SECTOR_SIZE 0x2000     /* 8 KB */
#define FLASH0_PAGE_SIZE 0x2000       /* 8 KB */
...
static int32_t Flash_ReadData(uint32_t addr, void *data, uint32_t cnt)
{
  ...
  else
  {
    ...
    BOOT_LOG_ERR("Double ECC error detected: FLASH_ECCR=0x%x", FLASH->ECCR);
                                 ...
                                 const void *data, uint32_t cnt)
{
  ...
  */
  ...
  if (err != HAL_OK)
  {
    ...
    BOOT_LOG_ERR("->>> Failed to write 0x%" PRIx32 " bytes at 0x%" PRIx32 " (Error 0x%" PRIx32 ")", cnt, (uint32_t)FLASH_BASE + addr, flashError);
  }
