...
static void NonSecure_Init(void)
{
  ...
  unsecure_sram1((uint32_t)(LOADER_NS_DATA_START - SRAM1_BASE_NS),(uint32_t)( LOADER_NS_DATA_START + LOADER_NS_DATA_SIZE - SRAM1_BASE_NS - 1));
...
static void  unsecure_sram1(uint32_t offset_start, uint32_t offset_end)
{
  ...
  uint32_t index;
  uint32_t block_start = offset_start / (GTZC_MPCBB_BLOCK_SIZE);
  uint32_t block_end = offset_end / (GTZC_MPCBB_BLOCK_SIZE);
