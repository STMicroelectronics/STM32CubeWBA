## __Zigbee_Thermostat_Server_Router Application Description__
...
Once the Zigbee mesh network is created, the Server sends regularly the Temperature to Client. The user can manipulate a Menu through joystick actions JT1, JT2 and  JT3.
...
* One STM32WBA55xx/STM32WBA65xx Discovery board loaded with application : **Zigbee_Thermostat_Client_Coord**  
* One or more STM32WBA55xx/STM32WBA65xx board loaded with application : **Zigbee_Thermostat_Server_Router**  
...
<pre>
           ...
           |            |                                                                |            |             
           |            |                                                                |            | <= Push JT1/JT2/JT3 : Change a Thermostat setting (see Menu description below)
           |            |                                                                |            |			 
           |            |                                                                |            |	
           |            |                                                                |            |			 
           +----------+                                                                  +------------+
...
* First, open the projects, build them and load your generated applications on your STM32WBA55xx/STM32WBA65xx devices.
...
&rarr;  At this stage, Blue LED blinks indicating that the Zigbee network is being created. This usually takes about 15 seconds. It is important to wait until the end of the commissioning before to start using the joystick.     
	 
	4. It is now possible to change the configuration of Thermostat by pressing on the JT1/JT2/JT3 joystick Center/LOW/High. See definition bellow.  
	
	5. JT1 joystick Center definition: MENU_BUTTON_ENTER.
	
	6. JT2 joystick Low definition: MENU_BUTTON_ARROW_LOW.
	
	7. JT3 joystick High definition: MENU_BUTTON_ARROW_HIGH.
	
	8. The following Menu is used to change Thermostat configuration:  
        ...
        * Sub-Menu **SysMode** is composed by :
            ...
            * Fan
            * Dry
	9. To navigate on Menu you need to :
		* Press JT1 to enter in Menu, enter in Sub-Menu or validate a value and exit menu.
		* Press JT3 to navigate down in the Menu or Sub-Menu or decrease a value.
		* Press JT2 to navigate up in the Menu or Sub-Menu or increase a value.
		* If a JTx is not pressed after 10 seconds, exit Menu.  
...
<pre> 
		   ...
		   +---------------------+   JT3    +-------------------+    JT3      +-------------------+           JT3           +-------------------+        JT3          +-----------+
		          ...
		          JT1 |                               |                          JT1  |                                               JT1|
		              v                               v                      (Default)|    +-------------------+                (Default)|      +-----------------+ 
	+------------------------------+	      Same as Cooling menu                    |    |    Cooling Only   |   JT1                   |      |     OFF         |   JT1   
	...
	+------------------------------+     | JT3                                        |        |            ^          |                 |          |         ^           |     
                 ...
                 JT2|                    |                                            |        v            |          |                 |          v         |           | 
                    v                    v                                            |    +-------------------+       |                 |      +-----------------+       |  
		+-------------------+  JT3  +-------------------+                             |    | Cooling + Reheat  |   JT1 |                 |----->|     Auto        |   JT1 | 
	|-->| Action: X=X-0.5   |------>|  Action: X=X+0.5  |<---|                        |    |                   |-------|                        |                 |-------| 
	|	|                   |<------|                   |    |                        |    +-------------------+       |                        +-----------------+       |  
	|	+-------------------+  JT2  +-------------------+    |                        |        |            ^          |                            |         ^           | 
	| JT2  |   |                      |               |  JT3 |                        |        |JT3         |JT2       |                            |JT3      |JT2        | 
	|------|   |                      |               |------|                        |        v            |          |                            v         |           | 
		       | JT1                  | JT1                                           |    +-------------------+       |                        +-----------------+       |  
			   v				      v		            							  |    |   Heating Only    |   JT1 |                        |     Not Used    |   JT1 | 
       +-----------------------------------------+                                    |    |                   |-------|                        |                 |-------| 					    
	   ...
	   +-----------------------------------------+                                    |        |JT3         |JT2       |                            |JT3     |JT2         | 
	                                                                                  |        v            |          |                            v        |            | 
																					  |    +-------------------+       |                        +-----------------+       |  
																					  |    | Heating + Reheat  |  JT1  |                        |     Cooling     |  JT1  | 
																					  |    |                   |-------|                        |                 |-------| 
																					  |    +-------------------+       |                        +-----------------+       |     
						...
						change in all cases											  |  +----------------------+      |                                                  |
																					  |->| Cooling and Heating  |  JT1 |    +-----------------------------------------+   |
																						 ...
																						 +----------------------+      |    |                                         |<--|
																							   ...
																							   v            |          |
																				  +-------------------------------+    |
																				  | Cooling and Heating + Reheat  | JT1|
																				  |                               |----|
																				  +-------------------------------+
...
* This example runs on STM32WBA55xx/STM32WBA65xx devices.  

* This example has been tested with an STMicroelectronics STM32WBA55G_Discovery/STM32WBA65I_Discovery board and can be easily tailored to any other supported device and development board.  
