 ...
 * @retval None
 */
static void APP_THREAD_SetJoystickRightAction(void *arg)
{
  UTIL_SEQ_SetTask(TASK_BSP_JOY_RIGHT, CFG_SEQ_PRIO_1);
}
#endif

static void APP_THREAD_AppInit(void)
{
  UTIL_SEQ_SetTask(TASK_BSP_JOY_LEFT, CFG_SEQ_PRIO_0);
#ifdef APP_THREAD_PERIODIC_TRANSMIT
  ...
  UTIL_TIMER_Create(&APP_Thread_transmitTimerId,
                    0,
                    UTIL_TIMER_PERIODIC,
                    &APP_THREAD_SetJoystickRightAction,
                    0);
#endif
}
...
{
  /* USER CODE BEGIN JoystickRightAction */
#ifdef APP_THREAD_PERIODIC_TRANSMIT
  ...
  /* USER CODE END JoystickRightAction */
}
...
/**
 * @brief Task associated to the joystick left button.
 * @param  None
 * @retval None
 */
void APP_BSP_JoystickLeftAction(void)
{
  LOG_INFO_APP("Send a CoAP CONFIRMABLE PUT Request");
#ifdef APP_THREAD_PERIODIC_TRANSMIT
  /* USER CODE BEGIN JoystickLeftAction */
  UTIL_TIMER_Stop(&APP_Thread_transmitTimerId);
  LOG_INFO_APP("INFO: PERIODIC_TRANSMIT timer stopped");
