...
static tBleStatus HAPAPP_HAPInit(HAP_Role_t Role, uint8_t CSIPConfId);
...
void HAPAPP_Init(uint8_t csip_config_id)
{
  ...
  if (status == BLE_STATUS_SUCCESS)
  {
    ...
    status = HAPAPP_HAPInit(APP_HAP_ROLE, csip_config_id);
          ...
          if (HAPAPP_Context.ACL_Conn[conn].Acl_Conn_Handle != 0xFFFFu)
          {
                ...
                || (p_other_conn->AvailableSrcAudioContext != HAPAPP_Context.AvailableSrcAudioContext))
            {
              ...
              LOG_INFO_APP("Send Update Available Snk Audio Contexts 0x%04X and Available Src Audio Contexts 0x%04X with CAP Initiator on ConnHandle 0x%04X returns status 0x%02X\n",
                           ...
                           p_other_conn->Acl_Conn_Handle,
                           status);
              p_other_conn->AvailableSnkAudioContext = HAPAPP_Context.AvailableSnkAudioContext;
              p_other_conn->AvailableSrcAudioContext = HAPAPP_Context.AvailableSrcAudioContext;
              UNUSED(status);
            }
            else
...
static tBleStatus CAPAPP_Init(Audio_Role_t AudioRole, uint8_t csip_config_id)
{
  ...
  if (audio_role & AUDIO_ROLE_SINK)
  {
    ...
    /*Set Codec ID configuration*/
    HAPAPP_Context.aSnkPACRecord[0].Cap.CodecID.CodingFormat = AUDIO_CODING_FORMAT_LC3;
    HAPAPP_Context.aSnkPACRecord[0].Cap.CodecID.CompanyID = COMPANY_ID;
    HAPAPP_Context.aSnkPACRecord[0].Cap.CodecID.VsCodecID = 0x0000;

    /* Set supported codec capabilities configuration */
    HAPAPP_Context.aSnkCap[0][0] = 0x03;
    HAPAPP_Context.aSnkCap[0][1] = CODEC_CAP_SAMPLING_FREQ;
    for (uint8_t i = 0; i < num_snk_capabilities ;i++)
    {
      HAPAPP_Context.aSnkCap[0][2] |= (uint8_t) (APP_CodecSpecificCap[a_snk_cap_id[i]].Freq);
      HAPAPP_Context.aSnkCap[0][3] |= (uint8_t) ((APP_CodecSpecificCap[a_snk_cap_id[i]].Freq >> 8 ));
    ...
    HAPAPP_Context.aSnkCap[0][4] = 0x02;
    HAPAPP_Context.aSnkCap[0][5] = CODEC_CAP_FRAME_DURATION;
    for (uint8_t i = 0; i < num_snk_capabilities ;i++)
    {
      HAPAPP_Context.aSnkCap[0][6] |= APP_CodecSpecificCap[a_snk_cap_id[i]].FrameDuration;
    }

    HAPAPP_Context.aSnkCap[0][7] = 0x05;
    HAPAPP_Context.aSnkCap[0][8] = CODEC_CAP_OCTETS_PER_CODEC_FRAME;
    for (uint8_t i = 0; i < num_snk_capabilities ;i++)
    {
      ...
      if (i > 0)
      {
        min_octet_per_codec_frame = MIN(min_octet_per_codec_frame,
                                        HAPAPP_Context.aSnkCap[0][9] + (HAPAPP_Context.aSnkCap[0][10] << 8));
        max_octet_per_codec_frame = MAX(max_octet_per_codec_frame,
                                        HAPAPP_Context.aSnkCap[0][11] + (HAPAPP_Context.aSnkCap[0][12] << 8));
      }

      HAPAPP_Context.aSnkCap[0][9] = (uint8_t) (min_octet_per_codec_frame);
      HAPAPP_Context.aSnkCap[0][10] = (uint8_t) (min_octet_per_codec_frame >> 8 );
      HAPAPP_Context.aSnkCap[0][11] = (uint8_t) (max_octet_per_codec_frame);
      HAPAPP_Context.aSnkCap[0][12] = (uint8_t) (max_octet_per_codec_frame >> 8);
    }

    HAPAPP_Context.aSnkCap[0][13] = 0x02;
    HAPAPP_Context.aSnkCap[0][14] = CODEC_CAP_AUDIO_CHNL_COUNTS;
    HAPAPP_Context.aSnkCap[0][15] = (APP_MAX_CHANNEL_PER_SNK_ASE | SUPPORTED_AUDIO_CHNL_COUNT_1);

    HAPAPP_Context.aSnkPACRecord[0].Cap.pSpecificCap = &HAPAPP_Context.aSnkCap[0][0];
    HAPAPP_Context.aSnkPACRecord[0].Cap.SpecificCapLength = 16u;

    /* metadata in Audio Codec Configuration*/
    HAPAPP_Context.aSnkPACRecord[0].Cap.MetadataLength = 0x04;
    HAPAPP_Context.aSnkMetadata[0][0] = METADATA_PREFERRED_AUDIO_CONTEXTS_LENGTH;
    HAPAPP_Context.aSnkMetadata[0][1] = METADATA_PREFERRED_AUDIO_CONTEXTS;
    HAPAPP_Context.aSnkMetadata[0][2] = (uint8_t) (preferred_audio_context);
    HAPAPP_Context.aSnkMetadata[0][3] = (uint8_t) (preferred_audio_context >> 8 );
    HAPAPP_Context.aSnkPACRecord[0].Cap.pMetadata = &HAPAPP_Context.aSnkMetadata[0][0];

    status = CAP_RegisterSnkPACRecord(&HAPAPP_Context.aSnkPACRecord[0],&HAPAPP_Context.aSnkPACRecordHandle[0]);
    if (status == BLE_STATUS_SUCCESS)
    {
  ...
  if (audio_role & AUDIO_ROLE_SOURCE)
  {
    ...
    /*Set Codec ID configuration*/
    HAPAPP_Context.aSrcPACRecord[0].Cap.CodecID.CodingFormat = AUDIO_CODING_FORMAT_LC3;
    HAPAPP_Context.aSrcPACRecord[0].Cap.CodecID.CompanyID = COMPANY_ID;
    HAPAPP_Context.aSrcPACRecord[0].Cap.CodecID.VsCodecID = 0x0000;

    /* Set supported codec capabilities configuration */
    HAPAPP_Context.aSrcCap[0][0] = 0x03;
    HAPAPP_Context.aSrcCap[0][1] = CODEC_CAP_SAMPLING_FREQ;
    for (uint8_t i = 0; i < num_src_capabilities ;i++)
    {
      HAPAPP_Context.aSrcCap[0][2] |= (uint8_t) (APP_CodecSpecificCap[a_src_cap_id[i]].Freq);
      HAPAPP_Context.aSrcCap[0][3] |= (uint8_t) ((APP_CodecSpecificCap[a_src_cap_id[i]].Freq >> 8 ));
    }

    HAPAPP_Context.aSrcCap[0][4] = 0x02;
    HAPAPP_Context.aSrcCap[0][5] = CODEC_CAP_FRAME_DURATION;
    for (uint8_t i = 0; i < num_src_capabilities ;i++)
    {
      HAPAPP_Context.aSrcCap[0][6] = APP_CodecSpecificCap[a_src_cap_id[i]].FrameDuration;
    }

    HAPAPP_Context.aSrcCap[0][7] = 0x05;
    HAPAPP_Context.aSrcCap[0][8] = CODEC_CAP_OCTETS_PER_CODEC_FRAME;
    for (uint8_t i = 0; i < num_src_capabilities ;i++)
    {
      ...
      {
        min_octet_per_codec_frame = MIN(min_octet_per_codec_frame,
                                        HAPAPP_Context.aSrcCap[0][9] + (HAPAPP_Context.aSrcCap[0][10] << 8));
        max_octet_per_codec_frame = MAX(max_octet_per_codec_frame,
                                        HAPAPP_Context.aSrcCap[0][11] + (HAPAPP_Context.aSrcCap[0][12] << 8));
      }

      HAPAPP_Context.aSrcCap[0][9] = (uint8_t) (min_octet_per_codec_frame);
      HAPAPP_Context.aSrcCap[0][10] = (uint8_t) (min_octet_per_codec_frame >> 8 );
      HAPAPP_Context.aSrcCap[0][11] = (uint8_t) (max_octet_per_codec_frame);
      HAPAPP_Context.aSrcCap[0][12] = (uint8_t) (max_octet_per_codec_frame >> 8);
    }

    HAPAPP_Context.aSrcCap[0][13] = 0x02;
    HAPAPP_Context.aSrcCap[0][14] = CODEC_CAP_AUDIO_CHNL_COUNTS;
    HAPAPP_Context.aSrcCap[0][15] = (APP_MAX_CHANNEL_PER_SRC_ASE | SUPPORTED_AUDIO_CHNL_COUNT_1);

    HAPAPP_Context.aSrcPACRecord[0].Cap.pSpecificCap = &HAPAPP_Context.aSrcCap[0][0];
    HAPAPP_Context.aSrcPACRecord[0].Cap.SpecificCapLength = 16u;

    /* metadata in Audio Codec Configuration*/
    HAPAPP_Context.aSrcPACRecord[0].Cap.MetadataLength = 0x04;
    HAPAPP_Context.aSrcMetadata[0][0] = METADATA_PREFERRED_AUDIO_CONTEXTS_LENGTH;
    HAPAPP_Context.aSrcMetadata[0][1] = METADATA_PREFERRED_AUDIO_CONTEXTS;
    HAPAPP_Context.aSrcMetadata[0][2] = (uint8_t) (AUDIO_CONTEXT_CONVERSATIONAL);
    HAPAPP_Context.aSrcMetadata[0][3] = (uint8_t) (AUDIO_CONTEXT_CONVERSATIONAL >> 8 );
    HAPAPP_Context.aSrcPACRecord[0].Cap.pMetadata = &HAPAPP_Context.aSrcMetadata[0][0];

    status = CAP_RegisterSrcPACRecord(&HAPAPP_Context.aSrcPACRecord[0],&HAPAPP_Context.aSrcPACRecordHandle[0]);
    if (status == BLE_STATUS_SUCCESS)
    {
  ...
  if (status == BLE_STATUS_SUCCESS)
  {
    ...
    HAPAPP_Context.NumSnkASEs = APP_NUM_SNK_ASE;
    HAPAPP_Context.NumSrcASEs = APP_NUM_SRC_ASE;
  }
  else
  {
...
static tBleStatus HAPAPP_HAPInit(HAP_Role_t Role, uint8_t CSIPConfId)
{
  ...
  if (status == BLE_STATUS_SUCCESS)
  {
    ...
    if (status == BLE_STATUS_SUCCESS)
    {
      ...
      for (i = 0; ((i < APP_HAP_NUM_PRESETS) && (status == BLE_STATUS_SUCCESS)); i++)
      {
...
static void HAPAPP_CAPNotification(CAP_Notification_Evt_t *pNotification)
{
  ...
  switch(pNotification->EvtOpcode)
  {
    case CAP_LINKUP_COMPLETE_EVT:
    {
      ...
      if (p_conn != 0)
      {
        ...
        if (p_conn->LinkupRetry != 1u)
        {
          ...
          {
            if ( (HAPAPP_Context.ACL_Conn[conn].Acl_Conn_Handle != 0xFFFFu) \
                  && (HAPAPP_Context.ACL_Conn[conn].LinkupRetry == 1u))
            {
              UTIL_TIMER_Create(&LinkupRetry_Timer,
                                LINKUP_RETRY_TIMEOUT,
                                UTIL_TIMER_ONESHOT,
    ...
    case CAP_UNICAST_SERVER_ENABLE_REQ_EVT:
    {
      ...
      Audio_Context_t snk_context = HAPAPP_Context.AvailableSnkAudioContext;
      Audio_Context_t src_context = HAPAPP_Context.AvailableSrcAudioContext;
      ...
      if (p_conn != 0)
      {
        ...
        else
        {
          ...
          for (uint8_t conn = 0u; conn < CFG_BLE_NUM_LINK ; conn++)
          {
            uint8_t streaming_ongoing = 0u;
            if ((HAPAPP_Context.ACL_Conn[conn].Acl_Conn_Handle != 0xFFFFu) \
            	&& (&HAPAPP_Context.ACL_Conn[conn] != p_conn))
            {
              p_other_conn = &HAPAPP_Context.ACL_Conn[conn];
#if (APP_NUM_SNK_ASE > 0u)
              for ( uint8_t i = 0; i < HAPAPP_Context.NumSnkASEs;i++)
    ...
    case CAP_UNICAST_PREFERRED_QOS_REQ_EVT:
    {
        ...
        *(info->pReason) = ASE_OP_RESP_NO_REASON;
        ...
        /* Remove extra margins added by the codec manager since they could be considered only one time but not for each ASE */
...
static void APP_LinkupTaskHandler(void)
{
  ...
  for (uint8_t conn = 0; conn < CFG_BLE_NUM_LINK ; conn++)
  {
    if ( HAPAPP_Context.ACL_Conn[conn].LinkupRetry == 1u)
    {
      ...
      HAPAPP_Context.ACL_Conn[conn].LinkupRetry = 0xFFu;
      status = HAPAPP_Linkup(HAPAPP_Context.ACL_Conn[conn].Acl_Conn_Handle);
      if (status == BLE_STATUS_SUCCESS)
      {
        return;
      }
  ...
  if (p_conn != 0)
  {
    ...
    if(status == 1u)
    {
      ...
      if (pASE->streaming_audio_context != audio_context)
      {
        ...
        /* if context are now available, update them */
        available_snk_context = HAPAPP_Context.AvailableSnkAudioContext | snk_context;
        available_src_context = HAPAPP_Context.AvailableSrcAudioContext | src_context;
            ...
            if ((HAPAPP_Context.ACL_Conn[conn].Acl_Conn_Handle != 0xFFFFu) && (&HAPAPP_Context.ACL_Conn[conn] != p_conn))
            {
                  ...
                  || (HAPAPP_Context.ACL_Conn[conn].AvailableSrcAudioContext != HAPAPP_Context.AvailableSrcAudioContext))
              {
                ...
                LOG_INFO_APP("Send Update Available Snk Audio Contexts 0x%04X and Available Src Audio Contexts 0x%04X with CAP Initiator on ConnHandle 0x%04X returns status 0x%02X\n",
                             ...
                             HAPAPP_Context.ACL_Conn[conn].Acl_Conn_Handle,
                             status);
                HAPAPP_Context.ACL_Conn[conn].AvailableSnkAudioContext = HAPAPP_Context.AvailableSnkAudioContext;
                HAPAPP_Context.ACL_Conn[conn].AvailableSrcAudioContext = HAPAPP_Context.AvailableSrcAudioContext;
              }
              else
              {
                                             ...
                                             Audio_Context_t AudioContexts)
{
  ...
  if (Type == ASE_SINK)
  {
    ...
    {
      /*If Audio Contexts includes Media type :  Media, Alerts and Unspecified Contexts would be no more available*/
      res_audio_contexts &= ( ~(AUDIO_CONTEXT_MEDIA | AUDIO_CONTEXT_UNSPECIFIED | AUDIO_CONTEXT_ALERTS ));
    }
    if (AudioContexts & AUDIO_CONTEXT_ALERTS)
    {
      /*If Audio Contexts includes Alerts type :  Media, Alerts and Unspecified Contexts would be no more available*/
      res_audio_contexts &= ( ~(AUDIO_CONTEXT_ALERTS | AUDIO_CONTEXT_UNSPECIFIED | AUDIO_CONTEXT_MEDIA ));
    }
    if (AudioContexts & AUDIO_CONTEXT_RINGTONE)
    {
      /*If Audio Contexts includes RingTone type :  Ringtone, Media, Alerts and Unspecified Contexts would be no more available*/
      res_audio_contexts &= ( ~(AUDIO_CONTEXT_RINGTONE | AUDIO_CONTEXT_UNSPECIFIED | AUDIO_CONTEXT_MEDIA | AUDIO_CONTEXT_ALERTS ));
    }
    if (AudioContexts & AUDIO_CONTEXT_CONVERSATIONAL)
    {
      /*If Audio Contexts includes Conversational type :  Conversational still available but Ringtone, Media, Alerts and Unspecified Contexts would no more available*/
      res_audio_contexts &= ( ~(AUDIO_CONTEXT_UNSPECIFIED | AUDIO_CONTEXT_RINGTONE | AUDIO_CONTEXT_MEDIA | AUDIO_CONTEXT_ALERTS ));
    }
  }
  else
  {
  ...
  if (pASE->type == ASE_SINK)
  {
    ...
    for (uint8_t conn = 0u; conn < CFG_BLE_NUM_LINK ; conn++)
    {
      if (HAPAPP_Context.ACL_Conn[conn].Acl_Conn_Handle != 0xFFFFu)
      {
        for ( uint8_t i = 0; i < HAPAPP_Context.NumSnkASEs;i++)
        {
          if ((HAPAPP_Context.ACL_Conn[conn].pASEs->aSnkASE[i].allocated == 1) \
              && (&HAPAPP_Context.ACL_Conn[conn].pASEs->aSnkASE[i] != pASE))
          {
            if ((HAPAPP_Context.ACL_Conn[conn].pASEs->aSnkASE[i].state == ASE_STATE_ENABLING) \
              || (HAPAPP_Context.ACL_Conn[conn].pASEs->aSnkASE[i].state == ASE_STATE_STREAMING))
            {
              use_context |= HAPAPP_Context.ACL_Conn[conn].pASEs->aSnkASE[i].streaming_audio_context;
              available_context &= ~HAPAPP_Context.ACL_Conn[conn].pASEs->aSnkASE[i].streaming_audio_context;
              LOG_INFO_APP("Streaming Audio Contexts 0x%04X associated to ASE ID %d on ConnHandle 0x%04X still in use\n",
                           HAPAPP_Context.ACL_Conn[conn].pASEs->aSnkASE[i].streaming_audio_context,
                           HAPAPP_Context.ACL_Conn[conn].pASEs->aSnkASE[i].ID,
                           HAPAPP_Context.ACL_Conn[conn].Acl_Conn_Handle);
            }
          }
        }
    ...
    if (available_context != 0x0000)
    {
      ...
      {
        available_context |= (AUDIO_CONTEXT_CONVERSATIONAL | AUDIO_CONTEXT_MEDIA | \
  ...
  if (pASE->type == ASE_SOURCE)
  {
    ...
    for (uint8_t conn = 0u; conn < CFG_BLE_NUM_LINK ; conn++)
    {
      if (HAPAPP_Context.ACL_Conn[conn].Acl_Conn_Handle != 0xFFFFu)
      {
        for ( uint8_t i = 0; i < HAPAPP_Context.NumSrcASEs;i++)
        {
          if ((HAPAPP_Context.ACL_Conn[conn].pASEs->aSrcASE[i].allocated == 1) \
              && (&HAPAPP_Context.ACL_Conn[conn].pASEs->aSrcASE[i] != pASE))
          {
            if ((HAPAPP_Context.ACL_Conn[conn].pASEs->aSrcASE[i].state == ASE_STATE_ENABLING) \
              || (HAPAPP_Context.ACL_Conn[conn].pASEs->aSrcASE[i].state == ASE_STATE_STREAMING))
            {
              use_context |= HAPAPP_Context.ACL_Conn[conn].pASEs->aSrcASE[i].streaming_audio_context;
              available_context &= ~HAPAPP_Context.ACL_Conn[conn].pASEs->aSrcASE[i].streaming_audio_context;
              LOG_INFO_APP("Streaming Audio Contexts 0x%04X associated to ASE ID %d on ConnHandle 0x%04X still in use\n",
                           HAPAPP_Context.ACL_Conn[conn].pASEs->aSrcASE[i].streaming_audio_context,
                           HAPAPP_Context.ACL_Conn[conn].pASEs->aSrcASE[i].ID,
                           HAPAPP_Context.ACL_Conn[conn].Acl_Conn_Handle);
            }
          }
        }
...
static APP_ACL_Conn_t *APP_AllocateACLConn(uint16_t ConnHandle)
{
  ...
  for (uint8_t i = 0; i < CFG_BLE_NUM_LINK; i++)
  {
    if ( HAPAPP_Context.ACL_Conn[i].Acl_Conn_Handle == 0xFFFFu)
    {
      ...
      HAPAPP_Context.ACL_Conn[i].ConfirmIndicationRequired = 0u;
      HAPAPP_Context.ACL_Conn[i].ForceCompleteLinkup = 0u;
      HAPAPP_Context.ACL_Conn[i].CurrentContentCtrlOp = 0x0000;
      HAPAPP_Context.ACL_Conn[i].PendingContentCtrlOp = 0x0000;
      return &HAPAPP_Context.ACL_Conn[i];
    }
  }
  return 0;
