...
void (* usart_txcpltclbk_saved)(struct __UART_HandleTypeDef *) = NULL;
...
#if (CFG_LOG_SUPPORTED != 0)
  usart_txcpltclbk_saved = LOG_UART_HANDLER.TxCpltCallback;
#endif /* CFG_LOG_SUPPORTED */
...
OPTIMIZED static void PWR_ExitOffMode( void )
{
  ...
  if ( 1UL == boot_after_standby )
  {
    NVIC_SetPriority(CODEC_MNGR_INTR_NUM, 15);
...
#if (CFG_LOG_SUPPORTED != 0)
  LOG_UART_HANDLER.TxCpltCallback = usart_txcpltclbk_saved;
#endif /* CFG_LOG_SUPPORTED */
...
OPTIMIZED static void PWR_EnterSleepMode( void )
{
  ...
  LL_RCC_SetAPB2Prescaler(LL_RCC_APB2_DIV_1); /*cannot go below to respect SAI register clock limitation */
  LL_RCC_SetAPB1Prescaler(LL_RCC_APB1_DIV_16);
  LL_RCC_SetAHBPrescaler(LL_RCC_SYSCLK_DIV_8); /*cannot go below otherwise DMA not fast enough */
