...
### __Hardware and Software environment__
 ...
 - This example runs on STM32WBA65xx devices with SMPS.
   ...
   - Connect a headset on Jack Connector CN3.
...
<br>
<br>
 - PBP Broadcast Names to synchronize are configurable via the aSourceList[] table in the pbp_app.c.
 - __At Startup__,
   - PBP Sink starts BLE Scanning and looks for remote PBP Source devices associated to the first Broadcast Name of the table (SCAN_INTERVAL and SCAN_WINDOW of the scanning are configurable in pbp_app.c file.).
   - PBP Sink starts automatically the Audio Synchronization process once PBP Source is found.
 - On the PBP Sink device, use Up/Down direction of the Joystick to change the Broadcast Name to look for. Target Broadcast Name is displayed on the LCD interface.
 - Power up PBP Source device with Broadcast Name matching with one of the names listed in PBP Sink application
 - Once PBP Source application is started, on the PBP Sink device, select with the joystick the Broadcast Name of the remote PBP Source to synchronize to.
