...
Menu_Content_Text_t startup_text = {3, {"BLE Audio", "TMAP Central", "Press Right"}};
...
Menu_Content_Text_t broadcast_config_text = {2, {"Broadcast Source", "48KHz"}};
...
uint8_t broadcast_freq_id = 4;

uint8_t a_broadcast_conf_id[5] = {
  1,  /* 8_2_1 */
  3,  /* 16_2_1 */
  5,  /* 24_2_1 */
  7,  /* 32_2_1 */
...
void Menu_SetProfilesLinked(audio_profile_t Profiles)
{
  ...
  snprintf(nostream_text.Lines[0], MENU_CONTROL_MAX_LINE_LEN, "Connected%s%s", bap_string, vcp_string);
  snprintf(linkup_text.Lines[0], MENU_CONTROL_MAX_LINE_LEN, "Linking Up%s%s", bap_string, vcp_string);
...
static void Menu_Start_Broadcast(void)
{
  ...
  TMAPAPP_StartBroadcastSource(a_broadcast_conf_id[broadcast_freq_id]);
}
...
static void Menu_Broadcast_Conf_Up(void)
{
  ...
  broadcast_freq_id = (broadcast_freq_id + 1) % 5;

  if (broadcast_freq_id == 4)
  {
  ...
  snprintf(&broadcast_config_text.Lines[1][0], MENU_CONTROL_MAX_LINE_LEN, "%s", a_broadcast_freq_name[broadcast_freq_id]);
  snprintf(&broadcast_stream_text.Lines[1][0], MENU_CONTROL_MAX_LINE_LEN, "%s", a_broadcast_freq_name[broadcast_freq_id]);
