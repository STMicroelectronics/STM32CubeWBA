...
{
  if (hadc == &hjoy_adc[JOY1])
  {
     GPIO_InitTypeDef         GPIO_Init;

    /* Enable GPIO clock */
    JOY1_TEST_CHANNEL_GPIO_CLK_ENABLE();

    /* Configure the selected ADC Channel as analog input */
    GPIO_Init.Pin       = JOY1_TEST_CHANNEL_GPIO_PIN;
    GPIO_Init.Mode      = GPIO_MODE_ANALOG;
    GPIO_Init.Speed     = GPIO_SPEED_FREQ_HIGH;
    GPIO_Init.Pull      = GPIO_NOPULL;
    HAL_GPIO_Init(JOY1_TEST_CHANNEL_GPIO_PORT, &GPIO_Init);
  }
}
...
void HAL_ADC_MspDeInit(ADC_HandleTypeDef *hadc)
{
  if (hadc == &hjoy_adc[JOY1])
  {
    HAL_GPIO_DeInit(JOY1_TEST_CHANNEL_GPIO_PORT, JOY1_TEST_CHANNEL_GPIO_PIN);
  }
}
#endif /*(CFG_TEST_VALIDATION == 1)*/
