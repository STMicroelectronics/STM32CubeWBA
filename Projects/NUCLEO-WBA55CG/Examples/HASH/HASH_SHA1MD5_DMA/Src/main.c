...
int main(void)
{
  ...
  if  (HAL_HASH_Start_DMA(&hhash, (uint8_t*)aInput, strlen((char const*)aInput), aSHA1Digest)  != HAL_OK) 
  {
    Error_Handler();
  }
  ...
  /* Start HASH computation using DMA transfer */
  if(HAL_HASH_Start_DMA(&hhash, (uint8_t*)aInput, strlen((char const*)aInput), aMD5Digest) != HAL_OK)
  {
    Error_Handler();
  }
