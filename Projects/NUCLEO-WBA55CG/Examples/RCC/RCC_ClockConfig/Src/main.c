...
static void SwitchSystemClock(void)
{
if (__HAL_RCC_GET_PLL1_OSCSOURCE() == RCC_PLLSOURCE_HSI)
  {
    /* PLL source is HSI oscillator */
    /* Set SYSCLK frequency to 100000000 Hz, coming from the PLL which is clocked by HSE  */
    SystemClockHSE_Config();
  }
else if (__HAL_RCC_GET_PLL1_OSCSOURCE() == RCC_PLLSOURCE_HSE)
  {
	/* PLL source is HSE  oscillator */
    /* Set SYSCLK frequency to 100000000 Hz, coming from the PLL which is clocked by HSI */
...
RCC_OscInitStruct.PLL1.PLLState          = RCC_PLL_ON;
 ...
 /* -3- Select the PLL as system clock source and configure the PCLK1, PCLK2 and PCLK7
     clocks dividers */ 
...
/* -2- Enable HSI Oscillator, select it as PLL source and finally activate the PLL */
  ...
  /* -3- Select the PLL as system clock source and configure the HCLK, PCLK1, PCLK2 and PCLK7
     clocks dividers */
  RCC_ClkInitStruct.ClockType = (RCC_CLOCKTYPE_SYSCLK | RCC_CLOCKTYPE_HCLK | RCC_CLOCKTYPE_PCLK1 \
                               | RCC_CLOCKTYPE_PCLK2 | RCC_CLOCKTYPE_PCLK7 | RCC_CLOCKTYPE_HCLK5);
  RCC_ClkInitStruct.SYSCLKSource    = RCC_SYSCLKSOURCE_PLLCLK;
