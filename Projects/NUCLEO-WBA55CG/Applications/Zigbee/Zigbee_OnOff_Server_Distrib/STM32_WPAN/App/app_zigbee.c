  ...
  while ( eStatus != ZB_STATUS_SUCCESS )
  {
    ...
    else
    {
      ...
      if ( stZigbeeAppInfo.eStartupControl == ZbStartTypeJoin )
      {
        ...
        /* On Distributed Network, if first 'Device' Startup is not OK, restart with 'Coordinator' Startup */
...
void APP_ZIGBEE_PrintGenericInfo( void )
{
  ...
  /* Display Link Key */
  LOG_INFO_APP( "Distrib. Global Key : %s", APP_ZIGBEE_GetDisplaySecKey( sec_key_distrib_uncert, ZB_SEC_KEYSIZE, true ) );
}
