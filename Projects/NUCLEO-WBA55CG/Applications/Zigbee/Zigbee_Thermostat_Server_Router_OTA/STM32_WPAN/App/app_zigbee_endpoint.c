...
#define FUOTA_PROFILE_ID                  ZCL_PROFILE_HOME_AUTOMATION
#define FUOTA_DEVICE_ID                   ZCL_DEVICE_THERMOSTAT

#define FUOTA_CLUSTER_ID                  ZCL_CLUSTER_OTA_UPGRADE
#define FUOTA_CLUSTER_NAME                "OTA Client"

/* USER CODE BEGIN PD */
#define APP_ZIGBEE_STARTUP_FAIL_DELAY     500u

/* Temperature Measures */
#define TEMP_MEASURED_MIN                 (int16_t)0xF060   /* Sensor limit : -40 degrees Celsius   */
#define TEMP_MEASURED_DEFAULT             (int16_t)0x09C4   /* Default Temp : +25 degrees Celsius   */
#define TEMP_MEASURED_MAX                 (int16_t)0x30d4   /* Sensor limit : +125 degrees Celsius  */
#define TEMP_TOLERANCE                    (int16_t)0x0032   /* Temp tolerance : +/-0.5 degrees Celsius */
...
#define TEMP_SIMU_INCREMENT               (int16_t)0x000A   /* Temp Simutation change : +/-0.1 degrees Celsius */
...
/* Private constants ---------------------------------------------------------*/
/* OTA Client Information about Certificate (if used) */
/* To obtains these keys, you need to create an account on Certicom site (www.certicom.com). */
#define   CA_PUB_KEY_SIZE       ( 37u )

const uint8_t szCAPubKey[CA_PUB_KEY_SIZE] =
{
  0,
};

PLACE_IN_SECTION("TAG_OTA_END") const uint32_t lMagicKeywordValue = FUOTA_MAGIC_KEYWORD_END_APPLI;
PLACE_IN_SECTION("TAG_OTA_INFO") const AppZbOta_ClientInfo_st stOtaClientInfo =
{
  /* OTA Client Information about this Firmware */
  .lFwEndMagicKeyAddress  = (uint32_t)&lMagicKeywordValue,
  .lBaseAddress           = FUOTA_APP_DOWNLOAD_BINARY_ADDRESS,
  .lOtaTagOffset          = FUOTA_APP_FW_TAG_OFFSET,
  .lOtaTagMagicKeyword    = FUOTA_MAGIC_KEYWORD_TAG_APPLI,

  .iManufacturerCode      = FUOTA_FW_APP_MANUFACTURER_CODE,
  .iImageType             = FUOTA_FW_APP_IMAGE_TYPE,
  .lCurrentFileVersion    = FUOTA_FW_APP_FILE_VERSION,

  .eActivationPolicy      = ZCL_OTA_ACTIVATION_POLICY_SERVER,
  .eTimeOutPolicy         = ZCL_OTA_TIMEOUT_POLICY_APPLY_UPGRADE,

  .pCertPublicKey         = (uint8_t *)szCAPubKey,
  .iCertPublicSize        = CA_PUB_KEY_SIZE,

  .cEndpoint              = FUOTA_ENDPOINT,
  .iProfile               = FUOTA_PROFILE_ID,
  .iDeviceId              = FUOTA_DEVICE_ID,
};
...
/* Private variables ---------------------------------------------------------*/
static bool                     bDownLoadServerFound;
...
static const struct MenuButtonsT stMenuButtonList[] = 
{
  ...
  { APP_MENU_EXIT,  "" ,              NULL,           NULL,                 0,                                0,                  0,              0 },
};
...
void APP_ZIGBEE_ApplicationStart( void )
{
  ...
  /* Verify if an OTA Server is found */
  bDownLoadServerFound = AppZbOtaClient_SearchOtaServer();
  if ( bDownLoadServerFound == false )
  {
    LOG_ERROR_APP( "OTA Server not found !" );
  }
...
void APP_ZIGBEE_ConfigEndpoints(void)
{
  ...
  /* Init OTA EndPoint & Cluster  */
  if ( AppZbOtaClient_ClusterInit( stZigbeeAppInfo.pstZigbee, &stOtaClientInfo ) == false )
  {
    LOG_ERROR_APP( "Error during OTA Client Endpoint/Cluster Register." );
  }
...
void APP_ZIGBEE_PrintApplicationInfo(void)
{
  ...
  LOG_INFO_APP( "  %s on Endpoint %d.", FUOTA_CLUSTER_NAME, FUOTA_ENDPOINT );
...
static void APP_ZIGBEE_ThermostatServerStart( void )
{
  ...
  /* Simulation of a Sensor. Start Temp in default Temp +/- 13 degrees Celsius */
  ZIGBEE_PLAT_RngGet( 1, (uint8_t *)&cTemp );
  iTemperature = TEMP_SIMU_START_DEFAULT + ( cTemp * 10 );
  
  /* Update default values for Thermostats. Diff between heating/cooling setpoint is 5 degrees Celsius and default mode is 'Auto'. */
