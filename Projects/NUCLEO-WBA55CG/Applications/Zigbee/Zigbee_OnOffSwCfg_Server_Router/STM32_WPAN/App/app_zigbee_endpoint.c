...
#define APP_ZIGBEE_CLUSTER2_NAME          "OnOffSwConfig Server"

/* OnoffConfig specific defines ----------------------------------------------------*/
#define APP_ZIGBEE_SWITCH_TYPE            ZCL_ONOFF_SWCONFIG_MOMENTARY
/* USER CODE BEGIN OnoffConfig defines */
/* USER CODE END OnoffConfig defines */


/* USER CODE BEGIN PD */
#define APP_ZIGBEE_SWITCH_ACTION          ZCL_ONOFF_SWCONFIG_TOGGLE_TOGGLE
...
#define OnOffSwConfigServer               pstZbCluster[1]
...
void APP_ZIGBEE_ConfigEndpoints(void)
{
  ...
  /* Add OnOffSwConfig Server Cluster */
  stZigbeeAppInfo.OnOffSwConfigServer = ZbZclOnOffSwConfigServerAlloc( stZigbeeAppInfo.pstZigbee, APP_ZIGBEE_ENDPOINT, APP_ZIGBEE_SWITCH_TYPE );
  assert( stZigbeeAppInfo.OnOffSwConfigServer != NULL );
  if ( ZbZclClusterEndpointRegister( stZigbeeAppInfo.OnOffSwConfigServer ) == false )
  {
    LOG_ERROR_APP( "Error during OnOffSwConfig Server Endpoint Register." );
  }

  /* USER CODE BEGIN APP_ZIGBEE_ConfigEndpoints2 */
  eStatus = ZbZclAttrIntegerWrite( stZigbeeAppInfo.OnOffSwConfigServer, ZCL_ONOFF_SWCONFIG_ATTR_ACTIONS, APP_ZIGBEE_SWITCH_ACTION );
  assert( eStatus == ZCL_STATUS_SUCCESS );
...
/**
 * @brief  Management of the SW1 button : Send the OnOff Command in function on OnOff-SwCfg.
 * @param  None
 * @retval None
 */
void APP_BSP_Button1Action(void)
{
  uint8_t               cSettingvalue;
  ...
  if ( APP_ZIGBEE_IsAppliJoinNetwork() != false )
  {
    ...
    cSettingvalue = (uint8_t)ZbZclAttrIntegerRead( stZigbeeAppInfo.OnOffSwConfigServer, ZCL_ONOFF_SWCONFIG_ATTR_ACTIONS, NULL, &eStatus );
    if ( eStatus == ZCL_STATUS_SUCCESS )
    {
      switch( cSettingvalue )
      {
        case  ZCL_ONOFF_SWCONFIG_ON_OFF:  LOG_INFO_APP( "[ONOFF-SWCFG] SW1 pushed, sending 'ON'" );
                                          eStatus = ZbZclOnOffClientOnReq( stZigbeeAppInfo.OnOffClient, &stDest, NULL, NULL );
                                          break;
                                          
        case  ZCL_ONOFF_SWCONFIG_OFF_ON:  LOG_INFO_APP( "[ONOFF-SWCFG] SW1 pushed, sending 'OFF'" );
                                          eStatus = ZbZclOnOffClientOffReq( stZigbeeAppInfo.OnOffClient, &stDest, NULL, NULL );
                                          break;
                                          
        case  ZCL_ONOFF_SWCONFIG_TOGGLE_TOGGLE:  
                                          LOG_INFO_APP( "[ONOFF-SWCFG] SW1 pushed, sending 'TOGGLE'" );
                                          eStatus = ZbZclOnOffClientToggleReq( stZigbeeAppInfo.OnOffClient, &stDest, NULL, NULL );
                                          break;
       
        default : eStatus = ZCL_STATUS_UNSUPP_ATTRIBUTE;
                  break;
      }
    }
    if ( eStatus != ZCL_STATUS_SUCCESS )
    {
      LOG_ERROR_APP( "[ONOFF-SWCFG] Error, OnOff Client Request failed (0x%02X).", eStatus );
    }
  }
}
...
/**
 * @brief  Management of the SW2 button : Send the OnOff Command in function on OnOff-SwCfg.
 * @param  None
 * @retval None
 */
void APP_BSP_Button2Action(void)
{
  ...
  if ( APP_ZIGBEE_IsAppliJoinNetwork() != false )
  {
    // -- For this button, it's work only if not Toggle Setting --
    cSettingvalue = (uint8_t)ZbZclAttrIntegerRead( stZigbeeAppInfo.OnOffSwConfigServer, ZCL_ONOFF_SWCONFIG_ATTR_ACTIONS, NULL, &eStatus );
    if  ( ( eStatus == ZCL_STATUS_SUCCESS ) && ( cSettingvalue != ZCL_ONOFF_SWCONFIG_TOGGLE_TOGGLE ) )
    {
      /* Prepare destination */
      memset( &stDest, 0, sizeof( stDest) );
      stDest.endpoint = APP_ZIGBEE_ENDPOINT;
      ...
      switch( cSettingvalue )
      {
        case  ZCL_ONOFF_SWCONFIG_ON_OFF:  LOG_INFO_APP( "[ONOFF-SWCFG] SW2 pushed, sending 'OFF'" );
                                          eStatus = ZbZclOnOffClientOffReq( stZigbeeAppInfo.OnOffClient, &stDest, NULL, NULL );
                                          break;
                                          
        case  ZCL_ONOFF_SWCONFIG_OFF_ON:  LOG_INFO_APP( "[ONOFF-SWCFG] SW2 pushed, sending 'ON'" );
                                          eStatus = ZbZclOnOffClientOnReq( stZigbeeAppInfo.OnOffClient, &stDest, NULL, NULL );
                                          break;
