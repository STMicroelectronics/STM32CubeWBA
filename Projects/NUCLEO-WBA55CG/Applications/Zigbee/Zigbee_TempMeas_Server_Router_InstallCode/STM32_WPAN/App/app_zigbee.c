...
/* Private constants ---------------------------------------------------------*/
/* Install Code : ZB_SEC_KEYSIZE + 2 bytes CRC */
const uint8_t szInstallCode[ZB_SEC_KEYSIZE + 2] = { 0x83, 0xFE, 0xD3, 0x40, 0x7A, 0x93, 0x97, 0x23, 0xA5, 0xC6, 0x39, 0xB2, 0x69, 0x16, 0xD5, 0x05, 0xC3, 0xB5 };
...
void APP_ZIGBEE_NwkFormOrJoinTaskInit( void )
{
  ...
  LOG_INFO_APP( "On Coordinator, send this sentence on UART :");
  LOG_INFO_APP( "IC:%08X%08X:%s", LOG_NUMBER64( ZbExtendedAddress( stZigbeeAppInfo.pstZigbee ) ), 
               APP_ZIGBEE_GetDisplaySecKey( szInstallCode, ( ZB_SEC_KEYSIZE + 2u ), false ) );
  LOG_INFO_APP( "\nAfter, start Network Join by Press SW3 on this Device." );
}
...
void APP_ZIGBEE_NwkFormOrJoin(void)
{
  ...
  if ( stZigbeeAppInfo.eJoinStatus != ZB_STATUS_SUCCESS )
  {
    ...
    ZbAesMmoHash( szInstallCode, ( ZB_SEC_KEYSIZE + 2u ), stConfig.security.preconfiguredLinkKey );
...
void APP_ZIGBEE_PrintGenericInfo( void )
{
  ...
  LOG_INFO_APP( "Install Code : %s", APP_ZIGBEE_GetDisplaySecKey( szInstallCode, ZB_SEC_KEYSIZE, true ) );
}
