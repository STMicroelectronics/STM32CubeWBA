...
static const struct ZbZclAttrT astOnOffServerAttrList[] =
{
  {
    .attributeId = ZCL_ONOFF_ATTR_ONOFF,
    .dataType = ZCL_DATATYPE_BOOLEAN,
    .flags = ZCL_ATTR_FLAG_REPORTABLE | ZCL_ATTR_FLAG_PERSISTABLE,
    .customValSz = 0,
    .callback = NULL,
    .range = {0.f, 0.f},
    .reporting = {0u, 0u},
...
void APP_ZIGBEE_PersistenceStartup(void)
{
  ...
  APP_ZIGBEE_Persistence_Init();
  bStatus = APP_ZIGBEE_Persistence_StartupNwk( &stZigbeeAppInfo, true );
  if ( bStatus == true )
  {
    LOG_INFO_APP( "Startup with Persistence : Success" );
...
void APP_ZIGBEE_ConfigEndpoints(void)
{
  ...
  ZbZclAttrAppendList( stZigbeeAppInfo.OnOffServer, astOnOffServerAttrList, ZCL_ATTR_LIST_LEN( astOnOffServerAttrList) );
...
/**
 * @brief  Restore LED state after a startup from persistence memory
 * @param  None
 * @retval None
 */
static void APP_ZIGBEE_OnOffServerRestoreLedState( void )
{
  enum ZclStatusCodeT   eStatus;
  uint8_t               cAttrVal;

  eStatus = ZbZclAttrRead( stZigbeeAppInfo.OnOffServer, ZCL_ONOFF_ATTR_ONOFF, NULL, &cAttrVal, sizeof(cAttrVal), false );
  if ( eStatus != ZCL_STATUS_SUCCESS )
  {
    LOG_ERROR_APP( "[ONOFF] Error, Failed to read ZCL Attribute (0x%02X).", eStatus );
    return;
  }
  ...
  if ( cAttrVal != 0u )
  {
    ...
    eStatus = APP_ZIGBEE_OnOffServerOnCallback( stZigbeeAppInfo.OnOffServer, NULL, NULL );
    if ( eStatus != ZCL_STATUS_SUCCESS )
    {
      LOG_ERROR_APP( "[ONOFF] Error, Failed in the 'On' callback function (0x%02X).", eStatus );
    }
  }
  else
  {
    /* Call the 'Off' callback */
    eStatus = APP_ZIGBEE_OnOffServerOffCallback( stZigbeeAppInfo.OnOffServer, NULL, NULL );
    if ( eStatus != ZCL_STATUS_SUCCESS )
    {
      LOG_ERROR_APP( "[ONOFF] Error, Failed in the 'Off' callback function (0x%02X).", eStatus );
    }
  }
}
...
static void APP_ZIGBEE_OnOffClientStart(void)
{
  APP_ZIGBEE_OnOffServerRestoreLedState();
}
...
void APP_BSP_Button2Action(void)
{
  ...
  {
    ( void )ZbPersistNotifyRegister( stZigbeeAppInfo.pstZigbee, NULL, NULL );
    APP_ZIGBEE_Persistence_Delete();
    UTIL_SEQ_WaitEvt( EVENT_ZIGBEE_SNVMA_WRITE_ENDED );
    NVIC_SystemReset();
  }
}
