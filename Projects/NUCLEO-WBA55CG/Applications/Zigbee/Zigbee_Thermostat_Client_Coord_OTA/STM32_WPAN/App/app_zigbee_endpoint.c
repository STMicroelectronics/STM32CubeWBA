...
#define FUOTA_CLUSTER_NAME                "OTA Server"
...
void APP_ZIGBEE_ApplicationStart( void )
{
  ...
  /* Verify if a Binary file to download is present */
  bDownLoadFileOk = AppZbOtaServer_GetBinInfo();
  if ( bDownLoadFileOk == false )
  {
    LOG_ERROR_APP( "Binary Image to Download not found !" );
  }
...
void APP_ZIGBEE_ConfigEndpoints(void)
{
  ...
  if ( AppZbOtaServer_ClusterInit( stZigbeeAppInfo.pstZigbee, FUOTA_ENDPOINT, FUOTA_PROFILE_ID, FUOTA_DEVICE_ID ) == false )
  {
    LOG_ERROR_APP( "Error during OTA Server Endpoint/Cluster Register." );
  }
...
void APP_BSP_Button2Action(void)
{
  if ( bDownLoadFileOk != false )
  {
    AppZbOtaServer_StartClientUpdate();
  }
}
