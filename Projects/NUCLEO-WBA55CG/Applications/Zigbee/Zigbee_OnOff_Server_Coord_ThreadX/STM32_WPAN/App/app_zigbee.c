...
TX_SEMAPHORE                    EndWaitCallbackSemaphore;
TX_SEMAPHORE                    NwkFormStartSemaphore;
TX_SEMAPHORE                    NwkFormRestartWaitEndSemaphore;
...
void APP_ZIGBEE_NwkFormOrJoinTaskInit( void )
{
  ...
  {
    ThreadXStatus |= tx_semaphore_create( &NwkFormRestartWaitEndSemaphore, "NwkFormRestartWaitEndSemaphore", 0 );
  }

  /* Register Semaphore to Wait a time between 2 'Join' tentatives */
  if ( ThreadXStatus == TX_SUCCESS )
  {
    ThreadXStatus |= tx_semaphore_create( &EndWaitCallbackSemaphore, "EndWaitCallbackSemaphore", 0 );
  }

  /* Create the Network Form Thread with this stack */
  if ( ThreadXStatus == TX_SUCCESS )
  {
    ThreadXStatus |= tx_byte_allocate( pBytePool, (void **)&pStack, TASK_STACK_SIZE_ZIGBEE_NETWORK_FORM, TX_NO_WAIT );
  }
  if ( ThreadXStatus == TX_SUCCESS )
  {
...
static void APP_ZIGBEE_ZbCallbackTimeOutTimerCallback( void * arg )
{
  ...
  tx_semaphore_put( &EndWaitCallbackSemaphore );
}
...
{
    tx_semaphore_put( &NwkFormRestartWaitEndSemaphore );
}
...
static void APP_ZIGBEE_WaitForCallback( uint32_t lTimeOut )
{
  ...
  tx_semaphore_get( &EndWaitCallbackSemaphore, TX_WAIT_FOREVER );
}
...
void APP_ZIGBEE_NwkFormOrJoin(void)
{
  ...
  while ( eStatus != ZB_STATUS_SUCCESS )
  {
    ...
    else
    {
      ...
      tx_semaphore_get( &NwkFormRestartWaitEndSemaphore, TX_WAIT_FOREVER );
    }
  }
}
