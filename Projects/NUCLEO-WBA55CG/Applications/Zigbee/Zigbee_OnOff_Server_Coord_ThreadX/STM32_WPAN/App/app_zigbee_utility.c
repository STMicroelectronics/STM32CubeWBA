...
static TX_SEMAPHORE             stZbUtilWaitCallbackSemaphore;

/* Private Functions ------------------------------------------------------------------*/
static void AppZbUtil_WaitReceiveCallbackSemaphoreSet( void )
{
  if ( stZbUtilWaitCallbackSemaphore.tx_semaphore_count == 0 )
  {
    tx_semaphore_put( &stZbUtilWaitCallbackSemaphore );
  }
}
...
static void AppZbUtil_WaitReceiveCallbackInit( void)
{
  ...
  if ( bFirst == false )
  {
    ...
    if ( tx_semaphore_create( &stZbUtilWaitCallbackSemaphore, "ZbUtilWaitCbSem", 0 ) != TX_SUCCESS )
    {
      LOG_ERROR_APP( "ThreadX Error : Creation of 'ZigbeeUtil Wait Callback' Semaphore failed." );
      while(1);
    }
    bFirst = true;
  }
...
static void AppZbUtil_WaitReceiveCallbackStart( uint32_t lTimeMs )
{
  ...
  tx_semaphore_get( &stZbUtilWaitCallbackSemaphore, TX_WAIT_FOREVER );
