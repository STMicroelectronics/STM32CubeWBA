...
#define APP_ZIGBEE_DEVICE_ID              ZCL_DEVICE_ONOFF_OCCUPANCY_SENSOR
#define APP_ZIGBEE_GROUP_ADDRESS          0x0001u

#define APP_ZIGBEE_CLUSTER_ID             ZCL_CLUSTER_OCCUPANCY
#define APP_ZIGBEE_CLUSTER_NAME           "Occupancy Client"
...
/* Report defines */
#define SERVER_REPORT_PERIOD_MIN          0u        /* Request a Report every 60s */
#define SERVER_REPORT_PERIOD_MAX          10u
#define SERVER_REPORT_CHANGE              1u 
...
#define OccupancyClient                   pstZbCluster[0]
...
static void APP_ZIGBEE_OccupancySensingServerReport ( struct ZbZclClusterT * pstCluster, struct ZbZclHeaderT * pstZclHeader, struct ZbApsdeDataIndT * pstDataInd,
                                                    uint16_t iAttributeId, enum ZclDataTypeT eDataType, const uint8_t * pDataInputPayload, uint16_t iDataInputLength,
                                                    bool * bDiscard );

static enum ZclStatusCodeT APP_ZIGBEE_OccupancySensingServerCommand  ( struct ZbZclClusterT * pstCluster, struct ZbZclHeaderT * pstZclHeader, struct ZbApsdeDataIndT * pstDataInd );
...
void APP_ZIGBEE_ConfigEndpoints(void)
{
  ...
  stZigbeeAppInfo.OccupancyClient->report = &APP_ZIGBEE_OccupancySensingServerReport;
  stZigbeeAppInfo.OccupancyClient->command = &APP_ZIGBEE_OccupancySensingServerCommand;
...
static void APP_ZIGBEE_ReportConfigCallback( struct ZbZclCommandRspT * stCommandRsp, void * arg )
{
  ...
  {
    LOG_ERROR_APP("[OCCUPANCY SENSING] Report Config Failed error (0x%02X)", stCommandRsp->status);
  }
  else
  {
    LOG_INFO_APP("[OCCUPANCY SENSING] Report Config set with success");
  }
}
...
void APP_ZIGBEE_ReportConfig(void)
{
  ...
  stReportConfig.record_list[0].attr_id = ZCL_OCC_ATTR_OCCUPANCY;
  stReportConfig.record_list[0].attr_type = ZCL_DATATYPE_BITMAP_8BIT;
  
  LOG_INFO_APP( "[OCCUPANCY SENSING] Send Report Request to 0x%04X", iDeviceShortAddress );
  eStatus = ZbZclAttrReportConfigReq( stZigbeeAppInfo.OccupancyClient, &stReportConfig, &APP_ZIGBEE_ReportConfigCallback, NULL );
  if ( eStatus != ZCL_STATUS_SUCCESS )
  {
    LOG_ERROR_APP( "[OCCUPANCY SENSING] Error during Report Config Request (0x%02X)", eStatus );
  }
}
...
static enum ZclStatusCodeT APP_ZIGBEE_OccupancySensingServerCommand( struct ZbZclClusterT * pstCluster, struct ZbZclHeaderT * pstZclHeader, struct ZbApsdeDataIndT * pstDataInd )
{
  LOG_INFO_APP("[OCCUPANCY SENSING] Command received");
                                                    ...
                                                    bool * bDiscard )
{
  ...
  if ( iAttrLen < 0 )
  {
    LOG_ERROR_APP( "[OCCUPANCY SENSING] Report error length (%d)", iAttrLen );
    return;
  }

  if ( (uint16_t)iAttrLen > iDataInputLength )
  {
    LOG_ERROR_APP( "[OCCUPANCY SENSING] Report error length (%d > %d)", iAttrLen, iDataInputLength );
    return;
  }

  if ( pstDataInd->dst.endpoint != APP_ZIGBEE_ENDPOINT ) 
  {
    LOG_ERROR_APP( "[OCCUPANCY SENSING] Report error wrong endpoint (%d)", pstDataInd->dst.endpoint );
    return;
  }

  switch ( iAttributeId )
  {
    case ZCL_OCC_ATTR_OCCUPANCY:
        cAttrValue = (uint8_t)pDataInputPayload[0];
        if ( cAttrValue != 0u )
        {
          LOG_INFO_APP( "[OCCUPANCY SENSING] From 0x%016" PRIX64 " , Room is occupied.", pstDataInd->src.extAddr );
        }
        else
        {
          LOG_INFO_APP( "[OCCUPANCY SENSING] From 0x%016" PRIX64 " , Room is empty.", pstDataInd->src.extAddr );
        }
