...
#define APP_ZIGBEE_CLUSTER1_ID            ZCL_CLUSTER_DIAGNOSTICS
#define APP_ZIGBEE_CLUSTER1_NAME          "Diagnostics Client"
...
#define DiagnosticsClient                 pstZbCluster[0]
...
void APP_ZIGBEE_ConfigEndpoints(void)
{
  ...
  /* Add Diagnostics Client Cluster */
  stZigbeeAppInfo.DiagnosticsClient = ZbZclDiagClientAlloc( stZigbeeAppInfo.pstZigbee, APP_ZIGBEE_ENDPOINT );
  assert( stZigbeeAppInfo.DiagnosticsClient != NULL );
  if ( ZbZclClusterEndpointRegister( stZigbeeAppInfo.DiagnosticsClient ) == false )
  {
    LOG_ERROR_APP( "Error during Diagnostics Client Endpoint Register." );
  }
...
/**
 * @brief  Read Diagnostic Attribute callback
 * @param  rsp: ZCL response
 * @param  arg: passed argument
 * @retval None
 */
static void APP_ZIGBEE_DiagnosticClient_ReadAttributeCallback( const struct ZbZclReadRspT * pstResponse, void * arg)
{
  uint8_t   szBuffer[256];
  ...
  {
    LOG_ERROR_APP( "[DIAGNOSTIC] The server returned an error after an Attribute Read (0x%02X)", pstResponse->status );
  }
  else
  {
    /* Read Response */
      ...
      if ( iLength <= sizeof( szBuffer ) )
      {
        ...
        switch( pstResponse->attr[iIndex].attrId )
        {
          case ZCL_DIAG_SVR_ATTR_APS_RX_UCAST:
              LOG_INFO_APP( "[DIAGNOSTIC] The Diagnostic Server device APS layer received %d unicasts (%d bytes)", *(uint16_t *)szBuffer, iLength );
              LOG_INFO_APP( "[DIAGNOSTIC] (including +1 for the remote reading)." );
              break;  
              
          case ZCL_DIAG_SVR_ATTR_APS_TX_UCAST_SUCCESS:
              LOG_INFO_APP( "[DIAGNOSTIC] The Diagnostic Server device APS layer successfully sent %d unicasts (%d bytes)", *(uint16_t *)szBuffer, iLength );
              LOG_INFO_APP( "[DIAGNOSTIC] (including +1 for the remote reading)." );
              break; 

          default:
            LOG_ERROR_APP( "[DIAGNOSTIC] Error on Attributes %d: unsupported attribute.", pstResponse->attr[iIndex].attrId );
            break;
        } 
      }
      else
      {
        LOG_ERROR_APP( "[DIAGNOSTIC] Error on Attributes %d: attribute too long (%d).", iIndex, iLength );
      }
    }
  }
}
...
void APP_BSP_Button1Action(void)
{
  ...
  {
    LOG_INFO_APP( "[DIAGNOSTIC] Read 'RX/TX UCAST' attributes:" );
    ...
    stRequest.count = 2;
    stRequest.attr[0] = ZCL_DIAG_SVR_ATTR_APS_RX_UCAST;
    stRequest.attr[1] = ZCL_DIAG_SVR_ATTR_APS_TX_UCAST_SUCCESS;
    
    eStatus = ZbZclReadReq( stZigbeeAppInfo.DiagnosticsClient, &stRequest, APP_ZIGBEE_DiagnosticClient_ReadAttributeCallback, NULL );
    if ( eStatus != ZCL_STATUS_SUCCESS) 
    {
      LOG_ERROR_APP( "[DIAGNOSTIC] Error during Attribute Read (0x%02X).", eStatus );
    }
  }
}
