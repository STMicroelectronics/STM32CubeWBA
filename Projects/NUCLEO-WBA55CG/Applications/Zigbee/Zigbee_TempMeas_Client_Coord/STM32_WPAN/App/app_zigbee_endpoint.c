...
static void APP_ZIGBEE_ReportConfigCallback( struct ZbZclCommandRspT * stCommandRsp, void * arg )
{
  ...
  {
    LOG_ERROR_APP("[TEMP MEAS] Report Config Failed error : %d", stCommandRsp->status);
  }
  else
  {
    LOG_INFO_APP("[TEMP MEAS] Report Config set with success");
  }
}
...
void APP_ZIGBEE_ReportConfig(void)
{
  ...
  LOG_INFO_APP( "[TEMP MEAS] Send Report Request to 0x%04X", iDeviceShortAddress );
  eStatus = ZbZclAttrReportConfigReq( stZigbeeAppInfo.TempMeasClient, &stReportConfig, &APP_ZIGBEE_ReportConfigCallback, NULL );
  if ( eStatus != ZCL_STATUS_SUCCESS )
  {
    LOG_ERROR_APP( "[TEMP MEAS] Error, ZbZclAttrReportConfigReq %d", eStatus );
  }
}
...
static enum ZclStatusCodeT APP_ZIGBEE_TempMeasServerCommand( struct ZbZclClusterT * pstCluster, struct ZbZclHeaderT * pstZclHeader, struct ZbApsdeDataIndT * pstDataInd )
{
  LOG_INFO_APP("[TEMP MEAS] Command received");
  return( ZCL_STATUS_SUCCESS );
}


/**
 * @brief  Receive Report from TempMeas Server
                                             ...
                                             bool * bDiscard )
{
  ...
  if ( iAttrLen < 0 )
  {
    LOG_ERROR_APP( "[TEMP MEAS] Report error length (%d)", iAttrLen );
    return;
  }

  if ( (uint16_t)iAttrLen > iDataInputLength )
  {
    LOG_ERROR_APP( "[TEMP MEAS] Report error length (%d > %d)", iAttrLen, iDataInputLength );
    return;
  }

  if ( pstDataInd->dst.endpoint != APP_ZIGBEE_ENDPOINT ) 
  {
    LOG_ERROR_APP( "[TEMP MEAS] Report error wrong endpoint (%d)", pstDataInd->dst.endpoint );
    return;
  }
  ...
  switch ( iAttributeId )
  {
    ...
    default:
        //LOG_WARNING_APP("[From %016llx] - Reported another attribute %04X", pstDataInd->src.extAddr, iAttributeId );
        break;
  }
}
