...
/* USER CODE BEGIN PD */
#define BASIC_MFT_NAME                    "ST"                      /* New Manufacturer Name */
#define BASIC_POWER_SOURCE                ZCL_BASIC_POWER_BATTERY   /* New Power Source */

#define APP_ZIGBEE_APPLICATION_NAME       "OnOff Client with new Basic Attributes"
...
static void APP_ZIGBEE_OnOffClientStart(void)
{
  ...
  cBufSize = strlen( BASIC_MFT_NAME );
  memset( szBuffer, 0, sizeof(szBuffer) );
  memcpy( &szBuffer[1], BASIC_MFT_NAME, cBufSize );
  szBuffer[0] = cBufSize;
  
  /* ZCL_BASIC_ATTR_MFR_NAME attribute change */
  LOG_INFO_APP( "[BASIC] Writing 'Manufacturer Name' attribute." );
  eStatus = ZbZclBasicWriteDirect( stZigbeeAppInfo.pstZigbee, ZCL_BASIC_ATTR_MFR_NAME, szBuffer, ( ZCL_BASIC_MANUFACTURER_NAME_LENGTH + 1u ) );
  if ( eStatus != ZCL_STATUS_SUCCESS )
  {
    LOG_ERROR_APP( "[BASIC] Error during writing 'Company Name' (0x%02X).", eStatus );
    while(1) {}
  }
  
  /* ZCL_BASIC_ATTR_POWER_SOURCE attribute change */
  cPowerSource = BASIC_POWER_SOURCE;
  LOG_INFO_APP( "[BASIC] Writing 'Power Source' attribute." );
  eStatus = ZbZclBasicWriteDirect( stZigbeeAppInfo.pstZigbee, ZCL_BASIC_ATTR_POWER_SOURCE, &cPowerSource, 1u );
  if ( eStatus != ZCL_STATUS_SUCCESS )
  {
