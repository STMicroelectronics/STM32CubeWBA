...
void * ZIGBEE_PLAT_ZbHeapMalloc(uint32_t iSize)
{
  ...
  if (AMM_Alloc(CFG_AMM_VIRTUAL_APP_ZB_INIT, BYTES_TO_WORD32(iSize), (uint32_t**)&ptr, NULL) != AMM_ERROR_OK)
  {
    ptr = NULL;
  }
...
void * ZIGBEE_PLAT_HeapMalloc( uint32_t iSize )
{
  ...
  if (AMM_Alloc(CFG_AMM_VIRTUAL_APP_ZB_HEAP, BYTES_TO_WORD32(iSize), (uint32_t**)&ptr, NULL) != AMM_ERROR_OK)
  {
    ptr = NULL;
  }
