...
static void Ble_Hci_Gap_Gatt_Init(void)
{
  ...
  LOG_INFO_APP("==>> Extra_data_buffer initialization\n");
  for (i = 0 ; i < sizeof(extra_data_buffer) ; i++)
  {
    extra_data_buffer[i] = (uint8_t)i;
  }
...
void APP_BSP_Button3Action(void)
{
  ...
  {
    UTIL_SEQ_SetTask(1u << CFG_TASK_P2PC_LONG_WRITE_CHAR_ID, CFG_SEQ_PRIO_0);
    LOG_INFO_APP("Long write triggered\n");
  }
