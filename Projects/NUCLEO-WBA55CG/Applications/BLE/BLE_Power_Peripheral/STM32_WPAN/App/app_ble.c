...
typedef enum
{
  BLE_APP_TRANSFER_REQ_OFF,
  BLE_APP_TRANSFER_REQ_ON
} BLE_App_Transfer_Req_Status_t;
...
typedef struct
{
  ...
  uint8_t B2_Switch;
  uint8_t B3_Switch;
  BLE_App_Transfer_Req_Status_t ButtonTransferReq;
  UTIL_TIMER_Object_t RX_Int_timer_Id;
  UTIL_TIMER_Object_t DISABLE_IOs;
...
static char a_GapDeviceName[] = {  'P', 'o', 'w', 'e', 'r', ' ', 'P', 'e', 'r', 'i', 'p', 'h', 'e', 'r', 'a', 'l' }; /* Gap Device Name */

/* Advertising Data */
uint8_t a_AdvData[29] =
{
  4, AD_TYPE_COMPLETE_LOCAL_NAME, 'P', 'W', 'R',  /* Complete name */
  23, AD_TYPE_MANUFACTURER_SPECIFIC_DATA, 0x30, 0x00,
                                          0x00 /*  */,
                                          0x00 /*  */,
                                          0x00 /*  */,
                                          0x00 /*  */,
...
static void GPIO_Off_Req(void *arg);
...
/* USER CODE BEGIN EV */
extern PWR_CO_GPIO_Status_t PWR_CO_GPIO_Status;
...
SVCCTL_UserEvtFlowStatus_t SVCCTL_App_Notification(void *p_Pckt)
{
  ...
  switch (p_event_pckt->evt)
  {
    case HCI_DISCONNECTION_COMPLETE_EVT_CODE:
    {
      ...
      PWRHandleNotification.EvtOpcode = PWR_DISCON_HANDLE_EVT;
      PWRHandleNotification.ConnectionHandle = p_disconnection_complete_event->Connection_Handle;
      PWR_APP_EvtRx(&PWRHandleNotification);
    ...
    case HCI_LE_META_EVT_CODE:
    {
      ...
      switch (p_meta_evt->subevent)
      {
        ...
        case HCI_LE_ENHANCED_CONNECTION_COMPLETE_SUBEVT_CODE:
        {
          ...
          PWRHandleNotification.EvtOpcode = PWR_CONN_HANDLE_EVT;
          PWRHandleNotification.ConnectionHandle = p_enhanced_conn_complete->Connection_Handle;
          PWR_APP_EvtRx(&PWRHandleNotification);
        ...
        case HCI_LE_CONNECTION_COMPLETE_SUBEVT_CODE:
        {
          ...
          PWRHandleNotification.EvtOpcode = PWR_CONN_HANDLE_EVT;
          PWRHandleNotification.ConnectionHandle = p_conn_complete->Connection_Handle;
          PWR_APP_EvtRx(&PWRHandleNotification);
          /* USER CODE BEGIN HCI_EVT_LE_CONN_COMPLETE */
             ret = aci_gatt_exchange_config(bleAppContext.connectionHandle);
          if (ret != BLE_STATUS_SUCCESS)
          {
            LOG_INFO_APP("  Fail   : change MTU 0x%x\n", ret);
          }
          else
          {
...
void APP_BLE_Procedure_Gap_Peripheral(ProcGapPeripheralId_t ProcGapPeripheralId)
{
  ...
  switch(ProcGapPeripheralId)
  {
    ...
    case PROC_GAP_PERIPH_CONN_PARAM_UPDATE:
    {
      ...
      {
        bleAppContext.connIntervalFlag = 0;
        paramA = CONN_INT_MS(CONN_INT_REQ);
        paramB = CONN_INT_MS(CONN_INT_REQ);
      }
      else
      {
        bleAppContext.connIntervalFlag = 1;
        paramA = CONN_INT_MS(CONN_INT_LP_REQ);
        paramB = CONN_INT_MS(CONN_INT_LP_REQ);
      }
...
static void LinkConfiguration(void)
{
  ...
  status = hci_le_set_data_length(bleAppContext.connectionHandle, 251, 2120);
  if (status != BLE_STATUS_SUCCESS)
  {
    LOG_ERROR_APP("  Fail   : set data length command   : error code: 0x%02X\n", status);
  }
  else
  {
...
static void fill_advData(uint8_t *p_adv_data, uint8_t tab_size)
{
  ...
  {
    p_adv_data[0] = 4;
    p_adv_data[1] = AD_TYPE_COMPLETE_LOCAL_NAME;
    p_adv_data[2] = 'P';
    p_adv_data[3] = 'W';
    p_adv_data[4] = 'R';
    p_adv_data[5] = tab_size - 6;
    p_adv_data[6] = AD_TYPE_MANUFACTURER_SPECIFIC_DATA;

    for(uint8_t i= 7; i <tab_size; i++ )
    {
      p_adv_data[i] = 0xFF;
    }
  }
...
void APP_BLE_Key_Button1_Action(void)
{
  ...
  switch(bleAppContext.Device_Connection_Status)
  {
    case APP_BLE_CONNECTED_SERVER:
    {
      if(bleAppContext.ButtonTransferReq!= BLE_APP_TRANSFER_REQ_OFF)
      {
        bleAppContext.ButtonTransferReq = BLE_APP_TRANSFER_REQ_OFF;
        UTIL_TIMER_Stop(&bleAppContext.RX_Int_timer_Id);
      }
      else
      {

        bleAppContext.ButtonTransferReq = BLE_APP_TRANSFER_REQ_ON;
        UTIL_TIMER_StartWithPeriod(&bleAppContext.RX_Int_timer_Id, NOTIFY_TX_INT_MS);
...
{
  uint8_t B1,B2,B3;
    if(PWR_CO_GPIO_Status == PWR_CO_GPIO_OFF)
  {
    return;
  }
  else
  ...
  if ((B1 != 1)&&(bleAppContext.B1_Switch == 1))
  {
    UTIL_SEQ_SetTask(1U << CFG_TASK_BUTTON_1, CFG_SEQ_PRIO_0);
    bleAppContext.B1_Switch = 0;
    PWR_CO_GPIO_Status = PWR_CO_GPIO_TIM_RESET_ON;
  }
  else if (B1 != 0)
  {
    bleAppContext.B1_Switch = 1;
  }
     if ((B2 != 1)&&(bleAppContext.B2_Switch == 1))
  {
    UTIL_SEQ_SetTask(1U << CFG_TASK_BUTTON_2, CFG_SEQ_PRIO_0);
    bleAppContext.B2_Switch = 0;
    PWR_CO_GPIO_Status = PWR_CO_GPIO_TIM_RESET_ON;
  }
  else if (B2 != 0)
  {
    bleAppContext.B2_Switch = 1;
  }

  if ((B3 != 1)&&(bleAppContext.B3_Switch == 1))
  {
    UTIL_SEQ_SetTask(1U << CFG_TASK_BUTTON_3, CFG_SEQ_PRIO_0);
    bleAppContext.B3_Switch = 0;
    PWR_CO_GPIO_Status = PWR_CO_GPIO_TIM_RESET_ON;
  }
  else if (B3 != 0)
  {
    bleAppContext.B3_Switch = 1;
  }

  if (PWR_CO_GPIO_Status == PWR_CO_GPIO_TIM_RESET_ON)
  {
    UTIL_TIMER_Stop(&bleAppContext.DISABLE_IOs);
    UTIL_TIMER_StartWithPeriod(&bleAppContext.DISABLE_IOs, Setup_Time);
...
static void Send_Data_Req(void *arg)
{
  UTIL_SEQ_SetTask(1 << CFG_TASK_PWR_TX_ID, CFG_SEQ_PRIO_0);
  return;
}

static void GPIO_Off_Req(void *arg)
{
  UTIL_SEQ_SetTask(1 << CFG_TASK_GPIO_OFF_ID, CFG_SEQ_PRIO_0);
  return;
}
