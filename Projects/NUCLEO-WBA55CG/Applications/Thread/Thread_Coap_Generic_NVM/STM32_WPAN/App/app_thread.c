...
static void APP_THREAD_PersistenceStartup(void)
{
  ...
  APP_THREAD_Persistence_Init();
  
  APP_THREAD_Persistence_Restore();
...
void APP_BSP_Button3Action(void)
{
  ...
  /* Short press -> Save the Persistence Buffer
     Long Press  -> Erase the Persistence Buffer */
  
  if ( APP_BSP_ButtonIsLongPressed(B3) != 0 )
  {
    LOG_INFO_APP("Erase the persistent data");
    
    APP_THREAD_Persistence_Earse();
  }
  else
  {
    LOG_INFO_APP("Save the persistent data");
    APP_THREAD_Persistence_Save();
  }
