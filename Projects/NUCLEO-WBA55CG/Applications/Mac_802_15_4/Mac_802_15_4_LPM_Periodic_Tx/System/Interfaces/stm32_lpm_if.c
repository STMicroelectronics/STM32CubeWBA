...
static uint8_t lpm_app_use_lsi2 = 0;
...
OPTIMIZED void PWR_ExitOffMode( void )
{
  ...
  __HAL_FLASH_PREFETCH_BUFFER_ENABLE();
    ...
    if(LPM_is_wakeup_time_profiling_done() == 0)
    {
      ...
      lpm_wakeup_time_standby -= SysTick->VAL;
      lpm_wakeup_time_standby *= 1000000U;
      lpm_wakeup_time_standby /= LSI2_VALUE;
      ...
      /* Disable LSI2 if not used by the application */
      if(lpm_app_use_lsi2 == 0)
      {
...
uint32_t is_boot_from_standby(void)
{
    ...
    )
  {
    ...
    if(LPM_is_wakeup_time_profiling_done() == 0)
    {
      ...
      {
        /* Remember that LSI2 is used  by the application */
        lpm_app_use_lsi2 = 1;
      }
