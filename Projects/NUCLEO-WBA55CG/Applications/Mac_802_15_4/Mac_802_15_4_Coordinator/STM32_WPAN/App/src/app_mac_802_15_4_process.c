...
extern uint16_t shortAssociationAddrList[MAX_DEVICES_ASSO];//tab with all devices short address
extern uint8_t idAssociationList;//number of devices associated
extern uint8_t indDeviceSendData;//which device to send data
extern uint8_t retry_check_device;//allows to retry when data is send
...
MAC_Status_t APP_MAC_mlmeAssociateIndCb( const  ST_MAC_associateInd_t * pAssociateInd )
{
  ...
  for (uint8_t j=0;j<MAX_DEVICES_ASSO && shortAssociationAddr==COORD_ADDRESS ;j++) {
      for (uint8_t i=0;i<idAssociationList;i++) {
        if (shortAssociationAddrList[i] == (FIRST_ASSOCIATED_SHORT_ADDRESS + j)) {
         address_selected = 1;
        } 
    }
    if (!address_selected) {
      shortAssociationAddr = FIRST_ASSOCIATED_SHORT_ADDRESS + j;//this value is selected
    }
    address_selected = 0;
  }
...
{
  APP_DBG("COORD MAC - Receive BeaconNotify.Indication\r\n");
  /* return */
  return MAC_SUCCESS;
    ...
    if (pCommStatusInd->dst_addr_mode == g_SHORT_ADDR_MODE_c) {
      ...
      if (pCommStatusInd->dst_addr_mode == g_EXTENDED_ADDR_MODE_c) {
        APP_DBG("COORD MAC - Association succeed for 0x%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x\r\n",pCommStatusInd->dst_address.a_extend_addr[0], 
                pCommStatusInd->dst_address.a_extend_addr[1],pCommStatusInd->dst_address.a_extend_addr[2],pCommStatusInd->dst_address.a_extend_addr[3],
                pCommStatusInd->dst_address.a_extend_addr[4],pCommStatusInd->dst_address.a_extend_addr[5],pCommStatusInd->dst_address.a_extend_addr[6],
...
{
  APP_DBG("COORD MAC APP - Disassociate IND RECEIVED by 0x%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x\r\n",pDisassociateInd->a_device_address[0],
            pDisassociateInd->a_device_address[1],pDisassociateInd->a_device_address[2],pDisassociateInd->a_device_address[3],pDisassociateInd->a_device_address[4],
            pDisassociateInd->a_device_address[5],pDisassociateInd->a_device_address[6],pDisassociateInd->a_device_address[7]);
  /* return */
  return MAC_SUCCESS;
...
MAC_Status_t APP_MAC_mlmeScanCnfCb( const  ST_MAC_scanCnf_t * pScanCnf )
{
  ...
  uint8_t i = 0;
  //APP_DBG("FFD MAC APP - ScanConf status %x\r\n", pScanCnf->status);
...
MAC_Status_t APP_MAC_mcpsDataCnfCb( const  ST_MAC_dataCnf_t * pDataCnf )
{
  ...
  if(pDataCnf->status == MAC_SUCCESS) {
    ...
    APP_DBG("OK \r\n");
    retry_check_device = 0;
    indDeviceSendData++;//pass to the next list element
    return MAC_SUCCESS;
  }
  if(pDataCnf->status == TRANSACTION_EXPIRED) {// TRANSACTION_EXPIRED=0xF0
    /* Enable retry if we have TRANSACTION_EXPIRED */
    retry_check_device++;
    if (retry_check_device < 3) {
      return MAC_SUCCESS;
    }
  }
  if (pDataCnf->status == NO_ACK ) {//NO_ACK=0xE9
    ...
    memcpy(&tmp_shortAssociationAddrList, &shortAssociationAddrList, idAssociationList*2);//copy tab
    uint8_t j=0;
    for (uint8_t i=0;i<idAssociationList;i++) {
      if (indDeviceSendData != i) {
        shortAssociationAddrList[j] = tmp_shortAssociationAddrList[i];
        j++;
      }
    }
