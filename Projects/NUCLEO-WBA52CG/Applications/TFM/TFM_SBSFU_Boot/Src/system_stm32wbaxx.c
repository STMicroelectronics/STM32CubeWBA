...
void SystemInit(void)
{
  ...
  /* Configure the System clock source, PLL Multiplier and Divider factors,
     AHB/APBx prescalers and Flash settings for System clock 100 MHz ---------*/
  SetSysClock();
}
...
/**
  ...
  *         AHB/APBx prescalers and Flash settings for System clock 100MHz
  * @retval None
  */
static void SetSysClock(void)
{
  ...
  CLEAR_BIT(RCC->AHB4ENR, RCC_AHB4ENR_PWREN);
  ...
  MODIFY_REG(RCC->PLL1CFGR
           , RCC_PLL1CFGR_PLL1SRC | RCC_PLL1CFGR_PLL1M
           , LL_RCC_PLL1SOURCE_HSI | ((2 - 1UL) << RCC_PLL1CFGR_PLL1M_Pos));
  MODIFY_REG(RCC->PLL1DIVR
           , RCC_PLL1DIVR_PLL1N | RCC_PLL1DIVR_PLL1R
           , ((25 - 1UL) << RCC_PLL1DIVR_PLL1N_Pos) | ((2 - 1UL) << RCC_PLL1DIVR_PLL1R_Pos));
  SET_BIT(RCC->CR, RCC_CR_PLL1ON);
  SET_BIT(RCC->PLL1CFGR, RCC_PLL1CFGR_PLL1REN);
  while((READ_BIT(RCC->CR, RCC_CR_PLL1RDY) == RCC_CR_PLL1RDY) != 1UL)
  {
  }
  ...
  /* AHB prescaler 1 */
  MODIFY_REG(RCC->CFGR2, RCC_CFGR2_HPRE, LL_RCC_SYSCLK_DIV_1);
  /* Set APB1, APB2 and APB7 prescalers */
  MODIFY_REG(RCC->CFGR2, RCC_CFGR2_PPRE1, LL_RCC_APB1_DIV_1);
  MODIFY_REG(RCC->CFGR2, RCC_CFGR2_PPRE2, LL_RCC_APB2_DIV_1);
  MODIFY_REG(RCC->CFGR2, RCC_CFGR2_PPRE2, LL_RCC_APB7_DIV_1);

  /* Enable the HSI oscillator for RNG peripherals */
  RCC->CR |= RCC_CR_HSION;
}
/**
  * @}
