...
static struct flash_range secure_vect[] =
{
...
#else
  /* Area for : writing confirm flag , reading swap info , reading copy done and writing
  on in installed image */
  { S_IMAGE_PRIMARY_PARTITION_OFFSET + FLASH_S_PARTITION_SIZE - 64, S_IMAGE_PRIMARY_PARTITION_OFFSET + FLASH_S_PARTITION_SIZE -1},
#endif /* (TFM_PARTITION_FIRMWARE_UPDATE) */
#endif /* (MCUBOOT_S_DATA_IMAGE_NUMBER == 1) */
#if (MCUBOOT_S_DATA_IMAGE_NUMBER == 1)
#if !defined(TFM_PARTITION_FIRMWARE_UPDATE)
  { S_DATA_IMAGE_PRIMARY_PARTITION_OFFSET + FLASH_S_DATA_PARTITION_SIZE - 32, S_DATA_IMAGE_PRIMARY_PARTITION_OFFSET + FLASH_S_DATA_PARTITION_SIZE - 16 - 1},
#else
  { S_DATA_IMAGE_PRIMARY_PARTITION_OFFSET + FLASH_S_DATA_PARTITION_SIZE - 64, S_DATA_IMAGE_PRIMARY_PARTITION_OFFSET + FLASH_S_DATA_PARTITION_SIZE  - 1},
#endif /* (TFM_PARTITION_FIRMWARE_UPDATE) */
#endif /* (MCUBOOT_S_DATA_IMAGE_NUMBER == 1) */
};
