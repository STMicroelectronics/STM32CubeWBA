                                ...
                                psa_ecc_family_t *curve_type)
{
...
#else
  uint8_t rand_value;
  char *key_src = (char *)(FLASH_BASE_S + S_DATA_IMAGE_PRIMARY_PARTITION_OFFSET + S_DATA_IMAGE_IAT_PRIV_KEY_OFFSET);
  ...
  *curve_type = PSA_ECC_CURVE_SECP256R1;
  ...
  tfm_hal_random_generate(&rand_value, sizeof(rand_value));

  /* Copy the private key from the secure data image to the destination. It shouldn't be done in a zero-offset sequential
     manner (from left to right). Instead, it should be done with a dynamic offset that traverses the array starting at
     that offset modulo the length of the array */
  for (uint8_t i = 0, j = (rand_value % key_size); i < key_size; i++, j = (j + 1) % key_size)
  {
      key_dst[j] = key_src[j];
  }
#else
