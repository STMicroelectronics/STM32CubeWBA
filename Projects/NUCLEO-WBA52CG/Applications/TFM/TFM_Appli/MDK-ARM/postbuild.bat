...
IF "%signing%" == "nonsecure" (
set "command_init=%python%%imgtool% sign -k %key_ns% %encrypt% -e little -S %image_ns_size% -M %image_ns_sectors% -H 0x400 --pad-header %option% -v %version% --confirm --pad -s auto --align %val% %tfm_ns% %tfm_ns_init% >> %projectdir%\output.txt 2>&1"
goto :docommand_init
)
set "command_init=%python%%imgtool% sign -k %key_s% %encrypt% -e little -S %image_s_size% -M %image_s_sectors% -H 0x400 --pad-header %option% -v %version% --confirm --pad -s auto --align %val% %tfm_s% %tfm_s_init% >> %projectdir%\output.txt 2>&1"
:docommand_init
%command_init%
