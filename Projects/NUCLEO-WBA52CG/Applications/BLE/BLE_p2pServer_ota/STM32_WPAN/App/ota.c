...
/* Private variables ---------------------------------------------------------*/
static const uint8_t SizeBase_Adr = 6;
static const uint8_t SizeConf = 1;
static const uint8_t SizeRaw_Data = 248;
      ...
      switch(p_blecore_evt->ecode)
      {
        ...
        case ACI_GATT_WRITE_PERMIT_REQ_VSEVT_CODE:
        {
          ...
          if(p_write_perm_req->Attribute_Handle == (OTA_Context.Raw_DataCharHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
          {
            ...
            {
              APP_DBG_MSG("==>> ACI GATT Write response Failed , result: %d \n\r", ret);
            }
        ...
        case ACI_GATT_INDICATION_VSEVT_CODE:
          ...
          if (ret != BLE_STATUS_SUCCESS)
          {
            APP_DBG_MSG("==>> ACI GATT Confirmation indication Failed , result: %d \n\r", ret);
          }
...
void OTA_Init(void)
{
  ...
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_service command: OTA, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_service command: OTA \n\r");
  }
  ...
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : BASE_ADR, error code: 0x%2X\n", ret);
  }
  else
  {
  ...
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : CONF, error code: 0x%2X\n", ret);
  }
  else
  {
  ...
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : RAW_DATA, error code: 0x%2X\n", ret);
  }
  else
  {
...
tBleStatus OTA_UpdateValue(OTA_CharOpcode_t CharOpcode, OTA_Data_t *pData)
{
  ...
  switch(CharOpcode)
  {
    case OTA_BASE_ADR:
      ...
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value BASE_ADR command, error code: 0x%2X\n", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value BASE_ADR command\n");
      }
    ...
    case OTA_CONF:
      ...
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value CONF command, error code: 0x%2X\n", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value CONF command\n");
      }
    ...
    case OTA_RAW_DATA:
      ...
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value RAW_DATA command, error code: 0x%2X\n", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value RAW_DATA command\n");
      }
