...
/* Private variables ---------------------------------------------------------*/
static const uint8_t SizeTx_Char = 255;
static const uint8_t SizeRx_Char = 255;
static const uint8_t SizeThrough_Char = 255;
...
void DT_SERV_Init(void)
{
  ...
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_service command: DT_SERV, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_service command: DT_SERV \n\r");
  }
  ...
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : TX_CHAR, error code: 0x%2X\n", ret);
  }
  else
  {
  ...
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : RX_CHAR, error code: 0x%2X\n", ret);
  }
  else
  {
  ...
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : THROUGH_CHAR, error code: 0x%2X\n", ret);
  }
  else
  {
...
tBleStatus DT_SERV_UpdateValue(DT_SERV_CharOpcode_t CharOpcode, DT_SERV_Data_t *pData)
{
  ...
  switch(CharOpcode)
  {
    case DT_SERV_TX_CHAR:
      ...
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value TX_CHAR command, error code: 0x%2X\n", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value TX_CHAR command\n");
      }
    ...
    case DT_SERV_RX_CHAR:
      ...
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value RX_CHAR command, error code: 0x%2X\n", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value RX_CHAR command\n");
      }
    ...
    case DT_SERV_THROUGH_CHAR:
      ...
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value THROUGH_CHAR command, error code: 0x%2X\n", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value THROUGH_CHAR command\n");
      }
