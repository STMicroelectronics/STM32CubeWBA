...
/* Private typedef -----------------------------------------------------------*/
/* Private define ------------------------------------------------------------*/
#define OTP_PROG_STEP_SIZE                ((uint8_t)0x10U)

/* Private macro -------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
/* Private function prototypes -----------------------------------------------*/
static void OPENBL_OTP_Program(uint32_t Address, uint32_t DataAddress);
...
void OPENBL_OTP_Write(uint32_t Address, uint8_t *pData, uint32_t DataLength)
{
  ...
  if ((pData != NULL) && (DataLength != 0U))
  {
    ...
    {
      for (index = 0U; index < DBGMCU_AHB1FZR_DBG_GPDMA1_CH3_STOP_Pos; index++)
      {
