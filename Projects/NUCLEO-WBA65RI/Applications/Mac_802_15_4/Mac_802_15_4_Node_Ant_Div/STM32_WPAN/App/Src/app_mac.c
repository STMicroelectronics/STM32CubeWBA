...
#define ANT_DIV_DFLT_ANTENNA    1
#define ANT_DIV_RSSI_THRESHOLD  -36 //between -75 and -36
#define ANT_DIV_ADP_LENGTH_RADIO_EVT 10
//#define INTERVAL_RADIO_ACTIVITY //comment to use timer configuration
#define ANT_DIV_ADP_LENGTH_TIME 60 * 1000000 //60sec
...
static void APP_MAC_Ant_Div_Setup(void);
...
/**
  * @brief  Antenna diversity setup function
  *
  * @param  Nthing
  * @retval Nothing
  */
static void APP_MAC_Ant_Div_Setup(void)
{
  /* Antenna diversity parameters init */
#ifdef INTERVAL_RADIO_ACTIVITY
  if (ST_MAC_sys_svc_ant_div_set_intrv_radio_evts(&mac_hndl, ANT_DIV_ADP_LENGTH_RADIO_EVT) != MAC_SUCCESS)
  {
    APP_DBG("Antenna diversity interval init failed\r\n");
  }
#else
  if (ST_MAC_sys_svc_ant_div_set_intrv_time(&mac_hndl, ANT_DIV_ADP_LENGTH_TIME) != MAC_SUCCESS)
  {
    APP_DBG("Antenna diversity interval init failed\r\n");
  }
#endif

  if (ST_MAC_sys_svc_ant_div_init(&mac_hndl, ANT_DIV_RSSI_THRESHOLD, ANT_DIV_DFLT_ANTENNA, g_coordShortAddr, g_coordExtendedAddr) != MAC_SUCCESS) //-50 dbm RSSI threshold, antenna 1
  {
    APP_DBG("Antenna diversity threshold init failed\r\n");
  }

  if (ST_MAC_sys_svc_ant_div_en(&mac_hndl) != MAC_SUCCESS)
  {
    APP_DBG("Antenna diversity enable failed\r\n");
  }
}
...
void APP_RFD_MAC_802_15_4_SetupTask(void)
{
  ...
  **/
  ...
  ScanReq.scan_duration = scan_duration; // Max frame duration in us (16 * 960 * (2^n + 1)) , with n = scan_duration
  ...
  BSP_LED_On(LD1);
  APP_DBG("Node MAC APP - Antenna diversity enabled \r\n");
  APP_MAC_Ant_Div_Setup(); 
