...
typedef struct
{
  ...
  uint8_t               ConnectionInterval_Level;
  UTIL_TIMER_Object_t   TimerSendData_Id;
...
static char a_GapDeviceName[] = {  'P', 'o', 'w', 'e', 'r', ' ', 'C', 'e', 'n', 't', 'r', 'a', 'l' }; /* Gap Device Name */
...
static void APP_Send_Data_timCB(void *arg);
...
void APP_BLE_Init(void)
{
  ...
  bleAppContext.a_deviceServerBdAddr[0] =(uint8_t)((CFG_PERIPH_BD_ADDRESS & 0x0000000000FF));
  bleAppContext.a_deviceServerBdAddr[1] =(uint8_t)((CFG_PERIPH_BD_ADDRESS & 0x00000000FF00) >> 8);
  bleAppContext.a_deviceServerBdAddr[2] =(uint8_t)((CFG_PERIPH_BD_ADDRESS & 0x000000FF0000) >> 16);
  bleAppContext.a_deviceServerBdAddr[3] =(uint8_t)((CFG_PERIPH_BD_ADDRESS & 0x0000FF000000) >> 24);
  bleAppContext.a_deviceServerBdAddr[4] =(uint8_t)((CFG_PERIPH_BD_ADDRESS & 0x00FF00000000) >> 32);
  bleAppContext.a_deviceServerBdAddr[5] =(uint8_t)((CFG_PERIPH_BD_ADDRESS & 0xFF0000000000) >> 40);
  
  bleAppContext.a_deviceServerBdAddrType = GAP_PUBLIC_ADDR;
  ...
  switch (p_event_pckt->evt)
  {
    case HCI_DISCONNECTION_COMPLETE_EVT_CODE:
    {
      ...
      if (p_disconnection_complete_event->Connection_Handle == bleAppContext.connectionHandle)
      {
        ...
        bleAppContext.SendDataStatus = SENDING_DATA_OFF;
        UTIL_TIMER_Stop(&(bleAppContext.TimerSendData_Id));
    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
    {
      ...
      switch (p_blecore_evt->ecode)
      {
        ...
        case ACI_GAP_PROC_COMPLETE_VSEVT_CODE:
        {
          ...
          {
            LOG_ERROR_APP("-- GAP_GENERAL_DISCOVERY_PROC completed\n");
    ...
    while(i < adv_data_size)
    {
      ...
      switch (ad_type)
      {
        ...
        case AD_TYPE_MANUFACTURER_SPECIFIC_DATA:
        {
          ...
          {
            LOG_ERROR_APP("  P2P server, db addr 0x%02X:%02X:%02X:%02X:%02X:%02X\n",
...
static void Connect_Request(void)
{
  ...
  {
    LOG_ERROR_APP("Create connection to BLE_Peripheral_PwrCo\n");

    result = aci_gap_create_connection(SCAN_INT_MS(500u), SCAN_WIN_MS(500u),
                                       bleAppContext.a_deviceServerBdAddrType,
                                       &bleAppContext.a_deviceServerBdAddr[0],
                                       CFG_BD_ADDRESS_TYPE,
                                       CONN_INT_MS(CONN_INTERVAL), CONN_INT_MS(CONN_INTERVAL),
                                       0u,
    ...
    if (result == BLE_STATUS_SUCCESS)
    {
      ...
      LOG_ERROR_APP("  wait for event HCI_LE_CONNECTION_COMPLETE_SUBEVT_CODE\n");
...
void APP_BLE_Key_Button1_Action(void)
{
  if (bleAppContext.Device_Connection_Status == APP_BLE_CONNECTED_CLIENT)
  {
    if(bleAppContext.SendDataStatus == SENDING_DATA_OFF)
    {
      bleAppContext.SendDataStatus = SENDING_DATA_ON;
      UTIL_TIMER_Stop(&(bleAppContext.TimerSendData_Id));
      UTIL_TIMER_StartWithPeriod( &(bleAppContext.TimerSendData_Id), WRITE_TX_INT_MS);
    }
    else
    {
      bleAppContext.SendDataStatus = SENDING_DATA_OFF;
...
static void APP_Send_Data_timCB(void *arg)
{
   ...
   */
  UTIL_SEQ_SetTask(1<<CFG_TASK_POCO_WRITE_CHAR_ID, CFG_SEQ_PRIO_0);
...
void APP_BLE_Key_Button2_Action(void)
{
  ...
  if (bleAppContext.Device_Connection_Status != APP_BLE_CONNECTED_CLIENT)
  {
    ...
    if (ret != BLE_STATUS_SUCCESS)
    {
      LOG_ERROR_APP("==>> aci_gap_clear_security_db - Fail, result: 0x%02X \n", ret);
    }
    else
    {
...
void APP_BLE_Key_Button3_Action(void)
{
  ...
  if (bleAppContext.Device_Connection_Status != APP_BLE_CONNECTED_CLIENT)
  {
    LOG_INFO_APP("  ** No device connected **\n");
  }
  else
  {
    LOG_INFO_APP("  ** CONNECTION UPDATE **\n");
    ...
    aci_gap_start_connection_update(bleAppContext.connectionHandle,
