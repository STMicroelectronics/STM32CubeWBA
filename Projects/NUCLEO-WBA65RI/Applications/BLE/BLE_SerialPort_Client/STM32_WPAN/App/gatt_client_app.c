...
typedef struct
{
  ...
  /* SVC_ID_SPC Serial Port Client characteristics handle */
  /* handles of the SerialPort service */
  uint16_t SPCServiceHdl;
  uint16_t SPCServiceEndHdl;

  /* handles of the Tx characteristic - Write To Server */
  uint16_t SPCWriteToServerCharHdl;
  uint16_t SPCWriteToServerValueHdl;
  uint16_t SPCWriteToServerDescHdl;

  /* handles of the Rx characteristic - Notification From Server */
  uint16_t SPCNotificationCharHdl;
  uint16_t SPCNotificationValueHdl;
  uint16_t SPCNotificationDescHdl;
...
#define SPC_SERVICE_UUID                0xF2E0
#define ST_SPC_TX_CHAR_UUID             0xF2E1
#define ST_SPC_RX_CHAR_UUID             0xF2E2
...
/* USER CODE BEGIN PFP */
static void SPCclient_write_char(void);
...
void GATT_CLIENT_APP_Init(void)
{
  ...
  for(index = 0; index < CFG_BLE_NUM_CLIENT_CONTEXTS; index++)
  {
    ...
    for(svcIndex = 0; svcIndex < SVC_ID_MAX; svcIndex++)
    {
      ...
      a_ClientContext[index].connHdl = 0;
    ...
    a_ClientContext[index].svcInfo[SVC_ID_SPC].uuid  = SPC_SERVICE_UUID;
  ...
  UTIL_SEQ_RegTask(1U << CFG_TASK_WRITE_RX_CHAR_ID, UTIL_SEQ_RFU, SPCclient_write_char);

  LOG_INFO_APP("-- SERIAL PORT CLIENT INITIALIZED \n");
  ...
  else
  {
    ...
    switch (GattProcId)
    {
      ...
      case PROC_GATT_DISC_ALL_CHARS:
      {
        ...
        for (svcIdx = 0; svcIdx < SVC_ID_MAX; svcIdx++)
        {
             ...
             && a_ClientContext[index].svcInfo[svcIdx].uuid != 0)
          {
            ...
            if(a_ClientContext[index].SPCServiceHdl != 0)
            {            
              LOG_INFO_APP("  Service SPC_SERVICE_UUID 0x%04X, [0x%04X - 0x%04X]\n",
                                SPC_SERVICE_UUID,
                                a_ClientContext[index].SPCServiceHdl,
                                a_ClientContext[index].SPCServiceEndHdl);
              LOG_INFO_APP("  aci_gatt_disc_all_char_of_service(SPC_SERVICE_UUID\n");
              bleStatus = aci_gatt_disc_all_char_of_service(
                                SPC_SERVICE_UUID,
                                a_ClientContext[index].SPCServiceHdl,
                                a_ClientContext[index].SPCServiceEndHdl);
              ...
              {
                gatt_cmd_resp_wait();
                LOG_INFO_APP("  Service SPC_SERVICE_UUID 0x%04X characteristics discovered Successfully\n\n", SPC_SERVICE_UUID);
              }
              else
              {
                LOG_ERROR_APP("  Service SPC_SERVICE_UUID 0x%04X characteristics discovery Failed, status =0x%02X\n\n", SPC_SERVICE_UUID, bleStatus);
                status++;
              }
      ...
      case PROC_GATT_PROPERTIES_ENABLE_ALL:
      {
        ...
        if(a_ClientContext[index].SPCNotificationDescHdl != 0x0000)
        {
          charPropVal = 0x0001;
          
          bleStatus = aci_gatt_write_char_desc(a_ClientContext[index].connHdl,
                                       a_ClientContext[index].SPCNotificationDescHdl,
                                       2,
                                       (uint8_t *) &charPropVal);
          ...
          {
          gatt_cmd_resp_wait();
            LOG_INFO_APP("aci_gatt_write_char_desc success SPCNotificationDescHdl =0x%04X\n",a_ClientContext[index].SPCNotificationDescHdl);
          }
          else
          {
            LOG_ERROR_APP(" SPCNotificationDescHdl write Failed, status =0x%02X\n", bleStatus);
            status++;
          }
        }
    ...
    */
    ...
    for (i = 0; i < numServ; i++)
    {
      ...
      for (j = 0; j < SVC_ID_MAX; j++)
      {
        ...
        else if (uuid == SPC_SERVICE_UUID)
        {
          a_ClientContext[index].SPCServiceHdl = ServiceStartHdl;
          a_ClientContext[index].SPCServiceEndHdl = ServiceEndHdl;

          LOG_INFO_APP(", SPC_SERVICE_UUID found 0x%04X\n", uuid);
          LOG_INFO_APP("\t\t\t, SPCServiceHdl 0x%04X\n", a_ClientContext[index].SPCServiceHdl);
          LOG_INFO_APP("\t\t\t, SPCServiceEndHdl 0x%04X\n", a_ClientContext[index].SPCServiceEndHdl);
        }
...
/* USER CODE BEGIN gatt_parse_chars_1 */
        else if (uuid == ST_SPC_TX_CHAR_UUID)
        {
          st_spc_tx_handle = CharStartHdl;
          a_ClientContext[index].SPCNotificationCharHdl = CharStartHdl;
          a_ClientContext[index].SPCNotificationValueHdl = CharValueHdl;
          LOG_INFO_APP(", ST_SPC_TX_CHAR_UUID charac found [0x%04X, 0x%04X]\n", CharStartHdl, CharValueHdl);
        }
        else if (uuid == ST_SPC_RX_CHAR_UUID)
        {
          rx_handle = CharStartHdl;
          a_ClientContext[index].SPCWriteToServerCharHdl = CharStartHdl;
          a_ClientContext[index].SPCWriteToServerValueHdl = CharValueHdl;
          LOG_INFO_APP(", ST_SPC_RX_CHAR_UUID charac found [0x%04X, 0x%04X]\n", CharStartHdl, CharValueHdl);
        } 
...
/* USER CODE BEGIN gatt_parse_descs_1 */
        ...
        else if (a_ClientContext[index].SPCWriteToServerValueHdl == gattCharValueHdl)
        {
          UNUSED(gattCharStartHdl);
          a_ClientContext[index].SPCWriteToServerDescHdl = handle;
          LOG_INFO_APP(", SPCWrite found : Desc UUID=0x%04X handle=0x%04X-0x%04X-0x%04X\n",
                      uuid, gattCharStartHdl, gattCharValueHdl, handle);
        }
        else if (a_ClientContext[index].SPCNotificationValueHdl == gattCharValueHdl)
        {
          UNUSED(gattCharStartHdl);
          a_ClientContext[index].SPCNotificationDescHdl = handle;
          LOG_INFO_APP(", SerialPortNotification found : Desc UUID=0x%04X handle=0x%04X-0x%04X-0x%04X\n",
                      uuid, gattCharStartHdl, gattCharValueHdl, handle);
        }
...
/* USER CODE BEGIN gatt_parse_descs_2 */
        ...
        {
          LOG_INFO_APP("\t\t\t, found ST_SPC_TX_CHAR_UUID\n");
        }
        else if (uuid == ST_SPC_RX_CHAR_UUID) 
        {
          LOG_INFO_APP("\t\t\t, found ST_SPC_RX_CHAR_UUID\n");
        }
...
/* USER CODE BEGIN gatt_parse_notification_1 */
  ...
  uint16_t attr_handle = p_evt->Attribute_Handle;

  /* Safety check: prevent buffer overflow */
  if (p_evt->Attribute_Value_Length > sizeof(p_evt->Attribute_Value))
  {
      LOG_INFO_APP("Error: Attribute_Value_Length too large!\n");
      return ret;
  }

  if (attr_handle == st_spc_tx_handle + 1) 
  {
      My_Log_Module_Print( LOG_VERBOSE_INFO, LOG_REGION_APP, "Received data: ");
      for (int i = 0; i < p_evt->Attribute_Value_Length; i++)
      {
          My_Log_Module_Print( LOG_VERBOSE_INFO, LOG_REGION_APP, "%c", p_evt->Attribute_Value[i]);
      }
      My_Log_Module_Print( LOG_VERBOSE_INFO, LOG_REGION_APP, "\n");
  }
  else
  {
      LOG_INFO_APP("Received data from unrecognized attribute handle 0x%04X\n", p_evt->Attribute_Handle);
  }
...
static void SPCclient_write_char(void)
{
  ...
  for(index = 0; index < CFG_BLE_NUM_CLIENT_CONTEXTS; index++)
  {
    if (a_ClientContext[index].connHdl != 0xFFFF)
    {
      if(a_ClientContext[index].SPCWriteToServerValueHdl != 0)
      {
        // Log the data to be sent
        My_Log_Module_Print(LOG_VERBOSE_INFO, LOG_REGION_APP, "Sending data: ");
        for (uint16_t i = 0; i < buffSpTxLen; i++)
        {
          My_Log_Module_Print(LOG_VERBOSE_INFO, LOG_REGION_APP, "%c", a_buffSpTx[i]);
        }
        My_Log_Module_Print(LOG_VERBOSE_INFO, LOG_REGION_APP, "\n");

        ret = aci_gatt_write_without_resp(a_ClientContext[index].connHdl,
                                          a_ClientContext[index].SPCWriteToServerValueHdl,
                                          buffSpTxLen,
                                          (const uint8_t *)&a_buffSpTx);

        if (ret != BLE_STATUS_SUCCESS)
        {
          LOG_INFO_APP("aci_gatt_write_without_resp failed, connHdl=0x%04X, ValueHdl=0x%04X, result: 0x%02X\n",
                        a_ClientContext[index].connHdl,
                        a_ClientContext[index].SPCWriteToServerValueHdl,
                        ret);
        }
      ...
      {
        LOG_INFO_APP("ERROR  - SPCclient_write_char - a_ClientContext[0x%02X].SPCWriteToServerValueHdl = 0x%04X\n", index, a_ClientContext[index].SPCWriteToServerValueHdl);
      }
    }
  }
