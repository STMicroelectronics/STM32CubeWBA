...
typedef struct{
  uint16_t  Df_tagSvcHdle;                  /**< Df_tag Service Handle */
...
/* Private variables ---------------------------------------------------------*/
static const uint16_t SizeLed_C = 1;
static const uint16_t SizeSwitch_C = 1;

static DF_TAG_Context_t DF_TAG_Context;
...
static SVCCTL_EvtAckStatus_t DF_TAG_EventHandler(void *p_pckt);
 ...
 */
#define COPY_DF_TAG_UUID(uuid_struct)       COPY_UUID_128(uuid_struct,0x00,0x00,0xfe,0x40,0xcc,0x7a,0x48,0x2a,0x98,0x4a,0x7f,0x2e,0xd5,0xb3,0xe5,0x8f)
 ...
 */
static SVCCTL_EvtAckStatus_t DF_TAG_EventHandler(void *p_Event)
{
  SVCCTL_EvtAckStatus_t return_value;
  hci_event_pckt *p_event_pckt;
  evt_blecore_aci *p_blecore_evt;
  aci_gatt_attribute_modified_event_rp0 *p_attribute_modified;
  DF_TAG_NotificationEvt_t                 notification;
    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
      ...
      switch(p_blecore_evt->ecode)
      {
        case ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE:
        {
          ...
          if(p_attribute_modified->Attr_Handle == (DF_TAG_Context.Switch_CCharHdle + CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))
          {
            ...
            switch(p_attribute_modified->Attr_Data[0])
            {
              ...
              case (0x00):
                ...
                notification.EvtOpcode = DF_TAG_SWITCH_C_NOTIFY_DISABLED_EVT;
                DF_TAG_Notification(&notification);
              ...
              case GATT_CHAR_UPDATE_SEND_NOTIFICATION:
                ...
                notification.EvtOpcode = DF_TAG_SWITCH_C_NOTIFY_ENABLED_EVT;
                DF_TAG_Notification(&notification);
          ...
          else if(p_attribute_modified->Attr_Handle == (DF_TAG_Context.Led_CCharHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
          {
            return_value = SVCCTL_EvtAckFlowEnable;

            notification.EvtOpcode = DF_TAG_LED_C_WRITE_NO_RESP_EVT;
...
void DF_TAG_Init(void)
{
   ...
   */
  SVCCTL_RegisterSvcHandler(DF_TAG_EventHandler);

  /**
   * DF_Tag
   *
   * Max_Attribute_Records = 1 + 2*2 + 1*no_of_char_with_notify_or_indicate_property + 1*no_of_char_with_broadcast_property
   * service_max_attribute_record = 1 for DF_Tag +
   *                                2 for LED_C +
   *                                2 for SWITCH_C +
  ...
  COPY_DF_TAG_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_service(UUID_TYPE_128,
                             (Service_UUID_t *) &uuid,
                             PRIMARY_SERVICE,
                             max_attr_record,
                             &(DF_TAG_Context.Df_tagSvcHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    LOG_INFO_BLE("  Fail   : aci_gatt_add_service command: DF_Tag, error code: 0x%02X\n", ret);
  }
  else
  {
    LOG_INFO_BLE("  Success: aci_gatt_add_service command: Df_tagSvcHdle = 0x%04X\n",DF_TAG_Context.Df_tagSvcHdle);
  }
  ...
  ret = aci_gatt_add_char(DF_TAG_Context.Df_tagSvcHdle,
                          UUID_TYPE_128,
                          (Char_UUID_t *) &uuid,
                          SizeLed_C,
  ...
  ret = aci_gatt_add_char(DF_TAG_Context.Df_tagSvcHdle,
                          ...
                          0x10,
                          CHAR_VALUE_LEN_CONSTANT,
                          &(DF_TAG_Context.Switch_CCharHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
...
tBleStatus DF_TAG_UpdateValue(DF_TAG_CharOpcode_t CharOpcode, DF_TAG_Data_t *pData)
{
  ...
  {
    case DF_TAG_LED_C:
      ret = aci_gatt_update_char_value(DF_TAG_Context.Df_tagSvcHdle,
                                       DF_TAG_Context.Led_CCharHdle,
                                       0, /* charValOffset */
