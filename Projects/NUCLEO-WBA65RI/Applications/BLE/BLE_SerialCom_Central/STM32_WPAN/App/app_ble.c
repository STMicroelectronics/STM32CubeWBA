    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
    {
      ...
      switch (p_blecore_evt->ecode)
      {
        ...
        case ACI_L2CAP_CONNECTION_UPDATE_REQ_VSEVT_CODE:
        {
          ...
          {
            LOG_INFO_APP("  Fail   : aci_l2cap_connection_parameter_update_resp command, result: 0x%02X\n", ret);
          ...
          if (ret != BLE_STATUS_SUCCESS)
          {
            LOG_INFO_APP("  Fail   : aci_hal_set_radio_activity_mask command, result: 0x%02X\n", ret);
          }
          else
          {
        ...
        case ACI_GAP_PASS_KEY_REQ_VSEVT_CODE:
        {
          ...
          ret = aci_gap_pass_key_resp(bleAppContext.connectionHandle, CFG_FIXED_PIN);
          if (ret != BLE_STATUS_SUCCESS)
          {
...
void BLE_SVC_L2CAP_Conn_Update(uint16_t ConnectionHandle, uint16_t conn1, uint16_t conn2)
{
    ...
    if (ret != BLE_STATUS_SUCCESS)
    {
      LOG_INFO_APP("BLE_SVC_L2CAP_Conn_Update(), Failed\n");
    }
    else
    {
      LOG_INFO_APP("BLE_SVC_L2CAP_Conn_Update(), Successfully\n");
    }
    ...
    while(i < adv_data_size)
    {
      ...
      switch (ad_type)
      {
        ...
        case AD_TYPE_MANUFACTURER_SPECIFIC_DATA:
          ...
          {
            LOG_INFO_APP("--- ST MANUFACTURER data BlueSTv2\n");
            if(p_adv_data[i + 6] == CFG_DEV_ID_BLE_COC)
            {
              ...
              LOG_INFO_APP("-- COC peripheral detected, db addr 0x%02X:%02X:%02X:%02X:%02X:%02X\n",
            ...
            {
              LOG_INFO_APP("-- COC peripheral not detected\n");
            }
          }
          else if ((ad_length >= 7) && (p_adv_data[i + 2] == 0x01))
          {
            LOG_INFO_APP("--- ST MANUFACTURER data BlueSTv1\n");
            if(p_adv_data[i + 3] == CFG_DEV_ID_BLE_COC)
            {
              ...
              LOG_INFO_APP("-- COC peripheral, db addr 0x%02X:%02X:%02X:%02X:%02X:%02X\n",
                              bleAppContext.a_deviceServerBdAddr[5],
                              bleAppContext.a_deviceServerBdAddr[4],
                              bleAppContext.a_deviceServerBdAddr[3],
                              bleAppContext.a_deviceServerBdAddr[2],
...
static void Connect_Request(void)
{
  ...
  if (bleAppContext.deviceServerFound != 0)
  {
    ...
    if (result == BLE_STATUS_SUCCESS)
    {
      ...
      LOG_INFO_APP("==>> Success: Create connection to SC_WBAxx\n");
      UTIL_SEQ_WaitEvt(1U << CFG_IDLEEVT_CONNECTION_COMPLETE);

    }
    else
    {
      LOG_INFO_APP("==>> Fail: Create connection to SC_WBAxx , result: 0x%02X\n", result);
      #if (CFG_LED_SUPPORTED == 1)
      BSP_LED_On(LED_RED);
      #endif
      bleAppContext.Device_Connection_Status = APP_BLE_IDLE;
...
static void Scan_Request(void)
{
  ...
  if (bleAppContext.Device_Connection_Status != APP_BLE_CONNECTED_CLIENT)
  {
    ...
    if (result == BLE_STATUS_SUCCESS)
    {
      LOG_INFO_APP("** START GENERAL DISCOVERY (SCAN) **\n");
    }
    else
    {
      #if (CFG_LED_SUPPORTED == 1)
      BSP_LED_On(LED_RED);
      #endif
      LOG_INFO_APP("-- BLE_App_Start_Limited_Disc_Req, Failed 0x%02X\n", result);
    }
  }
  BleStackCB_Process();
...
void APP_BSP_Button3Action(void)
{
  ...
  else
  {
    ...
    UTIL_SEQ_SetTask(1U << CFG_TASK_CENTRAL_CONN_INTERV_UPDATE_ID, CFG_SEQ_PRIO_0);
  }
  return;
}
#endif
