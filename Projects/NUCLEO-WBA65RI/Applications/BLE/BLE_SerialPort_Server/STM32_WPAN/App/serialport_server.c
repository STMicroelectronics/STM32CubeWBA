...
static SVCCTL_EvtAckStatus_t SERIALPORT_SERVER_EventHandler(void *p_pckt);
...
/*
 ...
 generator:
 D973F2E0B19E11E29E960800200C9A66: Service 128bits UUID
 D973F2E1B19E11E29E960800200C9A66: Characteristic 128bits UUID
 D973F2E2B19E11E29E960800200C9A66: Characteristic 128bits UUID
 */
#define COPY_SERIALPORT_SERVER_UUID(uuid_struct)       COPY_UUID_128(uuid_struct,0xd9,0x73,0xf2,0xe0,0xb1,0x9e,0x11,0xe2,0x9e,0x96,0x08,0x00,0x20,0x0c,0x9a,0x66)
#define COPY_TX_UUID(uuid_struct)       COPY_UUID_128(uuid_struct,0xd9,0x73,0xf2,0xe1,0xb1,0x9e,0x11,0xe2,0x9e,0x96,0x08,0x00,0x20,0x0c,0x9a,0x66)
#define COPY_RX_UUID(uuid_struct)       COPY_UUID_128(uuid_struct,0xd9,0x73,0xf2,0xe2,0xb1,0x9e,0x11,0xe2,0x9e,0x96,0x08,0x00,0x20,0x0c,0x9a,0x66)
...
void Data_Received(uint16_t length, uint8_t *data)
{
  ...
  {
     My_Log_Module_Print( LOG_VERBOSE_INFO, LOG_REGION_APP, "%c", data[i]);
  }  
 ...
 */
static SVCCTL_EvtAckStatus_t SERIALPORT_SERVER_EventHandler(void *p_Event)
{
      ...
      switch(p_blecore_evt->ecode)
      {
        case ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE:
        {
          ...
          else if(p_attribute_modified->Attr_Handle == (SERIALPORT_SERVER_Context.RxCharHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
          {
            ...
            Data_Received(p_attribute_modified->Attr_Data_Length, p_attribute_modified->Attr_Data);
...
void SERIALPORT_SERVER_Init(void)
{
   ...
   */
  SVCCTL_RegisterSvcHandler(SERIALPORT_SERVER_EventHandler);

  /**
   * SerialPort_server
   *
   * Max_Attribute_Records = 1 + 2*2 + 1*no_of_char_with_notify_or_indicate_property + 1*no_of_char_with_broadcast_property
   * service_max_attribute_record = 1 for SerialPort_server +
   *                                2 for TX +
   *                                2 for RX +
  ...
  COPY_SERIALPORT_SERVER_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_service(UUID_TYPE_128,
                             (Service_UUID_t *) &uuid,
                             PRIMARY_SERVICE,
                             max_attr_record,
                             &(SERIALPORT_SERVER_Context.Serialport_serverSvcHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    LOG_INFO_BLE("  Fail   : aci_gatt_add_service command: SerialPort_server, error code: 0x%02X\n", ret);
  }
  else
  {
    LOG_INFO_BLE("  Success: aci_gatt_add_service command: Serialport_serverSvcHdle = 0x%04X\n",SERIALPORT_SERVER_Context.Serialport_serverSvcHdle);
  }
  ...
  ret = aci_gatt_add_char(SERIALPORT_SERVER_Context.Serialport_serverSvcHdle,
                          UUID_TYPE_128,
                          (Char_UUID_t *) &uuid,
                          SizeTx,
                          CHAR_PROP_NOTIFY,
                          ...
                          &(SERIALPORT_SERVER_Context.TxCharHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    LOG_INFO_BLE("  Fail   : aci_gatt_add_char command   : TX, error code: 0x%02X\n", ret);
  }
  else
  {
    LOG_INFO_BLE("  Success: aci_gatt_add_char command   : TxCharHdle = 0x%04X\n",SERIALPORT_SERVER_Context.TxCharHdle);
  }
  ...
  ret = aci_gatt_add_char(SERIALPORT_SERVER_Context.Serialport_serverSvcHdle,
                          ...
                          0x10,
                          CHAR_VALUE_LEN_CONSTANT,
                          &(SERIALPORT_SERVER_Context.RxCharHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    LOG_INFO_BLE("  Fail   : aci_gatt_add_char command   : RX, error code: 0x%02X\n", ret);
  }
  else
  {
    LOG_INFO_BLE("  Success: aci_gatt_add_char command   : RxCharHdle = 0x%04X\n",SERIALPORT_SERVER_Context.RxCharHdle);
  }
...
tBleStatus SERIALPORT_SERVER_UpdateValue(SERIALPORT_SERVER_CharOpcode_t CharOpcode, SERIALPORT_SERVER_Data_t *pData)
{
  ...
  switch(CharOpcode)
  {
    case SERIALPORT_SERVER_TX:
      ret = aci_gatt_update_char_value(SERIALPORT_SERVER_Context.Serialport_serverSvcHdle,
                                       SERIALPORT_SERVER_Context.TxCharHdle,
                                       0, /* charValOffset */
      ...
      if (ret != BLE_STATUS_SUCCESS)
      {
        LOG_INFO_BLE("  Fail   : aci_gatt_update_char_value TX command, error code: 0x%02X\n", ret);
      }
      else
      {
        LOG_INFO_BLE("  Success: aci_gatt_update_char_value TX command\n");
      }
    ...
    case SERIALPORT_SERVER_RX:
      ...
      if (ret != BLE_STATUS_SUCCESS)
      {
        LOG_INFO_BLE("  Fail   : aci_gatt_update_char_value RX command, error code: 0x%02X\n", ret);
      }
      else
      {
        LOG_INFO_BLE("  Success: aci_gatt_update_char_value RX command\n");
      }
