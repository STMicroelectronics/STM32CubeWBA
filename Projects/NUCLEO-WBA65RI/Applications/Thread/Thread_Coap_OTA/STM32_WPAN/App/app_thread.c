...
#define FUOTA_MAGIC_KEYWORD                     FUOTA_MAGIC_KEYWORD_M33_APP
/**
 * These are the two tags used to manage a power failure during OTA
 * The MagicKeywordAddress shall be mapped @0x160 from start of the binary image
...
static void APP_THREAD_FuotaRebootTimerCb(void *args);
...
static UTIL_TIMER_Object_t fuotaRebootTimerId;

PLACE_IN_SECTION("TAG_OTA_END") const uint32_t lMagicKeywordValue = FUOTA_MAGIC_KEYWORD_M33_APP;
__attribute__((used)) PLACE_IN_SECTION("TAG_OTA_START") const uint32_t lMagicKeywordAddress = ( uint32_t )&lMagicKeywordValue;
...
static void APP_THREAD_DeviceConfig(void)
{
  ...
  otCoapAddResource(PtOpenThreadInstance, &OT_Ressource_FuotaReboot);
...
static void APP_THREAD_AppInit(void)
{
  UTIL_TIMER_Create( &fuotaRebootTimerId, 0, UTIL_TIMER_ONESHOT, &APP_THREAD_FuotaRebootTimerCb, NULL );
  
  /* disable OpenThread Logs */
  setDebugLevel(0);
  
  LOG_INFO_APP("+++++++++++++++++++++++++++++++++++++++++++++++++++++");
  LOG_INFO_APP("Thread_Coap_Generic_Ota Example with REBOOT RESOURCE: FUOTA_REBOOT");
  LOG_INFO_APP("+++++++++++++++++++++++++++++++++++++++++++++++++++++");
}
...
{
  LOG_INFO_APP("Reset to OTA Client FW Application");

  APP_THREAD_FuotaRebootTimerCb(NULL);
}
...
{
  uint32_t l_data = 0;
  LOG_INFO_APP(" Received CoAP request on %s", C_RESSOURCE_FUOTA_REBOOT);
  ...
  /* Set a timer to wait before rebooting to Thread_OTA_Client, 
     to give time to the stack to send the response before the reboot*/
  UTIL_TIMER_StartWithPeriod(&fuotaRebootTimerId, FUOTA_REBOOT_WAIT_TIME_MS);
}
...
static void APP_THREAD_FuotaRebootTimerCb(void *args)
{
  ...
  LOG_INFO_APP("REBOOT on Thread_Ota");
  /**
