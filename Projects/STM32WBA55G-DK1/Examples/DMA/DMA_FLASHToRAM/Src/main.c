...
/* USER CODE BEGIN PFP */
/* Private function prototypes -----------------------------------------------*/
static void TransferComplete(DMA_HandleTypeDef *handle_GPDMA1_Channel7);
static void TransferError(DMA_HandleTypeDef *handle_GPDMA1_Channel7);
...
int main(void)
{
  ...
  if (HAL_DMA_Start_IT(&handle_GPDMA1_Channel7, (uint32_t)&aSRC_Const_Buffer, (uint32_t)&aDST_Buffer, sizeof(aDST_Buffer)) != HAL_OK)
  {
    /* Transfer Error */
    Error_Handler();
  }
...
void Error_Handler(void)
{
  ...
  while (1)
  {
    /* Toggle LD3 with a period of 1 s */
    BSP_LED_Toggle(LD3);
    HAL_Delay(1000);
  }
