...
#if (CFG_LCD_SUPPORTED == 1)
static void LCD_DrawIcon(uint16_t Xpos, uint16_t Ypos, uint16_t width, uint16_t height, const uint8_t *pIcon);
#endif /* CFG_LCD_SUPPORTED */
/* USER CODE END PFP */
...
static void HRS_APP_Measurements(void)
{
  ...
  UTIL_LCD_DisplayStringAt(0, LINE(2), (uint8_t *)measurementText, RIGHT_MODE);
