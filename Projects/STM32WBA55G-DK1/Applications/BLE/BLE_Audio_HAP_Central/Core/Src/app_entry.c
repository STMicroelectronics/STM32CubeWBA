...
void AudioClock_Init(uint32_t audio_frequency_type)
{
  ...
  Codec_Frequency = audio_frequency_type;
...
static void BSP_MIC_Gain_Init(const uint8_t mode)
{
  ...
  else
  {
    ...
    BSP_I2C3_WriteReg(AUDIO_I2C_ADDRESS, CS42L51_ADCA_ATTENUATOR, &attenuator_gain, 0x01);
    BSP_I2C3_WriteReg(AUDIO_I2C_ADDRESS, CS42L51_MIC_CTRL, &mic_control, 0x01);
  }
}
                  ...
                  AudioDriverConfig driver_config)
{
  ...
  else if (driver_config == AUDIO_DRIVER_CONFIG_LINEIN)
  {
...
int32_t Start_TxAudio(void)
{
  ...
  if ((status == 0) && (HAPAPP_Context.num_cis_established > 0))
  {
    Audio_Role_t Audio_Role = HAPAPP_Context.audio_role_setup;
