...
tBleStatus PBP_PBA_AddBroadcastSource(uint16_t ConnHandle, BAP_BA_Broadcast_Source_Add_t *pBroadcastSource)
{
  ...
  if (PBP_Context.Role & PBP_ROLE_PUBLIC_BROADCAST_ASSISTANT)
  {
    ret = CAP_BroadcastAssistant_AddBroadcastSource(ConnHandle, pBroadcastSource);
  }
#endif /* (BLE_CFG_PBP_PUBLIC_BROADCAST_ASSISTANT_ROLE == 1u) */
  return ret;
}
                                                        ...
                                                        BAP_BA_Broadcast_Source_Modify_t *pBroadcastSource)
{
  ...
  if (PBP_Context.Role & PBP_ROLE_PUBLIC_BROADCAST_ASSISTANT)
  {
    ret = CAP_BroadcastAssistant_ModifyBroadcastSource(ConnHandle, pBroadcastSource);
  }
#endif /* (BLE_CFG_PBP_PUBLIC_BROADCAST_ASSISTANT_ROLE == 1u) */
  return ret;
}
...
tBleStatus PBP_PBA_RemoveBroadcastSource(uint16_t ConnHandle, uint8_t SourceId)
{
  ...
  if (PBP_Context.Role & PBP_ROLE_PUBLIC_BROADCAST_ASSISTANT)
  {
    ret = CAP_BroadcastAssistant_RemoveBroadcastSource(ConnHandle, SourceId);
  }
#endif /* (BLE_CFG_PBP_PUBLIC_BROADCAST_ASSISTANT_ROLE == 1u) */
  return ret;
}
...
tBleStatus PBP_PBA_SetBroadcastCode(uint16_t ConnHandle, uint8_t SourceId, uint32_t *pBroadcastCode)
{
  ...
  if (PBP_Context.Role & PBP_ROLE_PUBLIC_BROADCAST_ASSISTANT)
  {
    ret = CAP_BroadcastAssistant_SetBroadcastCode(ConnHandle, SourceId, pBroadcastCode);
  }
#endif /* (BLE_CFG_PBP_PUBLIC_BROADCAST_ASSISTANT_ROLE == 1u) */
  return ret;
}
...
tBleStatus PBP_PBA_ScanOffload(uint16_t ConnHandle, uint16_t PAHandle, uint8_t SourceId)
{
  ...
  if (PBP_Context.Role & PBP_ROLE_PUBLIC_BROADCAST_ASSISTANT)
  {
    ret = CAP_BroadcastAssistant_ScanOffload(ConnHandle, PAHandle, SourceId);
  }
#endif /* (BLE_CFG_PBP_PUBLIC_BROADCAST_ASSISTANT_ROLE == 1u) */
  return ret;
}
...
void CAP_PBP_Notification(CAP_Notification_Evt_t *pNotification)
{
  switch(pNotification->EvtOpcode)
  {
    ...
    {
      if (PBP_Context.PBAPASyncState == PBP_PBA_PA_SYNC_STATE_SYNCHRONIZING)
      {
    ...
    {
      if (PBP_Context.PBAPASyncState > PBP_PBA_PA_SYNC_STATE_IDLE)
      {
        PBP_Context.PBAPASyncState = PBP_PBA_PA_SYNC_STATE_IDLE;
        PBP_Notification_Evt_t notif = {
