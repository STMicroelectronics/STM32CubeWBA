...
static const osSemaphoreAttr_t  stZbUtilWaitCallbackSemaphore_attributes =
{
...
static osSemaphoreId_t          stZbUtilWaitCallbackSemaphore;

/* Private Functions ------------------------------------------------------------------*/
static void AppZbUtil_WaitReceiveCallbackSemaphoreSet( void )
{
  if ( osSemaphoreGetCount ( stZbUtilWaitCallbackSemaphore ) == 0 )
  {
    osSemaphoreRelease( stZbUtilWaitCallbackSemaphore );
  }
}
...
static void AppZbUtil_WaitReceiveCallbackInit( void)
{
  ...
  if ( bFirst == false )
  {
    ...
    stZbUtilWaitCallbackSemaphore = osSemaphoreNew( 1U, 0U, &stZbUtilWaitCallbackSemaphore_attributes );
    if ( stZbUtilWaitCallbackSemaphore == NULL )
    {
      LOG_ERROR_APP( "FreeRtos Error : Creation of 'ZigbeeUtil Wait Callback' Semaphore failed." );
      while(1);
    }
    bFirst = true;
  }
...
static void AppZbUtil_WaitReceiveCallbackStart( uint32_t lTimeMs )
{
  ...
  osSemaphoreAcquire( stZbUtilWaitCallbackSemaphore, osWaitForever );
