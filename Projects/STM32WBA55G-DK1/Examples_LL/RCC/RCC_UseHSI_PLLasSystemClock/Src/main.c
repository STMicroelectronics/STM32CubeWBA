...
{
  {RCC_FREQUENCY_LOW, 1, 8, 8, LL_FLASH_LATENCY_0},
  {RCC_FREQUENCY_HIGH, 1, 12, 2, LL_FLASH_LATENCY_3},
};
...
int main(void)
{
  ...
  while (1)
  {
    ...
    {
      /* Problem to switch to HSI, blink LD3 */
...
void LED_Blinking(uint32_t Period)
{
  ...
  else
  {
    ...
    while (1)
    {
      /* Error if LD3 is slowly blinking (1 sec. period) */
      LL_GPIO_TogglePin(LD3_GPIO_Port, LD3_Pin);
      LL_mDelay(Period);
    }
  }
