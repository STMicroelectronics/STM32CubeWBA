...
script_error_file="ob_flash_programming.log"
...
command="\"$stm32programmercli\" -c port=$com_port br=921600 $stm32ExtLoaderFlash -e [$ext_nvm_primary_code_start $ext_nvm_primary_code_stop]"
echo "!$command!"
eval "$command"
if [ $? -ne 0 ]; then error; return 1; fi
...
command="\"$stm32programmercli\" -c port=$com_port br=921600 $stm32ExtLoaderFlash -d $OEMuROT_hex --verify"
echo "!$command!"
eval "$command"
if [ $? -ne 0 ]; then error; return 1; fi
...
command="\"$stm32programmercli\" -c port=$com_port br=921600 $stm32ExtLoaderFlash -d $oemurot_appli_hex --verify"
echo "!$command!"
eval "$command"
if [ $? -ne 0 ]; then error; return 1; fi
